...
class DataController extends Controller
{
    ...
    public function getModuleOutputTax($params)
    {
            ...
            foreach ($transactions as $transaction) {
                foreach ($transaction->invoiceLines as $invoiceLine) {
                    if ($invoiceLine->tax_rate_id == $tax['id']) {
                        ...
                        if($transaction->currency_id == $business->currency_id) {
                            $total_tax += $item_tax * $invoiceLine->quantity;
                            $total_tax_currency2 += $item_tax * $invoiceLine->quantity / $transaction->exchange_rate;
                        } else {
                            $total_tax += $item_tax * $invoiceLine->quantity * $transaction->exchagne_rate;
                            $total_tax_currency2 += $item_tax * $invoiceLine->quantity;
                        }
                    }
                }
            }
        ...
        return [
            'total_tax' => $total_tax,
            'total_tax_currency2' => $total_tax_currency2
        ];
    }
}
