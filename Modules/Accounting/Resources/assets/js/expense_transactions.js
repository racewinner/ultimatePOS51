$(document).ready(function() {
    ...
    if ($('#expense_date_range').length == 1) {
        $('#expense_date_range').daterangepicker(
            ...
            function(start, end) {
                ...
                expense_transactions_table.ajax.reload();
            }
        );
    ...
    //Expense table
    expense_transactions_table = $('#expense_transactions_table').DataTable({
        processing: true,
        serverSide: true,
        aaSorting: [[1, 'desc']],
        ajax: {
            url: '/accounting/transactions/expenses',
            data: function(d) {
        ...
        columns: [
            ...
            { data: 'tax', name: 'tr.name', visible: false},
            { data: 'final_total', name: 'final_total' },
            { data: 'payment_due', name: 'payment_due' },
            { data: 'expense_for', name: 'expense_for' },
            { data: 'contact_name', name: 'c.name', visible: false },
            { data: 'additional_notes', name: 'additional_notes' },
            { data: 'added_by', name: 'usr.first_name'}
        ],
        fnDrawCallback: function(row, data, start, end, display) {
            var expense_total = sum_table_col($('#expense_transactions_table'), 'final-total');
            var total_due = sum_table_col($('#expense_transactions_table'), 'payment_due');
            ...
            $('.footer_payment_status_count').html(
                __sum_status_html($('#expense_transactions_table'), 'payment-status')
            );
        },
