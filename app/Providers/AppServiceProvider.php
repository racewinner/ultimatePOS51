...
class AppServiceProvider extends ServiceProvider
{
    ...
    public function boot()
    {
        ...
        // Share notification values
        view()->composer('*', function ($view) {
            if (Auth::check()) {
                $user_id = auth()->user()->id;
                $all_notifications = DatabaseNotification::select('*')
                ...
                if ($unread_notifications)
                    ...
                    $total_unread = 0;
                $view->with(compact('unread_notifications', 'total_unread', 'all_notifications'));
            }
        });
        ...
        //Bl    ade directive to format number into required format.
        ...
        Blade::directive('format_currency', function ($expression) {
            ...
            $number = $values[0];
            $currency = isset($values[1]) ? $values[1] : null;
            return "<?php echo app('\App\Utils\Util')->format_currency($number, $currency); ?>";
        });
