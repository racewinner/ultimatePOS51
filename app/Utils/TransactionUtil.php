...
class TransactionUtil extends Util
{
    ...
    public function createSellTransaction($business_id, $input, $invoice_total, $user_id, $uf_data = true)
    {
        ...
        $transaction = Transaction::create([
            ...
            'nation_exchg_rate' => $input['nation_exchg_rate'],
            ...
            'invoice_token' => $this->generateToken()
        ]);
    ...
    public function deleteRemitoLines($transaction, $adjust_qty = true)
    {
        if (! empty($transaction->remito_lines)) {
            //Adjust quanity
            foreach ($transaction->remito_lines as $line) {
                if ($adjust_qty) {
                    $this->adjustQuantity($transaction->location_id, $line->product_id, $line->variation_id, $line->quantity);
                }
            }

            $remito_lines_ids = $transaction->remito_lines->pluck('id')->toArray();
            RemitoLine::whereIn('id', $remito_lines_ids)->delete();
        }        
    }
    ...
    public function getReceiptDetails($transaction_id, $location_id, $invoice_layout, $business_details, $location_details, $receipt_printer_type)
    {
        ...
        if (in_array($transaction_type, ['sell', 'sales_order'])) {
            ...
            foreach ($details['lines'] as $line) {
                if($business_details->enable_inline_tax) {
            ...
            $output['subtotal_exc_tax'] = \App\Utils\Util::format_currency($subtotal_exc_tax, $transaction->currency);
            $output['total_line_discount'] = ! empty($total_line_discount) ? \App\Utils\Util::format_currency($total_line_discount, $transaction->currency) : 0;
        ...
        } else if($transaction_type == 'remito') {
            $lines = $transaction->remito_lines()->with(['product'])->get();
            $details = $this->_receiptDetailsRemitoLines($lines, $il, $business_details);
            $output['lines'] = $details['lines'];
        }
        ...
        $output['subtotal'] = ($transaction->total_before_tax != 0) ? \App\Utils\Util::format_currency($transaction->total_before_tax, $transaction->currency) : 0;
        ...
        $output['round_off'] = \App\Utils\Util::format_currency($transaction->round_off_amount, $transaction->currency);
        $output['round_off_amount'] = $transaction->round_off_amount;
        $output['total_exempt'] = \App\Utils\Util::format_currency($total_exempt, $transaction->currency);
        $output['total_exempt_uf'] = $total_exempt;

        $taxed_subtotal = $output['subtotal_unformatted'] - $total_exempt;
        $output['taxed_subtotal'] = \App\Utils\Util::format_currency($taxed_subtotal, $transaction->currency);

        //Discount
        $discount_amount = \App\Utils\Util::format_currency($transaction->discount_amount, $transaction->currency);
        ...
        $output['discount'] = ($discount != 0) ? \App\Utils\Util::format_currency($discount, $transaction->currency) : 0;

        //reward points
        if ($business_details->enable_rp == 1 && ! empty($transaction->rp_redeemed)) {
            $output['reward_point_label'] = $business_details->rp_name;
            $output['reward_point_amount'] = \App\Utils\Util::format_currency($transaction->rp_redeemed_amount, $transaction->currency);
        }
        ...
        if (! empty($output['taxes'])) {
            ...
            foreach ($output['taxes'] as $key => $value) {
                ...
                $output['taxes'][$key] = \App\Utils\Util::format_currency($value, $transaction->currency);
            }

            $output['taxes'][trans('lang_v1.total_tax')] = \App\Utils\Util::format_currency($total_tax, $transaction->currency);
        }
        ...
        $output['tax_label'] .= ':';
        $output['tax'] = ($transaction->tax_amount != 0) ? \App\Utils\Util::format_currency($transaction->tax_amount, $transaction->currency) : 0;
        ...
        if ($transaction->tax_amount != 0 && $tax->is_tax_group) {
            ...
            foreach ($transaction_group_tax_details as $value) {
                $output['group_tax_details'][$value['name']] = \App\Utils\Util::format_currency($value['calculated_tax'], $transaction->currency);
            }
        }

        //Shipping charges
        $output['shipping_charges'] = ($transaction->shipping_charges != 0) ? \App\Utils\Util::format_currency($transaction->shipping_charges, $transaction->currency) : 0;
        ...
        $output['packing_charge'] = ($transaction->packing_charge != 0) ? \App\Utils\Util::format_currency($transaction->packing_charge, $transaction->currency) : 0;

        //Total
        if ($transaction_type == 'sell_return') {
            $output['total_label'] = $invoice_layout->cn_amount_label.':';
            $output['total'] = \App\Utils\Util::format_currency($transaction->final_total, $transaction->currency);
        } else {
        ...
        if ($transaction_type == 'sell' && $transaction->status == 'final') {
            ...
            $output['total_paid'] = ($paid_amount == 0) ? 0 : \App\Utils\Util::format_currency($paid_amount, $transaction->currency);
            $output['total_paid_label'] = $il->paid_label;
            $output['total_due'] = ($due == 0) ? 0 : \App\Utils\Util::format_currency($due, $transaction->currency);
            ...
            if ($il->show_previous_bal == 1) {
                ...
                if (! empty($all_due)) {
                    ...
                    $output['all_due'] = \App\Utils\Util::format_currency($all_due, $transaction->currency);
                }
            }
                        ...
                        if ($value['method'] == 'cash') {
                            $output['payments'][] =
                                ['method' => $method.($value['is_return'] == 1 ? ' ('.$il->change_return_label.')(-)' : ''),
                                    'amount' => \App\Utils\Util::format_currency($value['amount'], $transaction->currency),
        ...
        if (! empty($transaction->additional_expense_value_1) && ! empty($transaction->additional_expense_key_1)) {
            $output['additional_expenses'][$transaction->additional_expense_key_1] = \App\Utils\Util::format_currency($transaction->additional_expense_value_1, $transaction->currency);
        }
        if (! empty($transaction->additional_expense_value_2) && ! empty($transaction->additional_expense_key_2)) {
            $output['additional_expenses'][$transaction->additional_expense_key_2] = \App\Utils\Util::format_currency($transaction->additional_expense_value_2, $transaction->currency);
        }
        if (! empty($transaction->additional_expense_value_3) && ! empty($transaction->additional_expense_key_3)) {
            $output['additional_expenses'][$transaction->additional_expense_key_3] = \App\Utils\Util::format_currency($transaction->additional_expense_value_3, $transaction->currency);
        }
        if (! empty($transaction->additional_expense_value_4) && ! empty($transaction->additional_expense_key_4)) {
            $output['additional_expenses'][$transaction->additional_expense_key_4] = \App\Utils\Util::format_currency($transaction->additional_expense_value_4, $transaction->currency);
        }
        ...
        if (in_array($transaction_type, ['sell', 'sales_order'])) {
            ...
            if ($zatca_qr) {
                ...
                if (in_array('total_tax', $qr_code_fields)) {
                    $total_order_tax = $transaction->tax_amount + $total_line_taxes;
                    $total_order_tax_formatted = \App\Utils\Util::format_currency($total_order_tax, $transaction->currency);
    ...
    /**
     * Returns each line details for remito invoice display
     *
     * @return array
     */
    protected function _receiptDetailsRemitoLines($lines, $il, $business_details)
    {
        $is_lot_number_enabled = $business_details->enable_lot_number;
        $is_product_expiry_enabled = $business_details->enable_product_expiry;
    ...
    public function getPurchaseTotals($business_id, $start_date = null, $end_date = null, $location_id = null, $user_id = null)
    {
        $query = Transaction::from("transactions as t")
                ...
                ->select(
                    DB::raw('SUM( 
                        CASE 
                            WHEN t.currency_id is NULL or t.currency_id = b.currency_id
                            THEN t.final_total
                        END ) as final_total_sum'
                    ),
                    DB::raw('SUM( 
                        CASE 
                            WHEN t.currency_id > 0 and t.currency_id != b.currency_id
                            THEN t.final_total
                        END ) as final_total_sum_currency2'
                    ),
                    DB::raw('SUM(
                        CASE 
                            WHEN t.currency_id is NULL or t.currency_id = b.currency_id
                            THEN ( SELECT COALESCE(SUM(tp.amount), 0) 
                    ...
                    DB::raw('SUM(
                        CASE 
                            ...
                            THEN ( SELECT COALESCE(SUM(tp.amount), 0) 
                                    FROM transaction_payments as tp 
                                    WHERE tp.transaction_id=t.id )
                        END ) as total_paid_currency2'),
                    DB::raw('SUM(
                        CASE 
                            WHEN t.currency_id is NULL or t.currency_id = b.currency_id
                            THEN t.total_before_tax
                        END ) as total_before_tax_sum'
                    ),
                    DB::raw('SUM(
                        CASE 
                            WHEN t.currency_id > 0 and t.currency_id != b.currency_id
                            THEN t.total_before_tax
                        END ) as total_before_tax_sum_currency2'
                    ),
                    DB::raw('SUM(
                        CASE 
                            WHEN t.currency_id is NULL or t.currency_id = b.currency_id
                            THEN t.shipping_charges
                        END ) as total_shipping_charges'
                    ),
                    DB::raw('SUM(
                        CASE 
                            WHEN t.currency_id > 0 and t.currency_id != b.currency_id
                            THEN t.shipping_charges
                        END ) as total_shipping_charges_currency2'
                    ),
                    DB::raw('SUM(
                        CASE 
                            WHEN t.currency_id is NULL or t.currency_id = b.currency_id
                            THEN t.additional_expense_value_1 + t.additional_expense_value_2 + t.additional_expense_value_3 + t.additional_expense_value_4
                    ...
                    DB::raw('SUM(
                        CASE 
                            WHEN t.currency_id > 0 and t.currency_id != b.currency_id
                            THEN t.additional_expense_value_1 + t.additional_expense_value_2 + t.additional_expense_value_3 + t.additional_expense_value_4
                        END ) as total_expense_currency2'
                    ),
                );
        ...
        $output['total_purchase_inc_tax'] = !empty($purchase_details->final_total_sum) ? $purchase_details->final_total_sum : 0;
        $output['total_purchase_inc_tax_currency2'] = !empty($purchase_details->final_total_sum_currency2) ? $purchase_details->final_total_sum_currency2 : 0;

        //$output['total_purchase_exc_tax'] = $purchase_details->sum('total_exc_tax');
        $output['total_purchase_exc_tax'] = !empty($purchase_details->total_before_tax_sum) ? $purchase_details->total_before_tax_sum : 0;
        $output['total_purchase_exc_tax_currency2'] = !empty($purchase_details->total_before_tax_sum_currency2) ? $purchase_details->total_before_tax_sum_currency2 : 0;

        $output['purchase_due'] = $purchase_details->final_total_sum - $purchase_details->total_paid;
        $output['purchase_due_currency2'] = $purchase_details->final_total_sum_currency2 - $purchase_details->total_paid_currency2;

        $output['total_shipping_charges'] = !empty($purchase_details->total_shipping_charges) ? $purchase_details->total_shipping_charges : 0;
        $output['total_shipping_charges_currency2'] = !empty($purchase_details->total_shipping_charges_currency2) ? $purchase_details->total_shipping_charges_currency2 : 0;

        $output['total_additional_expense'] = !empty($purchase_details->total_expense) ? $purchase_details->total_expense : 0;
        $output['total_additional_expense_currency2'] = !empty($purchase_details->total_expense_currency2) ? $purchase_details->total_expense_currency2 : 0;
    ...
    public function getTotalPurchaseReturnPaid($business_id, $start_date = null, $end_date = null, $location_id = null, $created_by = null)
    {
        $query = TransactionPayment::join('transactions as t', 't.id', 'transaction_payments.transaction_id')
                                ...
                                ->where('t.business_id', $business_id)
                                ->select(
                                    DB::raw('SUM(IF(t.currency_id is NULL or t.currency_id=b.currency_id, transaction_payments.amount, 0)) as total_paid'),
                                    DB::raw('SUM(IF(t.currency_id >0 and t.currency_id != b.currency_id, transaction_payments.amount, 0)) as total_paid_currency2')
                                );
        //Check for permitted locations of a user
        $permitted_locations = auth()->user()->permitted_locations();
        if ($permitted_locations != 'all') {
        ...
        return [
            'total_purchase_return_paid' => $ret->total_paid ?? 0,
            'total_purchase_return_paid_currency2' => $ret->total_paid_currency2 ?? 0
        ];
    }
    ...
    public function getTotalSellReturnPaid($business_id, $start_date = null, $end_date = null, $location_id = null, $created_by = null)
    {
        ...
        return [
            'total_sell_return_paid' => $ret->total_paid ?? 0,
            'total_sell_return_paid_currency2' => $ret->total_paid_currency2 ?? 0
        ];
    }
    ...
    public function getSellTotals($business_id, $start_date = null, $end_date = null, $location_id = null, $created_by = null)
    {
                    ...
                    ->select(
                        ...
                        DB::raw('SUM( 
                            CASE 
                                WHEN t.currency_id > 0 and t.currency_id != b.currency_id
                                THEN t.final_total
                            END ) as total_sell_currency2'
                        ),
                        DB::raw('SUM( 
                            CASE 
                                WHEN t.currency_id is NULL or t.currency_id = b.currency_id
                                THEN t.final_total - t.tax_amount
                            END ) as total_exc_tax'
                        ),
                        DB::raw('SUM( 
                            CASE 
                                WHEN t.currency_id > 0 and t.currency_id != b.currency_id
                                THEN t.final_total - t.tax_amount
                            END ) as total_exc_tax_currency2'
                        ),
                        DB::raw('SUM( 
                            CASE 
                                WHEN t.currency_id is NULL or t.currency_id = b.currency_id
                                THEN t.final_total - (SELECT COALESCE(SUM(IF(tp.is_return = 1, -1*tp.amount, tp.amount)), 0) FROM transaction_payments as tp WHERE tp.transaction_id = t.id)
                            END ) as total_due'
                        ),
                        DB::raw('SUM( 
                            CASE 
                        ...
                        DB::raw('SUM( 
                            CASE 
                                WHEN t.currency_id > 0 and t.currency_id != b.currency_id
                                THEN t.total_before_tax
                            END ) as total_before_tax_currency2'
                        ),
                        DB::raw('SUM( 
                            CASE 
                                WHEN t.currency_id is NULL or t.currency_id = b.currency_id
        ...
        $output['total_sell_inc_tax'] = !empty($sell_details->total_sell) ? $sell_details->total_sell : 0;
        $output['total_sell_inc_tax_currency2'] = !empty($sell_details->total_sell_currency2) ? $sell_details->total_sell_currency2 : 0;

        //$output['total_sell_exc_tax'] = $sell_details->sum('total_exc_tax');

        $output['total_sell_exc_tax'] = !empty($sell_details->total_before_tax) ? $sell_details->total_before_tax : 0;
        $output['total_sell_exc_tax_currency2'] = !empty($sell_details->total_before_tax_currency2) ? $sell_details->total_before_tax_currency2 : 0;
        
        $output['invoice_due'] = !empty($sell_details->total_due) ? $sell_details->total_due : 0;
        $output['invoice_due_currency2'] = !empty($sell_details->total_due_currency2) ? $sell_details->total_due_currency2 : 0;

        $output['total_shipping_charges'] = !empty($sell_details->total_shipping_charges) ? $sell_details->total_shipping_charges : 0;
        $output['total_shipping_charges_currency2'] = !empty($sell_details->total_shipping_charges_currency2) ? $sell_details->total_shipping_charges_currency2 : 0;

        $output['total_additional_expense'] = !empty($sell_details->total_expense) ? $sell_details->total_expense : 0;
        $output['total_additional_expense_currency2'] = !empty($sell_details->total_expense_currency2) ? $sell_details->total_expense_currency2 : 0;
    ...
    public function getTotalLedgerDiscount($business_id, $start_date = null, $end_date = null)
    {
        $query = Transaction::from("transactions as t")
                    ->join('business as b', 'b.id','=','t.business_id')
                    ->where('t.business_id', $business_id)
                    ->where('t.type', 'ledger_discount')
                    ->where('t.status', 'final')
                    ->select(
                        DB::raw('SUM(IF( (t.currency_id is NULL or t.currency_id=b.currency_id) and sub_type="sell_discount", final_total, 0)) as total_sell_discount'),
                        DB::raw('SUM(IF( (t.currency_id > 0 and t.currency_id != b.currency_id) and sub_type="sell_discount", final_total, 0)) as total_sell_discount_currency2'),

                        DB::raw('SUM(IF((t.currency_id is NULL or t.currency_id=b.currency_id) and sub_type="purchase_discount", final_total, 0)) as total_purchase_discount'),
                        DB::raw('SUM(IF((t.currency_id > 0 and t.currency_id=b.currency_id) and sub_type="purchase_discount", final_total, 0)) as total_purchase_discount_currency2')
                    );
        ...
        $output['total_sell_discount_currency2'] = $sell_details->total_sell_discount_currency2 ?? 0;
        $output['total_purchase_discount'] = $sell_details->total_purchase_discount ?? 0;
        $output['total_purchase_discount_currency2'] = $sell_details->total_purchase_discount_currency2 ?? 0;
    ...
    public function getInputTax($business_id, $start_date = null, $end_date = null, $location_id = null, $contact_id = null)
    {
        ...
        $query1 = Transaction::from("transactions as tr")
                        ->leftjoin("business as b", 'b.id', '=', 'tr.business_id')
                        ->where('tr.business_id', $business_id)
                        ->leftjoin('tax_rates as T', 'tr.tax_id', '=', 'T.id')
                        ->whereIn('type', ['purchase', 'purchase_return'])
                        ->whereNotNull('tr.tax_id')
                        ->select(
                            DB::raw('SUM( 
                                CASE 
                                    WHEN tr.currency_id is NULL or tr.currency_id = b.currency_id
                                    THEN IF(type="purchase", tr.tax_amount, -1 * tr.tax_amount)
                                END ) as transaction_tax'
                            ),
                            DB::raw('SUM( 
                                CASE 
                                    WHEN tr.currency_id > 0 and tr.currency_id != b.currency_id
                                    THEN IF(type="purchase", tr.tax_amount, -1 * tr.tax_amount)
                                END ) as transaction_tax_currency2'
                            ),
                            'T.name as tax_name',
                            'T.id as tax_id',
                            'T.is_tax_group'
        ...
        $query2 = Transaction::from("transactions as tr")
                        ->leftjoin("business as b", 'b.id', '=', 'tr.business_id')
                        ->where('tr.business_id', $business_id)
                        ->leftjoin('purchase_lines as pl', 'tr.id', '=', 'pl.transaction_id')
                        ...
                        ->select(
                            DB::raw('SUM( 
                                CASE 
                                    WHEN tr.currency_id is NULL or tr.currency_id = b.currency_id
                                    THEN (pl.quantity - pl.quantity_returned) * pl.item_tax
                                END ) as product_tax'
                            ),
                            DB::raw('SUM( 
                                CASE 
        ...
        $permitted_locations = auth()->user()->permitted_locations();
        if ($permitted_locations != 'all') {
            $query1->whereIn('tr.location_id', $permitted_locations);
            $query2->whereIn('tr.location_id', $permitted_locations);
        }
        ...
        if (! empty($location_id)) {
            $query1->where('tr.location_id', $location_id);
            $query2->where('tr.location_id', $location_id);
        }

        if (! empty($contact_id)) {
            $query1->where('tr.contact_id', $contact_id);
            $query2->where('tr.contact_id', $contact_id);
        }
        ...
        foreach ($transaction_tax_details as $transaction_tax) {
            ...
            $tax_details[$transaction_tax->tax_id]['tax_amount_currency2'] = $transaction_tax->transaction_tax_currency2;
        ...
        foreach ($product_tax_details as $product_tax) {
            if (! isset($tax_details[$product_tax->tax_id])) {
                ...
                $tax_details[$product_tax->tax_id]['tax_amount_currency2'] = $product_tax->product_tax_currency2;
        ...
        $output['total_tax_currency2'] = $transaction_tax_details->sum('transaction_tax_currency2') + $product_tax_details->sum('product_tax_currency2');
    ...
    public function getOutputTax($business_id, $start_date = null, $end_date = null, $location_id = null, $contact_id = null)
    {
        ...
        $query1 = Transaction::from("transactions as tr")
                        ...
                        ->select(
                            DB::raw('SUM( 
                                CASE 
                                    WHEN tr.currency_id is NULL or tr.currency_id = b.currency_id
                                    THEN IF(type="sell", tr.tax_amount, -1 * tr.tax_amount)
                                END ) as transaction_tax'
                            ),
                            DB::raw('SUM( 
                                CASE 
        ...
        $query2 = Transaction::from("transactions as tr")
                        ->leftjoin("business as b", 'b.id', '=', 'tr.business_id')
                        ->where('tr.business_id', $business_id)
                        ->leftjoin('transaction_sell_lines as tsl', 'tr.id', '=', 'tsl.transaction_id')
                        ...
                        ->select(
                            DB::raw('SUM( 
                                CASE 
                                    WHEN tr.currency_id is NULL or tr.currency_id = b.currency_id
                                    THEN (tsl.quantity - tsl.quantity_returned) * tsl.item_tax
                                END ) as product_tax'
                            ),
                            DB::raw('SUM( 
                                CASE 
    ...
    public function getExpenseTax($business_id, $start_date = null, $end_date = null, $location_id = null, $contact_id = null)
    {
        ...
        $query = Transaction::from("transactions as tr")
                        ->leftjoin("business as b", "b.id", '=', 'tr.business_id')
                        ->where('tr.business_id', $business_id)
                        ->leftjoin('tax_rates as T', 'tr.tax_id', '=', 'T.id')
                        ->where('type', 'expense')
        ...
        $permitted_locations = auth()->user()->permitted_locations();
        if ($permitted_locations != 'all') {
            $query->whereIn('tr.location_id', $permitted_locations);
        }
        ...
        if (! empty($location_id)) {
            $query->where('tr.location_id', $location_id);
        }

        if (! empty($contact_id)) {
            $query->where('tr.contact_id', $contact_id);
        }
        ...
        $output['total_tax_currency2'] = $transaction_tax_details->sum('transaction_tax_currency2');
    ...
    public function getSellsCurrentFy($business_id, $start, $end)
    {
        $query = Transaction::from("transactions as t")
            ->join("business as b", "b.id","=","t.business_id")
            ->leftjoin('transactions as SR', function ($join) {
                $join->on('SR.return_parent_id', '=', 't.id')->where('SR.type', 'sell_return');
            })
            ->where('t.business_id', $business_id)
            ->where('t.type', 'sell')
            ->where('t.status', 'final')
        ...
        $query->select(
            DB::raw("DATE_FORMAT(t.transaction_date, '%m-%Y') as yearmonth"),
            DB::raw('SUM( IF( (t.currency_id is NULL or t.currency_id = b.currency_id), t.final_total - COALESCE(SR.final_total, 0), 0) ) as total_sells'),
            DB::raw('SUM( IF( (t.currency_id > 0 and t.currency_id != b.currency_id), t.final_total - COALESCE(SR.final_total, 0), 0) ) as total_sells_currency2'),
    ...
    public function getExpenseReport(
        ...
        $query = Transaction::from("transactions as t")
                            ->leftjoin('business as b', 'b.id', '=', 't.business_id')
        ...
        if ($type == 'by_category') {
            $expenses = $query->select(
                DB::raw("SUM( 
                    CASE
                        WHEN t.currency_id is NULL or t.currency_id = b.currency_id
                        THEN IF(t.type='expense_refund', -1 * final_total, final_total)
                    END) as total_expense"
                ),
                DB::raw("SUM( 
                    CASE
    ...
    public function getSellReturnTotal($transacion_id) 
    {
        $total_return = Transaction::where('return_parent_id', $transacion_id)->where('type', 'sell_return')->sum('final_total');
    ...
    public function calculatePaymentStatus($transaction_id, $final_amount = null)
    {
        ...
        $total_return = $this->getSellReturnTotal($transaction_id);

        $total_paid += $total_return;
    ...
    public function purchaseCurrencyDetails($business_id)
    {
        ...
        if($business->second_currency_id > 0) {
            $currency = Currency::find($business->second_currency_id);
            $secondCurrency = [
                'id' => $business->second_currency_id,
                'thousand_separator' => $currency->thousand_separator,
                'decimal_separator' => $currency->decimal_separator,
                'symbol' => $currency->symbol,
                'code' => $currency->code,
    ...
    function calculateStockValue($date, $priceQueryPart, $isSameCurrency = true, $op = 'multiple') {
        $stockValue = DB::raw("(purchase_lines.quantity - purchase_lines.quantity_returned - purchase_lines.quantity_adjusted - 
                            (SELECT COALESCE(SUM(tspl.quantity - tspl.qty_returned), 0) FROM    
                                ...
                                WHERE tspl.purchase_line_id = purchase_lines.id AND date(sale.transaction_date) <= '$date')
                            ) * $priceQueryPart");
    
        if (!$isSameCurrency) {
            if($op == 'divide') $stockValue = DB::raw("($stockValue) / purchase.exchange_rate");
            if($op == 'multiple') $stockValue = DB::raw("($stockValue) * purchase.exchange_rate");
        }
    ...
    public function getOpeningClosingStock($business_id, $date, $location_id, $is_opening = false, $by_sale_price = false, $filters = [])
    {
        ...
        $query->leftjoin('variations as v', 'v.id', '=', 'purchase_lines.variation_id')
                ...
                ->leftjoin('business as b', 'b.id', '=', 'purchase.business_id');
        ...
        $query->select(
            DB::raw("SUM(
                CASE 
                    WHEN purchase.currency_id is NULL or purchase.currency_id = b.currency_id
                    THEN
                        (purchase_lines.quantity - purchase_lines.quantity_returned - purchase_lines.quantity_adjusted -
                        (SELECT COALESCE(SUM(tspl.quantity - tspl.qty_returned), 0) FROM 
                        transaction_sell_lines_purchase_lines AS tspl
            ...
            DB::raw("SUM(
                CASE 
                    WHEN purchase.currency_id > 0 and purchase.currency_id != b.currency_id
                    THEN
                        (purchase_lines.quantity - purchase_lines.quantity_returned - purchase_lines.quantity_adjusted -
                        (SELECT COALESCE(SUM(tspl.quantity - tspl.qty_returned), 0) FROM 
                        transaction_sell_lines_purchase_lines AS tspl
        ...
        $details = $query->first();
        return [
            'stock' => !empty($details->stock) ? $details->stock : 0,
            'stock_currency2' => !empty($details->stock_currency2) ? $details->stock_currency2 : 0
        ];
    }
    ...
    public function getTransactionTotals(
        ...
        if ($permitted_locations != 'all') {
            ...
            if (in_array('payroll', $transaction_types)) {
                $query->leftjoin('users as u1', 'u1.id', '=', 't.expense_for')
        ...
        if (! empty($location_id)) {
            //if payroll check employees's work location
            if (in_array('payroll', $transaction_types)) {
                $query->leftjoin('users as u', 'u.id', '=', 't.expense_for')
        ...
        if (in_array('purchase_return', $transaction_types)) {
            $query->addSelect(
                DB::raw('SUM( 
                    CASE 
                        WHEN t.currency_id is NULL or t.currency_id = b.currency_id
                        THEN IF(t.type="purchase_return", t.final_total, 0)
                    END ) as total_purchase_return_inc_tax'
                ),
                DB::raw('SUM( 
                    CASE 
                        WHEN t.currency_id > 0 and t.currency_id != b.currency_id
                        THEN IF(t.type="purchase_return", t.final_total, 0)
                    END ) as total_purchase_return_inc_tax_currency2'
                ),
                DB::raw('SUM( 
                    CASE 
                        WHEN t.currency_id is NULL or t.currency_id = b.currency_id
                        THEN IF(t.type="purchase_return", t.total_before_tax, 0)
                    END ) as total_purchase_return_exc_tax'
                ),
                DB::raw('SUM( 
                    CASE 
                        WHEN t.currency_id > 0 and t.currency_id != b.currency_id
                        THEN IF(t.type="purchase_return", t.total_before_tax, 0)
                    END ) as total_purchase_return_exc_tax_currency2'
                ),
            );
        }
        ...
        if (in_array('sell_return', $transaction_types)) {
            $query->addSelect(
                DB::raw('SUM( 
                    CASE 
                        WHEN t.currency_id is NULL or t.currency_id = b.currency_id
                        THEN IF(t.type="sell_return", t.final_total, 0)
                    END ) as total_sell_return_inc_tax'
                ),
                DB::raw('SUM( 
                    CASE 
                        WHEN t.currency_id > 0 and t.currency_id != b.currency_id
                        THEN IF(t.type="sell_return", t.final_total, 0)
                    END ) as total_sell_return_inc_tax_currency2'
                ),
                DB::raw('SUM( 
                    CASE 
                        WHEN t.currency_id is NULL or t.currency_id = b.currency_id
                        THEN IF(t.type="sell_return", t.total_before_tax, 0)
                    END ) as total_sell_return_exc_tax'
                ),
                DB::raw('SUM( 
                    CASE 
                        WHEN t.currency_id > 0 and t.currency_id != b.currency_id
                        THEN IF(t.type="sell_return", t.total_before_tax, 0)
                    END ) as total_sell_return_exc_tax_currency2'
                ),
                DB::raw('SUM( 
                    CASE 
                        WHEN t.currency_id is NULL or t.currency_id = b.currency_id
                        THEN IF(t.type="sell_return", IF(t.discount_type = "percentage", COALESCE(t.discount_amount, 0)*t.total_before_tax/100, COALESCE(t.discount_amount, 0)), 0)
                    END ) as total_sell_return_discount'
                ),
                DB::raw('SUM( 
                    CASE 
                        WHEN t.currency_id > 0 and t.currency_id != b.currency_id
                        THEN IF(t.type="sell_return", IF(t.discount_type = "percentage", COALESCE(t.discount_amount, 0)*t.total_before_tax/100, COALESCE(t.discount_amount, 0)), 0)
                    END ) as total_sell_return_discount_currency2'
                ),
            );
        }
        ...
        if (in_array('sell_transfer', $transaction_types)) {
            $query->addSelect(
                DB::raw('SUM( 
                    CASE 
                        WHEN t.currency_id is NULL or t.currency_id = b.currency_id
                        THEN IF(t.type="sell_transfer", t.shipping_charges, 0)
                    END ) as total_transfer_shipping_charges'
                ),
                DB::raw('SUM( 
                    CASE 
                        WHEN t.currency_id > 0 and t.currency_id != b.currency_id
                        THEN IF(t.type="sell_transfer", t.shipping_charges, 0)
                    END ) as total_transfer_shipping_charges_currency2'
                )
            );
        }
        ...
        if (in_array('expense', $transaction_types)) {
            $query->addSelect(
                DB::raw('SUM( 
                    CASE 
                        WHEN t.currency_id is NULL or t.currency_id = b.currency_id
                        THEN IF(t.type="expense", t.final_total, 0)
                    END ) as total_expense'
                ),
                DB::raw('SUM( 
                    CASE 
            ...
            $query->addSelect(
                DB::raw('SUM( 
                    CASE 
                        WHEN t.currency_id is NULL or t.currency_id = b.currency_id
                        THEN IF(t.type="expense_refund", t.final_total, 0)
                    END ) as total_expense_refund'
                ),
                DB::raw('SUM( 
                    CASE 
                        WHEN t.currency_id > 0 and t.currency_id != b.currency_id
                        THEN IF(t.type="expense_refund", t.final_total, 0)
                    END ) as total_expense_refund_currency2'
                )
            );
        }
        ...
        if (in_array('payroll', $transaction_types)) {
            $query->addSelect(
                DB::raw('SUM( 
                    CASE 
                        WHEN t.currency_id is NULL or t.currency_id = b.currency_id
                        THEN IF(t.type="payroll", t.final_total, 0)
                    END ) as total_payroll'
                ),
                DB::raw('SUM( 
                    CASE 
                        WHEN t.currency_id > 0 and t.currency_id != b.currency_id
                        THEN IF(t.type="payroll", t.final_total, 0)
                    END ) as total_payroll_currency2'
                )
            );
        }
        ...
        if (in_array('stock_adjustment', $transaction_types)) {
            $query->addSelect(
                DB::raw('SUM( 
                    CASE 
                        WHEN t.currency_id is NULL or t.currency_id = b.currency_id
                        THEN IF(t.type="stock_adjustment", t.final_total, 0)
                ...
                DB::raw('SUM( 
                    CASE 
                        WHEN t.currency_id > 0 and t.currency_id != b.currency_id
                        THEN IF(t.type="stock_adjustment", t.final_total, 0)
                    END ) as total_adjustment_currency2'
                ),
                DB::raw('SUM( 
                    CASE 
                        WHEN t.currency_id is NULL or t.currency_id = b.currency_id
                        THEN IF(t.type="stock_adjustment", t.total_amount_recovered, 0)
                ...
                DB::raw('SUM( 
                    CASE 
                        WHEN t.currency_id > 0 and t.currency_id != b.currency_id
                        THEN IF(t.type="stock_adjustment", t.total_amount_recovered, 0)
                    END ) as total_recovered_currency2'
                ),
            );
        }
        ...
        if (in_array('purchase', $transaction_types)) {
            $query->addSelect(
                DB::raw('SUM( 
                    CASE 
                        WHEN t.currency_id is NULL or t.currency_id = b.currency_id
                        THEN IF(t.type="purchase", IF(t.discount_type = "percentage", COALESCE(t.discount_amount, 0)*t.total_before_tax/100, COALESCE(t.discount_amount, 0)), 0)
                    END ) as total_purchase_discount'
                ),
                DB::raw('SUM( 
                    CASE 
                        WHEN t.currency_id > 0 and t.currency_id != b.currency_id
                        THEN IF(t.type="purchase", IF(t.discount_type = "percentage", COALESCE(t.discount_amount, 0)*t.total_before_tax/100, COALESCE(t.discount_amount, 0)), 0)
                    END ) as total_purchase_discount_currency2'
                ),
            );
        }
        ...
        if (in_array('sell', $transaction_types)) {
            $query->addSelect(
                DB::raw('SUM( 
                    CASE 
                        WHEN t.currency_id is NULL or t.currency_id = b.currency_id
                        THEN IF(t.type="sell" AND t.status="final", IF(t.discount_type = "percentage", COALESCE(t.discount_amount, 0)*t.total_before_tax/100, COALESCE(t.discount_amount, 0)), 0)
                    END ) as total_sell_discount'
                ),
                DB::raw('SUM( 
                    CASE 
                        WHEN t.currency_id > 0 and t.currency_id != b.currency_id
                        THEN IF(t.type="sell" AND t.status="final", IF(t.discount_type = "percentage", COALESCE(t.discount_amount, 0)*t.total_before_tax/100, COALESCE(t.discount_amount, 0)), 0)
                    END ) as total_sell_discount_currency2'
                ),
                DB::raw('SUM( 
                    CASE 
                        WHEN t.currency_id is NULL or t.currency_id = b.currency_id
                        THEN IF(t.type="sell" AND t.status="final", rp_redeemed_amount, 0)
                    END ) as total_reward_amount'
                ),
                DB::raw('SUM( 
                    CASE 
                        WHEN t.currency_id > 0 and t.currency_id != b.currency_id
                        THEN IF(t.type="sell" AND t.status="final", rp_redeemed_amount, 0)
                    END ) as total_reward_amount_currency2'
                ),
                DB::raw('SUM( 
                    CASE 
                        WHEN t.currency_id is NULL or t.currency_id = b.currency_id
                        THEN IF(t.type="sell" AND t.status="final", t.round_off_amount, 0)
                    END ) as total_sell_round_off'
                ),
                DB::raw('SUM( 
                    CASE 
                        WHEN t.currency_id > 0 and t.currency_id != b.currency_id
                        THEN IF(t.type="sell" AND t.status="final", t.round_off_amount, 0)
                    END ) as total_sell_round_off_currency2'
                ),
            );
        }
        ...
        if (in_array('purchase_return', $transaction_types)) {
            ...
            $output['total_purchase_return_inc_tax_currency2'] = ! empty($transaction_totals->total_purchase_return_inc_tax_currency2) ?
                $transaction_totals->total_purchase_return_inc_tax_currency2 : 0;
            ...
            $output['total_purchase_return_exc_tax_currency2'] =
                ! empty($transaction_totals->total_purchase_return_exc_tax_currency2) ?
                $transaction_totals->total_purchase_return_exc_tax_currency2 : 0;
        }
        ...
        if (in_array('sell_return', $transaction_types)) {
            ...
            $output['total_sell_return_inc_tax_currency2'] =
                ! empty($transaction_totals->total_sell_return_inc_tax_currency2) ?
                $transaction_totals->total_sell_return_inc_tax_currency2 : 0;                
            ...
            $output['total_sell_return_exc_tax_currency2'] =
                ! empty($transaction_totals->total_sell_return_exc_tax_currency2) ?
                $transaction_totals->total_sell_return_exc_tax_currency2 : 0;                
            ...
            $output['total_sell_return_discount_currency2'] =
                ! empty($transaction_totals->total_sell_return_discount_currency2) ?
                $transaction_totals->total_sell_return_discount_currency2 : 0;
        }
        ...
        if (in_array('sell_transfer', $transaction_types)) {
            ...
            $output['total_transfer_shipping_charges_currency2'] =
                ! empty($transaction_totals->total_transfer_shipping_charges_currency2) ?
                $transaction_totals->total_transfer_shipping_charges_currency2 : 0;
        }

        if (in_array('expense', $transaction_types)) {
            $total_expense = ! empty($transaction_totals->total_expense) ?
                $transaction_totals->total_expense : 0;
            $total_expense_currency2 = ! empty($transaction_totals->total_expense_currency2) ?
                $transaction_totals->total_expense_currency2 : 0;
            $total_expense_refund = ! empty($transaction_totals->total_expense_refund) ?
                $transaction_totals->total_expense_refund : 0;
            $total_expense_refund_currency2 = ! empty($transaction_totals->total_expense_refund_currency2) ?
                $transaction_totals->total_expense_refund_currency2 : 0;
            $output['total_expense'] = $total_expense - $total_expense_refund;
            $output['total_expense_currency2'] = $total_expense_currency2 - $total_expense_refund_currency2;
        }
        ...
        if (in_array('payroll', $transaction_types)) {
            ...
            $output['total_payroll_currency2'] =
                ! empty($transaction_totals->total_payroll_currency2) ?
                $transaction_totals->total_payroll_currency2 : 0;                
        }
        ...
        if (in_array('stock_adjustment', $transaction_types)) {
            ...
            $output['total_adjustment_currency2'] =
                ! empty($transaction_totals->total_adjustment_currency2) ?
                $transaction_totals->total_adjustment_currency2 : 0;

            $output['total_recovered'] =
                ! empty($transaction_totals->total_recovered) ?
                $transaction_totals->total_recovered : 0;
            $output['total_recovered_currency2'] =
                ! empty($transaction_totals->total_recovered_currency2) ?
                $transaction_totals->total_recovered_currency2 : 0;
        }
        ...
        if (in_array('purchase', $transaction_types)) {
            ...
            $output['total_purchase_discount_currency2'] =
                ! empty($transaction_totals->total_purchase_discount_currency2) ?
                $transaction_totals->total_purchase_discount_currency2 : 0;
        }
        ...
        if (in_array('sell', $transaction_types)) {
            ...
            $output['total_sell_discount_currency2'] =
                ! empty($transaction_totals->total_sell_discount_currency2) ?
                $transaction_totals->total_sell_discount_currency2 : 0;                

            $output['total_reward_amount'] =
                ! empty($transaction_totals->total_reward_amount) ?
                $transaction_totals->total_reward_amount : 0;
            $output['total_reward_amount_currency2'] =
                ! empty($transaction_totals->total_reward_amount_currency2) ?
                $transaction_totals->total_reward_amount_currency2 : 0;

            $output['total_sell_round_off'] =
                ! empty($transaction_totals->total_sell_round_off) ?
                $transaction_totals->total_sell_round_off : 0;
            $output['total_sell_round_off_currency2'] =
                ! empty($transaction_totals->total_sell_round_off_currency2) ?
                $transaction_totals->total_sell_round_off_currency2 : 0;
        }
    ...
    public function getGrossProfit($business_id, $start_date = null, $end_date = null, $location_id = null, $user_id = null)
    {
        $query = TransactionSellLine::join('transactions as sale', 'transaction_sell_lines.transaction_id', '=', 'sale.id')
            ...
            ->leftjoin("business as b", "b.id",'=', "sale.business_id")
        ...
        $query->select(
            DB::raw("SUM(
                CASE
                    WHEN sale.currency_id is NULL or sale.currency_id = b.currency_id
                    THEN 
                        IF (TSPL.id IS NULL AND P.type='combo', ( 
            ...
            DB::raw("SUM(
                CASE
                    WHEN sale.currency_id > 0 and sale.currency_id != b.currency_id
                    THEN 
                        IF (TSPL.id IS NULL AND P.type='combo', ( 
                            SELECT Sum((tspl2.quantity - tspl2.qty_returned) * (tsl.unit_price_inc_tax - pl2.purchase_price_inc_tax)) AS total
                                FROM transaction_sell_lines AS tsl
        ...
        $gross_profit_currency2 = ! empty($gross_profit_obj->gross_profit_currency2) ? $gross_profit_obj->gross_profit_currency2 : 0;

        //KNOWS ISSUE: If products are returned then also the discount gets applied for it.

        return [
            'gross_profit' => $gross_profit,
            'gross_profit_currency2' => $gross_profit_currency2
        ];
    }
    ...
    public function deleteRemito($business_id, $transaction_id)
    {
        $transaction = Transaction::where('id', $transaction_id)
                    ->where('business_id', $business_id)
                    ->where('type', 'remito')
        ...
        if (! empty($transaction)) {
            ...
            $log_type = 'remito_deleted';
            $this->activityLog($transaction, $log_type, null, $log_properities);

            // To adjust product stock
            foreach ($transaction->remito_lines as $line) {
                $this->adjustQuantity($transaction->location_id, $line->product_id, $line->variation_id, $line->quantity);
            }

            // To delete remito_lines
            RemitoLine::where('transaction_id', $transaction_id)->delete();
    ...
    public function getRemitos($business_id)
    {
        $remitos = Transaction::leftJoin('contacts', 'transactions.contact_id', '=', 'contacts.id')
                    ->join(
                    ...
                    ->where('transactions.business_id', $business_id)
                    ->where('transactions.type', 'remito')
                    ->select(
                        'transactions.id',
    ...
    public function getListPurchases($business_id)
    {
        $purchases = Transaction::leftJoin('contacts', 'transactions.contact_id', '=', 'contacts.id')
                    ...
                    ->select(
                        ...
                        'transactions.nation_exchg_rate',
                        'transactions.document',
                        'transactions.document2',
                        'transactions.transaction_date',
                        'transactions.ref_no',
                        'contacts.name',
                        'contacts.supplier_business_name',
    ...
    public function getListByPymoInvoice($business_id, $pymo_invoices)
    {
        $transactions = Transaction::leftJoin('contacts', 'transactions.contact_id', '=', 'contacts.id')
                ...
                ->leftJoin('transactions AS SR','transactions.return_parent_id','=','SR.id')
                ->leftJoin('types_of_services AS tos','transactions.types_of_service_id','=','tos.id')
                ->where('transactions.business_id', $business_id)
                ->whereIn('transactions.pymo_invoice', $pymo_invoices)
                ->select(
                    'transactions.id',
                    'transactions.business_id',
                    'transactions.transaction_date',
                    ...
                    'transactions.document_type',
                    'SR.pymo_serie as sell_return_pymo_serie',
                    DB::raw("CONCAT(COALESCE(u.surname, ''),' ',COALESCE(u.first_name, ''),' ',COALESCE(u.last_name,'')) as added_by"),
    ...
    public function getLedgerDetails($contact_id, $start, $end, $format = 'format_1', $location_id = null, $line_details = false)
    {
        ...
        $previous_transaction_sums = $this->__transactionQuery($contact_id, $start, null, $location_id)
                ->select(
                    DB::raw("SUM(
                        CASE 
                            WHEN t.currency_id is NULL or t.currency_id = b.currency_id
                            THEN IF(type = 'purchase', final_total, 0)
                        END
                    ) as total_purchase"),
                    DB::raw("SUM(
                        CASE 
                            ...
                            THEN IF(type = 'purchase', final_total, 0)
                        END
                    ) as total_purchase_currency2"),
                    DB::raw("SUM(
                        CASE
                            WHEN t.currency_id is NULL or t.currency_id = b.currency_id
                            THEN IF(type = 'sell' AND status = 'final', final_total, 0)
                        END
                    ) as total_invoice"),
                    DB::raw("SUM(
                        CASE
                            ...
                            THEN IF(type = 'sell' AND status = 'final', final_total, 0)
                        END
                    ) as total_invoice_currency2"),
                    DB::raw("SUM(
                        CASE 
                            WHEN t.currency_id is NULL or t.currency_id = b.currency_id
                            THEN IF(type = 'sell_return', final_total, 0)
                        END
                    ) as total_sell_return"),
                    DB::raw("SUM(
                        CASE 
                            WHEN t.currency_id > 0 and t.currency_id != b.currency_id
                            THEN IF(type = 'sell_return', final_total, 0)
                        END
                    ) as total_sell_return_currency2"),
                    DB::raw("SUM(
                        CASE 
                            WHEN t.currency_id is NULL or t.currency_id = b.currency_id
                            THEN IF(type = 'purchase_return', final_total, 0)
                        END
                    ...
                    DB::raw("SUM(
                        CASE 
                            ...
                            THEN IF(type = 'purchase_return', final_total, 0)
                        END
                    ) as total_purchase_return_currency2"),
                    DB::raw("SUM(
                        CASE 
                            WHEN t.currency_id is NULL or t.currency_id = b.currency_id
                            THEN IF(type = 'opening_balance', final_total, 0)
                        END
                    ...
                    DB::raw("SUM(
                        CASE 
                            ...
                            THEN IF(type = 'opening_balance', final_total, 0)
                        END
                    ) as total_opening_balance_currency2"),
                    DB::raw("SUM(
                        CASE
                            WHEN t.currency_id is NULL or t.currency_id = b.currency_id
                            THEN IF(type = 'ledger_discount', final_total, 0)
                        END
                    ) as total_ledger_discount"),
                    DB::raw("SUM(
                        CASE
                            WHEN t.currency_id > 0 and t.currency_id != b.currency_id
                            THEN IF(type = 'ledger_discount', final_total, 0)
                        END
                    ) as total_ledger_discount_currency2")
                )->first();
        ...
        // prev_total_invoice_paid
        $prev_total_invoice_paid = $prev_payments->where('transaction_type', 'sell')
                ->where('is_return', 0)
                ->filter(function($item) use($business) {return $business->isMainCurrency($item->currency_id);})
                ->sum('amount');
        $prev_total_invoice_paid_currency2 = $prev_payments->where('transaction_type', 'sell')
                ->where('is_return', 0)
                ->filter(function($item) use($business){return !$business->isMainCurrency($item->currency_id);})
                ->sum('amount');

        // prev_total_ob_paid
        $prev_total_ob_paid = $prev_payments->where('transaction_type', 'opening_balance')
                ->where('is_return', 0)
                ->filter(function($item) use($business) {return $business->isMainCurrency($item->currency_id);})
                ->sum('amount');
        $prev_total_ob_paid_currency2 = $prev_payments->where('transaction_type', 'opening_balance')
                ->where('is_return', 0)
                ->filter(function($item) use($business){return !$business->isMainCurrency($item->currency_id);})
                ->sum('amount');

        // prev_total_sell_change_return
        $prev_total_sell_change_return = $prev_payments->where('transaction_type', 'sell')
                ->where('is_return', 1)
                ->filter(function($item) use($business) {return $business->isMainCurrency($item->currency_id);})
                ->sum('amount');
        $prev_total_sell_change_return_currency2 = $prev_payments->where('transaction_type', 'sell')
                ->where('is_return', 1)
                ->filter(function($item) use($business){return !$business->isMainCurrency($item->currency_id);})
                ->sum('amount');                        
        
        $prev_total_invoice_paid -= $prev_total_sell_change_return;
        $prev_total_invoice_paid_currency2 -= $prev_total_sell_change_return_currency2;

        // prev_total_purchase_paid
        $prev_total_purchase_paid = $prev_payments->where('transaction_type', 'purchase')
                ->where('is_return', 0)
                ->filter(function($item) use($business){return $business->isMainCurrency($item->currency_id);})
                ->sum('amount');
        $prev_total_purchase_paid_currency2 = $prev_payments->where('transaction_type', 'purchase')
                ->where('is_return', 0)
                ->filter(function($item) use($business){return !$business->isMainCurrency($item->currency_id);})
                ->sum('amount');

        // prev_total_sell_return_paid
        $prev_total_sell_return_paid = $prev_payments->where('transaction_type', 'sell_return')
            ->filter(function($item) use($business){return $business->isMainCurrency($item->currency_id);})
            ->sum('amount');
        $prev_total_sell_return_paid_currency2 = $prev_payments->where('transaction_type', 'sell_return')
            ->filter(function($item) use($business){return !$business->isMainCurrency($item->currency_id);})
            ->sum('amount');

        // prev_total_purchase_return_paid 
        $prev_total_purchase_return_paid = $prev_payments->where('transaction_type', 'purchase_return')
            ->filter(function($item) use($business){return $business->isMainCurrency($item->currency_id);})
            ->sum('amount');
        $prev_total_purchase_return_paid_currency2 = $prev_payments->where('transaction_type', 'purchase_return')
            ->filter(function($item) use($business){return !$business->isMainCurrency($item->currency_id);})
            ->sum('amount');

        // prev_total_advance_payment
        $rows = $this->__paymentQuery($contact_id, $start, null, $location_id)
            ...
            ->where('is_advance', 1)
            ->get();
        $prev_total_advance_payment = $rows->filter(function($item) use($business){
            return $business->isMainCurrency($item->currency_id);
        })->sum('amount');
        $prev_total_advance_payment_currency2 = $rows->filter(function($item) use($business){
            return !$business->isMainCurrency($item->currency_id);
        })->sum('amount');
        ...
        $total_prev_paid =  $prev_total_invoice_paid 
                            ...
                            + $prev_total_ob_paid 
                            + $prev_total_advance_payment;
        $total_prev_paid_currency2 = $prev_total_invoice_paid_currency2 
                            + $prev_total_purchase_paid_currency2 
                            - $prev_total_sell_return_paid_currency2 
                            - $prev_total_purchase_return_paid_currency2 
                            + $prev_total_ob_paid_currency2 
                            + $prev_total_advance_payment_currency2;
        ...
        $total_prev_invoice = $previous_transaction_sums->total_purchase 
                            ...
                            - $previous_transaction_sums->total_ledger_discount;
        $total_prev_invoice_currency2 = $previous_transaction_sums->total_purchase_currency2 
                            + $previous_transaction_sums->total_invoice_currency2 
                            - $previous_transaction_sums->total_sell_return_currency2 
                            - $previous_transaction_sums->total_purchase_return_currency2 
                            + $previous_transaction_sums->total_opening_balance_currency2 
                            - $previous_transaction_sums->total_ledger_discount_currency2;
        //$total_prev_paid = $prev_payments_sum->total_paid;
        $beginning_balance = $total_prev_invoice - $total_prev_paid;
        $beginning_balance_currency2 = $total_prev_invoice_currency2 - $total_prev_paid_currency2;
        ...
        if ($line_details) {
            $with = ['location', 'sell_lines', 'sell_lines.sub_unit', 'sell_lines.product',
                ...
                'purchase_lines.product.unit', 'purchase_lines.product.unit.sub_units', 'currency'];
        }
        ...
        if ($format == 'format_1' || $format == 'format_4') {
            $transaction_query->leftjoin('transaction_payments as tp', 'tp.transaction_id', '=', 't.id')
                            ->addSelect(DB::raw("COALESCE(SUM(if(tp.paid_on>='".$start."' and tp.paid_on<='".$end."', tp.amount,0)), 0) as total_paid"))
                            ->groupBy('t.id');
        }
        if ($format == 'format_2') {
            $transaction_query->leftjoin('transaction_payments as tp', 'tp.transaction_id', '=', 't.id')
        ...
        $opening_balance = 0;
        $opening_balance_currency2 = 0;
        $ledger_discount = 0;
        $ledger_discount_currency2 = 0;
        $opening_balance_paid = 0;
        $opening_balance_paid_currency2 = 0;

        foreach ($transactions as $transaction) {
            $is_maincurrency = $business->isMainCurrency($transaction->currency_id); 
            ...
            if ($transaction->type == 'ledger_discount') {
                if($is_maincurrency) $ledger_discount += $transaction->final_total;
                else $ledger_discount_currency2 += $transaction->final_total;
            }

            // if(in_array($transaction->type, ['sell', 'purchase_return']) || ($transaction->sub_type == 'purchase_discount')) {
            //     $debit = $transaction->final_total;
            //     $credit = $transaction->total_paid;
            // }
            // if(in_array($transaction->type, ['purchase', 'sell_return']) || ($transaction->sub_type == 'sell_discount')) {
            //     $debit = $transaction->total_paid;
            ...
            $temp_array = [
                ...
                'pymo_serie' => $transaction->pymo_serie,
                ...
                'payment_status' => ($transaction->type == 'sell') ? __('lang_v1.' . $transaction->payment_status) : '',
                'total' => '',
                'payment_method' => '',
                ...
                'currency' => $is_maincurrency ? $business->currency : $business->secondCurrency, 
                'is_maincurrency' => $is_maincurrency,
        ...
        $invoice_sum = $transactions->where('type', 'sell')
            ->filter(function($item) use($business) { return $business->isMainCurrency($item->currency_id);})
            ->sum('final_total');
        $invoice_sum_currency2 = $transactions->where('type', 'sell')
            ->filter(function($item) use($business) {return !$business->isMainCurrency($item->currency_id);})
            ->sum('final_total');

        $purchase_sum = $transactions->where('type', 'purchase')
            ->filter(function($item) use($business) {return $business->isMainCurrency($item->currency_id);})
            ->sum('final_total');
        $purchase_sum_currency2 = $transactions->where('type', 'purchase')
            ->filter(function($item) use($business) {return !$business->isMainCurrency($item->currency_id);})
            ->sum('final_total');

        $sell_return_sum = $transactions->where('type', 'sell_return')
            ->filter(function($item) use($business) {return $business->isMainCurrency($item->currency_id);})
            ->sum('final_total');
        $sell_return_sum_currency2 = $transactions->where('type', 'sell_return')
            ->filter(function($item) use($business) {return !$business->isMainCurrency($item->currency_id);})
            ->sum('final_total');

        $purchase_return_sum = $transactions->where('type', 'purchase_return')
            ->filter(function($item) use($business) {return $business->isMainCurrency($item->currency_id);})
            ->sum('final_total');
        $purchase_return_sum_currency2 = $transactions->where('type', 'purchase_return')
            ->filter(function($item) use($business) {return !$business->isMainCurrency($item->currency_id);})
            ->sum('final_total');
        ...
        $total_reverse_payment_currency2 = 0;

        foreach ($payments as $payment) {
            $is_maincurrency = $business->isMainCurrency($payment->currency_id);

            if ($payment->transaction_type == 'opening_balance') {
                if( $is_maincurrency )
                    $opening_balance_paid += $payment->amount;
                else 
                    $opening_balance_paid_currency2 += $payment->amount;
            }
            ...
            if ($contact->type == 'customer' && $payment->is_advance == 0 && empty($payment->transaction_id) && $payment->payment_type == 'debit') {
                if( $is_maincurrency )
                    ...
                    $total_reverse_payment_currency2 += $payment->amount;
            }
            ...
            } else if(in_array($payment->transaction_type, ['purchase', 'sell_return']) || ($payment->is_advance == 1 && $contact->type == 'supplier') || (in_array($payment->transaction_type, ['sell', 'purchase_return', 'opening_balance']) && $payment->is_return == 1)) {
                $debit = $payment->amount;
            } else if((in_array($payment->transaction_type, ['sell', 'purchase_return', 'opening_balance']) || ($payment->is_advance == 1 && in_array($contact->type, ['customer', 'both']))) && $payment->is_return == 0) {
                $credit = $payment->amount;
            }
        ...
        // total_excess_advance_payment
        $rows = $this->__paymentQuery($contact_id, $start, $end, $location_id)
            ->select(
                't.currency_id',
                DB::raw('(transaction_payments.amount - COALESCE((SELECT SUM(amount) from transaction_payments as tp where tp.parent_id = transaction_payments.id), 0)) as amount')
            )
            ->where('is_advance', 1)
            ->get();
        $total_excess_advance_payment = $rows->filter(function($item) use($business) {
            return $business->isMainCurrency($item->currency_id);
        })->sum('amount');
        $total_excess_advance_payment_currency2 = $rows->filter(function($item) use($business) {
            return !$business->isMainCurrency($item->currency_id);
        })->sum('amount');
        ...
        $rows = $this->__paymentQuery($contact_id, $start, $end, $location_id)
            ...
            ->get();
        $total_advance_payment = $rows->filter(function($item) use($business) {
            return $business->isMainCurrency($item->currency_id);
        })->sum('amount');
        $total_advance_payment_currency2 = $rows->filter(function($item) use($business) {
            return !$business->isMainCurrency($item->currency_id);
        })->sum('amount');

        // total_invoice_paid
        $total_invoice_paid = 0;
        $total_invoice_paid_currency2 = 0;
        if(!empty($payments)) {
            $total_invoice_paid = $payments->where('transaction_type', 'sell')
                ->where('is_return', 0)
                ->filter(function($item) use($business) {
                    return $business->isMainCurrency($item->currency_id);
                })
                ->sum('amount');
            $total_invoice_paid_currency2 = $payments->where('transaction_type', 'sell')
                ->where('is_return', 0)
                ->filter(function($item) use($business) {
                    return !$business->isMainCurrency($item->currency_id);
                })
        ...
        // total_sell_change_return
        $total_sell_change_return = 0;
        $total_sell_change_return_currency2 = 0;
        if(!empty($payments)) {
            $total_sell_change_return = $payments->where('transaction_type', 'sell')
                ->where('is_return', 1)
                ->filter(function($item) use($business) {
                    return $business->isMainCurrency($item->currency_id);
                })
                ->sum('amount');
            $total_sell_change_return_currency2 = $payments->where('transaction_type', 'sell')
                ->where('is_return', 1)
                ->filter(function($item) use($business) {
                    return !$business->isMainCurrency($item->currency_id);
                })
        ...
        $total_invoice_paid_currency2 -= $total_sell_change_return_currency2;

        // total_purchase_paid
        $total_purchase_paid = 0;
        $total_purchase_paid_currency2 = 0;
        if(!empty($payments)) {
            $total_purchase_paid = $payments->where('transaction_type', 'purchase')
                ->where('is_return', 0)
                ->filter(function($item) use($business) {
                    return $business->isMainCurrency($item->currency_id);
                })
                ->sum('amount');
            $total_purchase_paid_currency2 = $payments->where('transaction_type', 'purchase')
                ->where('is_return', 0)
                ->filter(function($item) use($business) {
                    return !$business->isMainCurrency($item->currency_id);
                })
        ...
        $total_sell_return_paid = 0;
        $total_sell_return_paid_currency2 = 0;
        if(!empty($payments)) {
            $total_sell_return_paid = $payments->where('transaction_type', 'sell_return')
                ->filter(function($item) use($business){
                    return $business->isMainCurrency($item->currency_id);
                })
                ->sum('amount');
            $total_sell_return_paid_currency2 = $payments->where('transaction_type', 'sell_return')
                ->filter(function($item) use($business){
                    return !$business->isMainCurrency($item->currency_id);
                })
                ->sum('amount');
        ...
        $total_purchase_return_paid_currency2 = 0;
        if(! empty($payments) ) {
            $total_purchase_return_paid = $payments->where('transaction_type', 'purchase_return')
                ->filter(function($item) use($business) {
                    return $business->isMainCurrency($item->currency_id);
                })
                ->sum('amount');
            $total_purchase_return_paid_currency2 = $payments->where('transaction_type', 'purchase_return')
                ->filter(function($item) use($business){
                    return !$business->isMainCurrency($item->currency_id);
                })
                ->sum('amount');
        ...
        $total_invoice_paid_currency2 += $opening_balance_paid_currency2;

        $start_date = $this->format_date($start);
        $end_date = $this->format_date($end);

        $total_invoice = $invoice_sum - $sell_return_sum;
        $total_invoice_currency2 = $invoice_sum_currency2 - $sell_return_sum_currency2;

        $total_purchase = $purchase_sum - $purchase_return_sum;
        $total_purchase_currency2 = $purchase_sum_currency2 - $purchase_return_sum_currency2;

        $opening_balance_due = $opening_balance;
        $opening_balance_due_currency2 = $opening_balance_currency2;
        ...
        $total_paid = $total_invoice_paid + $total_purchase_paid 
                    ...
                    + $total_excess_advance_payment 
                    - $total_advance_payment;
        $total_paid_currency2 = $total_invoice_paid_currency2 
                    + $total_purchase_paid_currency2 
                    - $total_sell_return_paid_currency2 
                    - $total_purchase_return_paid_currency2 
                    + $total_excess_advance_payment_currency2 
                    - $total_advance_payment_currency2;

        $total_transactions_paid = $total_invoice_paid 
                    + $total_purchase_paid 
                    - $total_sell_return_paid 
                    - $total_purchase_return_paid;
        $total_transactions_paid_currency2 = $total_invoice_paid_currency2
                    + $total_purchase_paid_currency2
                    - $total_sell_return_paid_currency2 
                    - $total_purchase_return_paid_currency2;
        ...
        $curr_due = $total_invoice 
                    ...
                    + $beginning_balance 
                    + $opening_balance_due;
        $curr_due_currency2 = $total_invoice_currency2
                    + $total_purchase_currency2
                    - $total_paid_currency2
                    + $beginning_balance_currency2 
                    + $opening_balance_due_currency2;
        ...
        if($format != 'format_4') {
            ...
            $sorted_ledger = [];
            $items_exclude_sell_return = array_filter($ledger, function($item) {
                return $item['transaction_type'] != 'sell_return' && $item['transaction_type'] != 'payment';
            });
            $items_sell_return = array_filter($ledger, function($item) {
                return $item['transaction_type'] == 'sell_return';
            });
            $items_payments = array_filter($ledger, function($item) {
                return $item['transaction_type'] == 'payment';
            });

            foreach($items_exclude_sell_return as $tx) {
                $srs = array_filter($items_sell_return, function($item) use($tx) {
                    return $item['return_parent_id'] == $tx['transaction_id'];
                });
                $tps = array_filter($items_payments, function($item) use($tx) {
                    return $item['transaction_id'] == $tx['transaction_id'];
                });

                $items_sell_return = array_filter($items_sell_return, function($item) use($tx) {
                    return $item['return_parent_id'] != $tx['transaction_id'];
                });
                $items_payments = array_filter($items_payments, function($item) use($tx) {
                    return $item['transaction_id'] != $tx['transaction_id'];
                });

                $sorted_ledger[] = $tx;
                foreach($srs as $sr) $sorted_ledger[] = $sr;
                foreach($tps as $tp) $sorted_ledger[] = $tp;
            }

            foreach($items_sell_return as $sr) $sorted_ledger[] = $sr;
            foreach($items_payments as $tp) $sorted_ledger[] = $tp;

            $ledger = $sorted_ledger;
        }

        $total_opening_bal = $beginning_balance + $opening_balance_due;
        $total_opening_bal_currency2 = $beginning_balance_currency2 + $opening_balance_due_currency2;
        ...
        if ($format != 'format_2') {
            ...
            $ledger = array_merge([[
                ...
                'debit_currency2' => $contact->type == 'customer' ? abs($total_opening_bal_currency2) : 0,
                'credit' => $contact->type == 'supplier' ? abs($total_opening_bal) : 0,
                'credit_currency2' => $contact->type == 'supplier' ? abs($total_opening_bal_currency2) : 0,
                'others' => '',
                'final_total' => abs($total_opening_bal),
                'final_total_currency2' => abs($total_opening_bal_currency2),
                'total_due' => 0,
                'due_date' => null,
                'is_statistic' => true
            ]], $ledger);
        ...
        foreach ($ledger as $key => $val) {
            if($val['is_statistic']) {
                $bal = $val['credit'] - $val['debit'];
                $bal_currency2 = $val['credit_currency2'] - $val['debit_currency2'];
            } else {
                $credit = $val['credit'] ?? 0;
                $debit = $val['debit'] ?? 0;
                ...
                if($val['is_maincurrency']) $bal += ($credit - $debit);
                else $bal_currency2 += ($credit - $debit);
            }

            $ledger[$key]['balance'] = $bal;
            $ledger[$key]['balance_currency2'] = $bal_currency2;
        }
        ...
        $output = [
            ...
            'total_invoice_currency2' => $total_invoice_currency2,

            'total_purchase' => $total_purchase,
            'total_purchase_currency2' => $total_purchase_currency2,

            'beginning_balance' => $beginning_balance + $opening_balance_due,
            'beginning_balance_currency2' => $beginning_balance_currency2 + $opening_balance_due_currency2,

            'balance_due' => $curr_due,
            'balance_due_currency2' => $curr_due_currency2,

            'total_paid' => $total_paid,
            'total_paid_currency2' => $total_paid_currency2,

            'total_reverse_payment' => $total_reverse_payment,
            'total_reverse_payment_currency2' => $total_reverse_payment_currency2,

            'ledger_discount' => $ledger_discount,
            'ledger_discount_currency2' => $ledger_discount_currency2,
        ];
    ...
    private function __transactionQuery($contact_id, $start, $end = null, $location_id = null)
    {
        ...
        $query = Transaction::from("transactions as t")
                        ...
                        ->whereIn('t.type', $transaction_type_keys);
        ...
        if (! empty($start) && ! empty($end)) {
            ...
            ->whereDate('t.transaction_date', '<=', $end)->get();
        }
    ...
    public function getProfitLossDetails($business_id, $location_id, $start_date, $end_date, $user_id = null)
    {
        ...
        $data['total_purchase_shipping_charge'] = $purchase_details['total_shipping_charges'];
        $data['total_purchase_shipping_charge_currency2'] = $purchase_details['total_shipping_charges_currency2'];

        $data['total_sell_shipping_charge'] = $sell_details['total_shipping_charges'];
        $data['total_sell_shipping_charge_currency2'] = $sell_details['total_shipping_charges_currency2'];

        $data['total_purchase_additional_expense'] = $purchase_details['total_additional_expense'];
        $data['total_purchase_additional_expense_currency2'] = $purchase_details['total_additional_expense_currency2'];

        $data['total_sell_additional_expense'] = $sell_details['total_additional_expense'];
        $data['total_sell_additional_expense_currency2'] = $sell_details['total_additional_expense_currency2'];
        
        //Shipping
        $data['total_transfer_shipping_charges'] = $transaction_totals['total_transfer_shipping_charges'];
        $data['total_transfer_shipping_charges_currency2'] = $transaction_totals['total_transfer_shipping_charges_currency2'];

        //Discounts
        $total_purchase_discount = $transaction_totals['total_purchase_discount'];
        $total_purchase_discount_currency2 = $transaction_totals['total_purchase_discount_currency2'];
        
        $total_sell_discount = $transaction_totals['total_sell_discount'];
        $total_sell_discount_currency2 = $transaction_totals['total_sell_discount_currency2'];

        $total_reward_amount = $transaction_totals['total_reward_amount'];
        $total_reward_amount_currency2 = $transaction_totals['total_reward_amount_currency2'];

        $total_sell_round_off = $transaction_totals['total_sell_round_off'];
        $total_sell_round_off_currency2 = $transaction_totals['total_sell_round_off_currency2'];
        
        $total_sell_return_discount = $transaction_totals['total_sell_return_discount'];
        $total_sell_return_discount_currency2 = $transaction_totals['total_sell_return_discount_currency2'];

        //Stocks
        $data['opening_stock'] = $opening_stock['stock'];
        $data['opening_stock_currency2'] = $opening_stock['stock_currency2'];

        $data['closing_stock'] = $closing_stock['stock'];
        $data['closing_stock_currency2'] = $closing_stock['stock_currency2'];

        //Purchase
        $data['total_purchase'] = $purchase_details['total_purchase_exc_tax'];
        $data['total_purchase_currency2'] = $purchase_details['total_purchase_exc_tax_currency2'];
        
        $data['total_purchase_discount'] = $total_purchase_discount;
        $data['total_purchase_discount_currency2'] = $total_purchase_discount_currency2;

        $data['total_purchase_return'] = $transaction_totals['total_purchase_return_exc_tax'];
        $data['total_purchase_return_currency2'] = $transaction_totals['total_purchase_return_exc_tax_currency2'];

        //Sales
        $data['total_sell'] = $sell_details['total_sell_exc_tax'];
        $data['total_sell_currency2'] = $sell_details['total_sell_exc_tax_currency2'];

        $data['total_sell_discount'] = $total_sell_discount;
        $data['total_sell_discount_currency2'] = $total_sell_discount_currency2;

        $data['total_sell_return_discount'] = $total_sell_return_discount;
        $data['total_sell_return_discount_currency2'] = $total_sell_return_discount_currency2;

        $data['total_sell_return'] = $transaction_totals['total_sell_return_exc_tax'];
        $data['total_sell_return_currency2'] = $transaction_totals['total_sell_return_exc_tax_currency2'];

        $data['total_sell_round_off'] = $total_sell_round_off;
        $data['total_sell_round_off_currency2'] = $total_sell_round_off_currency2;

        //Expense
        $data['total_expense'] = $transaction_totals['total_expense'];
        $data['total_expense_currency2'] = $transaction_totals['total_expense_currency2'];

        //Stock adjustments
        $data['total_adjustment'] = $transaction_totals['total_adjustment'];
        $data['total_adjustment_currency2'] = $transaction_totals['total_adjustment_currency2'];

        $data['total_recovered'] = $transaction_totals['total_recovered'];
        $data['total_recovered_currency2'] = $transaction_totals['total_recovered_currency2'];

        // $data['closing_stock'] = $data['closing_stock'] - $data['total_adjustment'];

        $data['total_reward_amount'] = $total_reward_amount;
        $data['total_reward_amount_currency2'] = $total_reward_amount_currency2;
        ...
        $data['net_profit'] = $module_total + $gross_profit['gross_profit']
                                + ( $data['total_sell_round_off'] + $data['total_recovered'] 
                                    + $data['total_sell_shipping_charge'] + $data['total_purchase_discount'] 
                                    + $data['total_sell_additional_expense'] + $data['total_sell_return_discount']) 
                                - ( $data['total_reward_amount'] + $data['total_expense'] 
                                    + $data['total_adjustment'] + $data['total_transfer_shipping_charges'] 
                                    + $data['total_purchase_shipping_charge'] + $data['total_purchase_additional_expense'] 
                                    + $data['total_sell_discount']
                                );
        $data['net_profit_currency2'] = $gross_profit['gross_profit_currency2']
                                + ( $data['total_sell_round_off_currency2'] + $data['total_recovered_currency2'] 
                                    + $data['total_sell_shipping_charge_currency2'] + $data['total_purchase_discount_currency2'] 
                                    + $data['total_sell_additional_expense_currency2'] + $data['total_sell_return_discount_currency2']) 
                                - ( $data['total_reward_amount_currency2'] + $data['total_expense_currency2'] 
                                    + $data['total_adjustment_currency2'] + $data['total_transfer_shipping_charges_currency2'] 
                                    + $data['total_purchase_shipping_charge_currency2'] + $data['total_purchase_additional_expense_currency2'] 
                                    + $data['total_sell_discount_currency2']
                                );
        ...
        if (! empty($project_module_data['Project']['gross_profit'])) {
            $gross_profit['gross_profit'] = $gross_profit + $project_module_data['Project']['gross_profit'];
            $data['gross_profit_label'] = __('project::lang.project_invoice');
        }

        $data['gross_profit'] = $gross_profit['gross_profit'];
        $data['gross_profit_currency2'] = $gross_profit['gross_profit_currency2'];

        //get sub type for total sales
        $sales_by_subtype = Transaction::from("transactions as t")
            ->join("business as b", "b.id", "=", "t.business_id")
            ->where('t.business_id', $business_id)
            ->where('t.type', 'sell')
            ->where('t.status', 'final');
        if (! empty($start_date) && ! empty($end_date)) {
            if ($start_date == $end_date) {
                $sales_by_subtype->whereDate('t.transaction_date', $end_date);
            } else {
                $sales_by_subtype->whereBetween(DB::raw('t.transaction_date'), [$start_date, $end_date]);
            }
        }
        $sales_by_subtype = $sales_by_subtype->select(
            DB::raw('SUM( 
                CASE 
                    WHEN t.currency_id is NULL or t.currency_id = b.currency_id
                    THEN t.total_before_tax
        ...
        $sales_by_subtype = $sales_by_subtype->whereNotNull('sub_type')
            ->groupBy('t.sub_type')
            ->get();
    ...
    public function createExpense($request, $business_id, $user_id, $format_data = true)
    {
        ...
        $transaction_data = $request->only(['ref_no', 'transaction_date',
            ...
            'expense_category_id', 'tax_id', 'contact_id', 'currency_id', 'nation_exchg_rate']);
    ...
    public function updateExpense($request, $id, $business_id, $format_data = true)
    {
        ...
        $transaction_data['currency_id'] = $request->input('currency_id');
        $transaction_data['nation_exchg_rate'] = $request->input('nation_exchg_rate');
        if($transaction_data['currency_id'] == $business->currency_id) {
            $transaction_data['exchange_rate'] = 1;
        } else {
            $transaction_data['exchange_rate'] = ($transaction_data['currency_id'] == $business->nation_currency_id) ? 
                                                1 / $request->input('nation_exchg_rate') : 
                                                $request->input('nation_exchg_rate');
        }
    ...
    public function payContact($request, $format_data = true)
    {
        ...
        $inputs = $request->only(['amount', 'method', 'note', 'card_number', 'card_holder_name',
            ...
            'cheque_number', 'bank_account_number', 'currency_id', 'nation_exchg_rate']);
    ...
    public function addSellReturn($input, $business_id, $user_id, $uf_number = true)
    {
        ...
        $sell_return_data = [
            ...
            'nation_exchg_rate' => $sell->nation_exchg_rate,
            'exchange_rate' => $sell->exchange_rate,
        ...
        $this->updatePaymentStatus($sell->id, $sell->final_total);

        // update cash_register_transaction if POS cash transaction
        $cash_register_tx = CashRegisterTransaction::where("transaction_type", "sell")
            ->where("transaction_id", $input['transaction_id'])
            ->first();

        if(!empty($cash_register_tx)) {
            $cash_register_return_tx = CashRegisterTransaction::where("transaction_type", "refund")
                ->where("transaction_id", $sell_return->id)
                ->first();

            if(!empty($cash_register_return_tx)) {
                $cash_register_return_tx->amount = $sell_return->final_total;
                $cash_register_return_tx->save();
            } else {
                $cash_register_return_tx = CashRegisterTransaction::create([
                    'cash_register_id' => $cash_register_tx->cash_register_id,
                    'amount' => $sell_return->final_total,
                    'pay_method' => $cash_register_tx->pay_method,
                    'type' => 'credit',
