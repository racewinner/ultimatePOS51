...
class NotificationUtil extends Util
{
    ...
    public function __construct() {
        $this->transactionUtil = new \App\Utils\TransactionUtil();
    }
                ...
                if (! empty($notification_template->auto_send) && ! empty($contact->email)) {
                    ...
                    try {
                        if (array_key_exists($notification_type, $customer_notifications)) {
                            ...
                            if ($notification_type == 'purchase_order') {
                                $data['pdf_name'] = 'PO-'.$transaction->ref_no.'.pdf';
                                $data['pdf'] = $this->transactionUtil->getPurchaseOrderPdf($business_id, $transaction->id, true);
                            }
                            Notification::route('mail', $data['to_email'])
    ...
    public function getNotificationType($controltype) {
        $type = "";
        $message = "";
        switch ($controltype) {
