...
class ProductUtil extends Util
{
        ...
        )
    {
        ...
        if($nation_exchg_rate == null) {
            $nation_exchg_rate = $business->nation_exchg_rate;
        }
        ...
        $variation_data = [
            ...
            'nation_exchg_rate' => $nation_exchg_rate,
    ...
    public function getDetailsFromVariation($variation_id, $business_id, $location_id = null, $check_qty = true)
    {
        ...
        $product = $query->select(
            ...
            'variations.currency_id',
            'variations.nation_exchg_rate',
    ...
    public function updateProductFromPurchase($variation_data)
    {
        ...
        if (($variation_details->default_purchase_price != $variation_data['pp_without_discount']) ||
            ...
            $variation_details->currency_id = $variation_data['currency_id'];
    ...
    public function createOrUpdateRemitoLines($transaction, $products) 
    {
        $updated_remito_lines = [];
        $updated_remito_line_ids = [0];
        $edit_ids = [0];
        ...
        foreach ($products as $data) {
            ...
            //update existing remito line
            if (isset($data['remito_line_id'])) {
                $remito_line = RemitoLine::findOrFail($data['remito_line_id']);
                $old_qty = $remito_line->quantity;

                $this->updateProductQuantity($transaction->location_id, $data['product_id'], $data['variation_id'], -$new_quantity_f, -$old_qty, null);
            } else {
                //create newly added remito lines
                $remito_line = new RemitoLine();
                $remito_line->product_id = $data['product_id'];
                $remito_line->variation_id = $data['variation_id'];

                $this->updateProductQuantity($transaction->location_id, $data['product_id'], $data['variation_id'], -$new_quantity_f, 0, null);
            }

            $remito_line->transaction_id = $transaction->id;
            $remito_line->quantity = $new_quantity;
            $remito_line->lot_number = ! empty($data['lot_number']) ? $data['lot_number'] : null;
            $remito_line->sub_unit_id = ! empty($data['sub_unit_id']) ? $data['sub_unit_id'] : null;

            $remito_line->save();

            $edit_ids[] = $remito_line->id;
        }

        // To delete removed remito_lines
        if(!empty($edit_ids)) {
            $deleted_lines = RemitoLine::where('transaction_id', $transaction->id)->whereNotIn('id', $edit_ids)->get();
            foreach($deleted_lines as $line) {
                $this->updateProductQuantity($transaction->location_id, $line->product_id, $line->variation_id, 0, -$line->quantity, null);
            }
            RemitoLine::where('transaction_id', $transaction->id)->whereNotIn('id', $edit_ids)->delete();
        }
    }
     ...
     * @param  array  $purchase_currency
     * @param  bool  $enable_product_editing
     * @param  string  $before_status = null
     * @return array
     */
    public function createOrUpdatePurchaseLines($transaction, $input_data, $purchase_currency, $enable_product_editing, $before_status = null)
    {
        ...
        foreach ($input_data as $data) {
            ...
            if (isset($data['purchase_line_id'])) {
                ...
                $this->updateProductStock($before_status, $transaction, $data['product_id'], $data['variation_id'], $new_quantity, $purchase_line->quantity, $purchase_currency);
            } else {
                ...
                if ($transaction->status == 'received') {
                    $this->updateProductQuantity($transaction->location_id, $data['product_id'], $data['variation_id'], $new_quantity_f, 0, $purchase_currency);
                }
            }

            $purchase_line->quantity = $new_quantity;
            $purchase_line->pp_without_discount = $this->num_uf($data['pp_without_discount'], $purchase_currency) / $multiplier;
            $purchase_line->discount_percent = $this->num_uf($data['discount_percent'], $purchase_currency);
            $purchase_line->purchase_price = $this->num_uf($data['purchase_price'], $purchase_currency) / $multiplier;
            $purchase_line->purchase_price_inc_tax = $this->num_uf($data['purchase_price_inc_tax'], $purchase_currency) / $multiplier;
            $purchase_line->item_tax = $this->num_uf($data['item_tax'], $purchase_currency) / $multiplier;
            $purchase_line->tax_id = $data['purchase_line_tax_id'] ? $data['purchase_line_tax_id'] : null;
            ...
            if ($enable_product_editing == 1 && $transaction->type == 'purchase') {
                if (isset($data['default_sell_price'])) {
                    $variation_data['sell_price_inc_tax'] = ($this->num_uf($data['default_sell_price'], $purchase_currency)) / $multiplier;
                }
                $variation_data['pp_without_discount'] = $this->num_uf($data['pp_without_discount'], $purchase_currency) / $multiplier;
                $variation_data['variation_id'] = $purchase_line->variation_id;
                $variation_data['purchase_price'] = $purchase_line->purchase_price;
                $variation_data['currency_id'] = $purchase_currency->id;
    ...
    public function changeRemitoLineUnit($business_id, $remito_line)
    {
        $unit_details = $this->getSubUnits($business_id, $remito_line->unit_id, false, $remito_line->product_id);

        $sub_unit = null;
        $sub_unit_id = $remito_line->sub_unit_id;
        foreach ($unit_details as $key => $value) {
            if ($key == $sub_unit_id) {
                $sub_unit = $value;
            }
        ...
        if (! empty($sub_unit)) {
            $multiplier = $sub_unit['multiplier'];
            $remito_line->quantity_ordered = $remito_line->quantity_ordered / $multiplier;
            $remito_line->sub_unit_multiplier = $multiplier;
            $remito_line->unit_details = $unit_details;
        }
        ...
        if (! empty($search_term)) {
            ...
            if ($search_type == 'like') {
                $query->where(function ($query) use ($search_term, $search_fields) {
                    ...
                    if (in_array('product_description', $search_fields)) {
                        $query->orWhere('product_description', 'like', '%'.$search_term.'%');
                    }
                });
            }
    ...
    public function getProductStockDetails($business_id, $filters, $for)
    {
        ...
        if(!empty($filters['to_date'])) {
            $to_date = $this->uf_date($filters['to_date']);
            $date_filter = " AND t.transaction_date <= '" . $to_date . "'";
        }
        ...
        $products = $query->select(
            ...
            DB::raw("(SELECT SUM(TSL.quantity - TSL.quantity_returned)
                        FROM transactions as t
                        JOIN business as b on b.id=t.business_id
                        JOIN transaction_sell_lines AS TSL ON t.id=TSL.transaction_id
                        WHERE t.status='final' AND t.type='sell' AND t.location_id=vld.location_id AND TSL.variation_id=variations.id "
                    . $date_filter
                    . " ) as total_sold"),
            DB::raw("(SELECT SUM(IF(t.type='sell_transfer', TSL.quantity, 0) ) FROM transactions as t
                        JOIN transaction_sell_lines AS TSL ON t.id=TSL.transaction_id
                        WHERE t.status='final' AND t.type='sell_transfer' AND t.location_id=vld.location_id AND (TSL.variation_id=variations.id) "
                    . $date_filter
                    . " ) as total_transfered"),
            DB::raw("(SELECT SUM(IF(t.type='stock_adjustment', SAL.quantity, 0) ) FROM transactions as t
                        JOIN stock_adjustment_lines AS SAL ON t.id=SAL.transaction_id
                        WHERE t.type='stock_adjustment' AND t.location_id=vld.location_id AND (SAL.variation_id=variations.id) "
                    . $date_filter
                    ." ) as total_adjusted"
            ),
            DB::raw("(SELECT SUM( COALESCE(pl.quantity - ($pl_query_string), 0) * purchase_price_inc_tax) 
                        FROM transactions as t
                        JOIN purchase_lines AS pl ON t.id=pl.transaction_id
                        WHERE (t.status='received' OR t.type='purchase_return')  AND t.location_id=vld.location_id AND (pl.variation_id=variations.id) "
                    . $date_filter
                    . " ) as stock_price"),
        ...
        if (isset($filters['show_manufacturing_data']) && $filters['show_manufacturing_data']) {
            $pl_query_string = $this->get_pl_quantity_sum_string('PL');
            $products->addSelect(
                DB::raw("(SELECT COALESCE(SUM(PL.quantity - ($pl_query_string)), 0) FROM transactions as t
                        JOIN purchase_lines AS PL ON t.id=PL.transaction_id
                        WHERE t.status='received' AND t.type='production_purchase' AND t.location_id=vld.location_id  
    ...
    public function getVariationStockDetails($business_id, $variation_id, $location_id)
    {
        ...
        $output = [
            ...
            'default_purchase_price' => $purchase_details->default_purchase_price,
            'dpp_inc_tax' => $purchase_details->dpp_inc_tax,
            'default_sell_price' => $purchase_details->default_sell_price,
            'sell_price_inc_tax' => $purchase_details->sell_price_inc_tax,
            'currency_id' => $purchase_details->currency_id
        ];
    ...
    public function getVariationStockHistory($business_id, $variation_id, $location_id)
    {
        ...
        foreach ($stock_history as $stock_line) {
            $temp_array = [
                ...
                'currency' => $business->isMainCurrency($stock_line->currency_id) ? $business->currency : $business->secondCurrency,
            ];
            if ($stock_line->transaction_type == 'sell') {
                ...
                $stock_history_array[] = array_merge($temp_array, [
                    ...
                    'stock_in_second_unit' => $this->roundQuantity($stock_in_second_unit),
                    'price' => $stock_line->purchase_price
                ]);
                ...
                $stock_history_array[] = array_merge($temp_array, [
                    ...
                    'ref_no' => $stock_line->ref_no,
                    'stock_in_second_unit' => $this->roundQuantity($stock_in_second_unit),
                    'price' => $stock_line->stock_adjusted_price,
                ]);
                ...
                $stock_history_array[] = array_merge($temp_array, [
                    ...
                    'ref_no' => $stock_line->ref_no,
                    'stock_in_second_unit' => $this->roundQuantity($stock_in_second_unit),
                    'price' => $stock_line->purchase_return_price,
                ]);
                ...
                $stock_history_array[] = array_merge($temp_array, [
                    ...
                    'ref_no' => $stock_line->invoice_no,
                    'stock_in_second_unit' => $this->roundQuantity($stock_in_second_unit),
                    'price' => $stock_line->sell_return_price,
                ]);
            }
        }
