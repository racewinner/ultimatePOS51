    ...
    public function getRegisterTransactionDetails($user_id, $open_time, $close_time, $is_types_of_service_enabled = false)
    {
        ...
        $product_details = Transaction::where('transactions.created_by', $user_id)
                ...
                ->select(
                    ...
                    DB::raw('SUM(TSL.quantity) as total_quantity'),
                    DB::raw('SUM(TSL.unit_price_inc_tax*TSL.quantity) as total_amount'),
                    DB::raw('SUM(TSL.quantity_returned) as return_quantity'),
                    DB::raw('SUM(TSL.unit_price_inc_tax*TSL.quantity_returned) as return_amount'),
                )
                ->get();
        ...
        $transaction_details = Transaction::where('transactions.created_by', $user_id)
                ...
                ->where('transactions.is_direct_sale', 0)
                ->where('transactions.status', 'final')
                ->leftjoin('transactions as sell_return', 'transactions.id', '=', 'sell_return.return_parent_id')
                ->select(
                    ...
                    DB::raw('SUM(sell_return.final_total) as total_refund'),
                    DB::raw('SUM(sell_return.tax_amount) as return_total_tax'),
                    DB::raw('SUM(IF(sell_return.discount_type = "percentage", sell_return.total_before_tax*sell_return.discount_amount/100, sell_return.discount_amount)) as return_total_discount'),
                )
                ->first();
    
        $transaction_payment_details = Transaction::where('transactions.created_by', $user_id)
                ->whereBetween('transactions.created_at', [$open_time, $close_time])
                ->where('transactions.type', 'sell')
                ->where('transactions.is_direct_sale', 0)
                ->where('transactions.status', 'final')
                ...
                ->select(
                    DB::raw('SUM(tp.amount) as total_sales'),
                    DB::raw('SUM(IF(tp.method = "cash", tp.amount, 0)) as total_cash'),
                    DB::raw('SUM(IF(tp.method = "card", tp.amount, 0)) as total_card'),
                    DB::raw('SUM(IF(tp.method = "card" and tp.card_type = "credit", tp.amount, 0)) as total_credit'),
                    DB::raw('SUM(IF(tp.method = "card" and tp.card_type != "credit", tp.amount, 0)) as total_debit'),
                )
                ->first();
        
        $credit_sale_details = [
            'total_sales' => round(($transaction_details->total_sales ?? 0) - ($transaction_payment_details->total_sales ?? 0), 2),
        ];
        ...
        return ['product_details_by_brand' => $product_details_by_brand,
            ...
            'product_return_details' => $product_return_details,
