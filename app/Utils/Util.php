...
class Util
{
    public static function format_currency($number, $currency=null, $show_symbol=true) {
        $formated_number = "";

        if($currency == null) {
            $currency_decimal_separator = session("currency")["decimal_separator"];
            $currency_thousand_separator = session("currency")["thousand_separator"];
        ...
        if ($show_symbol && session("business.currency_symbol_placement") == "before") {
            $formated_number .= $currency_symbol . " ";
        }

        $formated_number .= number_format($number, session("business.currency_precision", 2) , $currency_decimal_separator, $currency_thousand_separator);

        if ($show_symbol && session("business.currency_symbol_placement") == "after") {
            $formated_number .= " " . $currency_symbol;
        }
    ...
    public function deleteFile($file_name, $dir_name) {
        $filepath = public_path("uploads/" . $dir_name . "/" . $file_name);
        ...
        foreach ($notifications as $notification) {
            ...
            switch ( $notification->type ) {
                ...
                case \App\Notifications\RecurringExpenseNotification::class:
                    ...
                    array_push($notifications_data, [
                        'msg' => $msg,
                        'icon_class' => $icon_class,
                        'link' => $link,
                        'read_at' => $notification->read_at,
                        'updated_at' => $notification->created_at->diffForHumans(),
                        'name' => $cUser->username
                    ]);
                    break;
                case \App\Notifications\PurchaseNotification::class:
                    $msg = __('notifications.invoice_created');
                    $icon_class = 'fas fa-recycle bg-green';
                    if (isset($data['transaction_id'])) {
                        $link = action([\App\Http\Controllers\PurchaseController::class, 'edit'], ['purchase' => $data['transaction_id']]);
                    } else {
                ...
                case \App\Notifications\PurchaseOrderNotification::class:
                    $msg = __('notifications.purchase_order_created');
                    $icon_class = 'fas fa-recycle bg-green';
                    if (isset($data['transaction_id'])) {
                        $link = action([\App\Http\Controllers\PurchaseOrderController::class, 'edit'], ['purchase_order' => $data['transaction_id']]);
                    } else {
                        $link = action([\App\Http\Controllers\PurchseOrderController::class, 'index']);
                    }
                    $notifications_data[] = [
                        'msg' => $msg,
                        'icon_class' => $icon_class,
                ...
                case \App\Notifications\PaymentNotification::class:
                    $msg = __('notifications.payment_created');
                    $icon_class = 'fas fa-recycle bg-green';
                    if (isset($data['transaction_id'])) {
                        $msg .= $data['transaction_id'];
                        $link = action([\App\Http\Controllers\PurchaseController::class, 'index']);
                    } else {
                        $link = action([\App\Http\Controllers\PurchseController::class, 'index']);
                    }
                    $notifications_data[] = [
                        'msg' => $msg,
                        'icon_class' => $icon_class,
    ...
    public static function getMonthName($dateStr, $format) {
