...
class PurchaseReturnController extends Controller
{
    ...
    public function index()
    {
            ...
            return Datatables::of($purchases_returns)
                ->addColumn('action', function ($row) {
                    ...
                    if (! empty($row->return_parent_id)) {
                        $html .= '<li><a class="'.($row->canEdit() ? "":"disabled").'" href="'.action([\App\Http\Controllers\PurchaseReturnController::class, 'add'], $row->return_parent_id).'" ><i class="glyphicon glyphicon-edit"></i>'.
                                __('messages.edit').
                                '</a></li>';
                    } else {
                        $html .= '<li><a class="'.($row->canEdit() ? "":"disabled").'" href="'.action([\App\Http\Controllers\CombinedPurchaseReturnController::class, 'edit'], $row->id).'" ><i class="glyphicon glyphicon-edit"></i>'.
                                __('messages.edit').
                                '</a></li>';
                    }
                    ...
                    $html .= '<li><a class="delete_purchase_return '.($row->canEdit() ? "":"disabled").'" href="'.action([\App\Http\Controllers\PurchaseReturnController::class, 'destroy'], $row->id).'" ><i class="fa fa-trash"></i>'.
                                __('messages.delete').
                                '</a></li>';
                    $html .= '</ul></div>';
                ...
                ->editColumn(
                    'final_total', function($row) use ($business) {
                        ...
                        $data_orig = ($row->currency_id == NULL || $row->currency_id == $business->currency_id) ? "data-orig-value" : "data-orig-value-currency2";
                        return  '<span class="final_total" ' . $data_orig . '="' . $total . '"> ' 
                                . \App\Utils\Util::format_currency($total, $row->currency)
                                .'</span>';
                    }
                )
                ...
                ->addColumn('payment_due', function ($row) use($business) {
                    ...
                    return  '<span class="payment_due" ' . $data_orig . '="'.$due.'">'
                            . \App\Utils\Util::format_currency($due, $currency)
                            . '</sapn>';
                })
                ->setRowAttr([
        ...
        return view('purchase_return.index')->with(compact('business_locations', 'currencies'));
    }
    ...
    public function add($id)
    {
        ...
        $purchase = Transaction::where('business_id', $business_id)
                        ->where('type', 'purchase')
                        ->with(['purchase_lines','currency', 'contact', 'tax', 'return_parent', 'purchase_lines.sub_unit', 'purchase_lines.product', 'purchase_lines.product.unit'])
                        ->find($id);
    ...
    public function store(Request $request)
    {
        ...
        try {
            ...
            if (! empty($return_transaction)) {
                ...
                $return_transaction_data['currency_id'] = $purchase->currency_id;
                $return_transaction_data['nation_exchg_rate'] = $purchase->nation_exchg_rate;
                $return_transaction_data['exchange_rate'] = $purchase->nation_exchg_rate;
