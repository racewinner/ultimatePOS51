...
class ReportController extends Controller
{
    ...
    { 
        ...
        if ($request->ajax()) {
            ...
            return view('report.partials.profit_loss_details', compact('data', 'business', 'currencies'))->render();
        }

        $business_locations = BusinessLocation::forDropdown($business_id, true);

        return view('report.profit_loss', compact('business_locations', 'currencies'));
    }
    ...
    public function getPurchaseSell(Request $request)
    {
        ...
        if ($request->ajax()) {
            ...
            $difference = [
                'total' =>  $sell_details['total_sell_inc_tax'] 
                            - $transaction_totals['total_sell_return_inc_tax'] - 
                            ($purchase_details['total_purchase_inc_tax'] - $transaction_totals['total_purchase_return_inc_tax']),
                'total_currency2' => $sell_details['total_sell_inc_tax_currency2'] 
                            - $transaction_totals['total_sell_return_inc_tax_currency2'] 
                            - ($purchase_details['total_purchase_inc_tax_currency2'] - $transaction_totals['total_purchase_return_inc_tax_currency2']),

                'due' => $sell_details['invoice_due'] - $purchase_details['purchase_due'],
                'due_currency2' => $sell_details['invoice_due_currency2'] - $purchase_details['purchase_due_currency2'],
            ];
        ...
        return view('report.purchase_sell')->with(compact('business_locations', 'currencies'));
    }
    ...
    public function getCustomerSuppliers(Request $request)
    {
        ...
        if ($request->ajax()) {
                ...
                ->select(
                    DB::raw('SUM( 
                        CASE 
                            ...
                            THEN IF(t.type = "purchase", final_total, 0)
                    ...
                    DB::raw('SUM( 
                        CASE 
                            ...
                            THEN IF(t.type = "purchase", final_total, 0)
                        END ) as total_purchase_currency2'
                    ),
                    DB::raw('SUM( 
                        CASE 
                            WHEN t.currency_id is NULL or t.currency_id = b.currency_id
                            THEN IF(t.type = "purchase_return", final_total, 0)
                        END ) as total_purchase_return'
                    ),
                    DB::raw('SUM( 
                        CASE 
                            WHEN t.currency_id > 0 and t.currency_id != b.currency_id
                            THEN IF(t.type = "purchase_return", final_total, 0)
                        END ) as total_purchase_return_currency2'
                    ),
                    DB::raw('SUM( 
                        CASE 
                            WHEN t.currency_id is NULL or t.currency_id = b.currency_id
                            THEN IF(t.type = "sell" AND t.status = "final", final_total, 0)
                    ...
                    DB::raw('SUM( 
                        CASE 
                            WHEN t.currency_id > 0 and t.currency_id != b.currency_id
                            THEN IF(t.type = "sell" AND t.status = "final", final_total, 0)
                        END ) as total_invoice_currency2'
                    ),
                    DB::raw('SUM( 
                        CASE 
                            WHEN t.currency_id is NULL or t.currency_id = b.currency_id
                            THEN IF(t.type = "purchase", (SELECT SUM(amount) FROM transaction_payments WHERE transaction_payments.transaction_id=t.id), 0)
                        END ) as purchase_paid'
                    ),
                    DB::raw('SUM( 
                        CASE 
                            WHEN t.currency_id > 0 and t.currency_id != b.currency_id
                            THEN IF(t.type = "purchase", (SELECT SUM(amount) FROM transaction_payments WHERE transaction_payments.transaction_id=t.id), 0)
                        END ) as purchase_paid_currency2'
                    ),
                    DB::raw('SUM( 
                        CASE 
                            WHEN t.currency_id is NULL or t.currency_id = b.currency_id
                            THEN IF(t.type = "sell" AND t.status = "final", (SELECT SUM(IF(is_return = 1,-1*amount,amount)) FROM transaction_payments WHERE transaction_payments.transaction_id=t.id), 0)
                    ...
                    DB::raw('SUM( 
                        CASE 
                            WHEN t.currency_id > 0 and t.currency_id != b.currency_id
                            THEN IF(t.type = "sell" AND t.status = "final", (SELECT SUM(IF(is_return = 1,-1*amount,amount)) FROM transaction_payments WHERE transaction_payments.transaction_id=t.id), 0)
                        END ) as invoice_received_currency2'
                    ),
                    DB::raw('SUM( 
                        CASE 
                            WHEN t.currency_id is NULL or t.currency_id = b.currency_id
                            THEN IF(t.type = "sell_return", (SELECT SUM(amount) FROM transaction_payments WHERE transaction_payments.transaction_id=t.id), 0)
                        END ) as sell_return_paid'
                    ),
                    DB::raw('SUM( 
                        CASE 
                            WHEN t.currency_id > 0 and t.currency_id != b.currency_id
                            THEN IF(t.type = "sell_return", (SELECT SUM(amount) FROM transaction_payments WHERE transaction_payments.transaction_id=t.id), 0)
                        END ) as sell_return_paid_currency2'
                    ),
                    DB::raw('SUM( 
                        CASE 
                            WHEN t.currency_id is NULL or t.currency_id = b.currency_id
                            THEN IF(t.type = "purchase_return", (SELECT SUM(amount) FROM transaction_payments WHERE transaction_payments.transaction_id=t.id), 0)
                        END ) as purchase_return_received'
                    ),
                    DB::raw('SUM( 
                        CASE 
                            WHEN t.currency_id > 0 and t.currency_id != b.currency_id
                            THEN IF(t.type = "purchase_return", (SELECT SUM(amount) FROM transaction_payments WHERE transaction_payments.transaction_id=t.id), 0)
                        END ) as purchase_return_received_currency2'
                    ),
                    DB::raw('SUM( 
                        CASE 
                            WHEN t.currency_id is NULL or t.currency_id = b.currency_id
                            THEN IF(t.type = "sell_return", final_total, 0)
                        END ) as total_sell_return'
                    ),
                    DB::raw('SUM( 
                        CASE 
                            WHEN t.currency_id > 0 and t.currency_id != b.currency_id
                            THEN IF(t.type = "sell_return", final_total, 0)
                        END ) as total_sell_return_currency2'
                    ),
                    DB::raw('SUM( 
                        CASE 
                            WHEN t.currency_id is NULL or t.currency_id = b.currency_id
                            THEN IF(t.type = "opening_balance", final_total, 0)
                    ...
                    DB::raw('SUM( 
                        CASE 
                            WHEN t.currency_id > 0 and t.currency_id != b.currency_id
                            THEN IF(t.type = "opening_balance", final_total, 0)
                        END ) as opening_balance_currency2'
                    ),
                    DB::raw('SUM( 
                        CASE 
                            WHEN t.currency_id is NULL or t.currency_id = b.currency_id
                            THEN IF(t.type = "opening_balance", (SELECT SUM(IF(is_return = 1,-1*amount,amount)) FROM transaction_payments WHERE transaction_payments.transaction_id=t.id), 0)
                        END ) as opening_balance_paid'
                    ),
                    DB::raw('SUM( 
                        CASE 
                            WHEN t.currency_id > 0 and t.currency_id != b.currency_id
                            THEN IF(t.type = "opening_balance", (SELECT SUM(IF(is_return = 1,-1*amount,amount)) FROM transaction_payments WHERE transaction_payments.transaction_id=t.id), 0)
                        END ) as opening_balance_paid_currency2'
                    ),
                    DB::raw('SUM( 
                        CASE 
                            WHEN t.currency_id is NULL or t.currency_id = b.currency_id
                            THEN IF(t.type = "ledger_discount" AND sub_type="sell_discount", final_total, 0)
                        END ) as total_ledger_discount_sell'
                    ),
                    DB::raw('SUM( 
                        CASE 
                            WHEN t.currency_id > 0 and t.currency_id != b.currency_id
                            THEN IF(t.type = "ledger_discount" AND sub_type="sell_discount", final_total, 0)
                        END ) as total_ledger_discount_sell_currency2'
                    ),
                    DB::raw('SUM( 
                        CASE 
                            WHEN t.currency_id is NULL or t.currency_id = b.currency_id
                            THEN IF(t.type = "ledger_discount" AND sub_type="purchase_discount", final_total, 0)
                        END ) as total_ledger_discount_purchase'
                    ),
                    DB::raw('SUM( 
                        CASE 
                            WHEN t.currency_id > 0 and t.currency_id != b.currency_id
                            THEN IF(t.type = "ledger_discount" AND sub_type="purchase_discount", final_total, 0)
                        END ) as total_ledger_discount_purchase_currency2'
                    ),
            ...
            return Datatables::of($contacts)
                ...
                ->editColumn(
                    'total_purchase', function($row) use($business) {
                        if($business->second_currency_id > 0) {
                            return '<span class="total_purchase" data-orig-value="'. $row->total_purchase . '" data-orig-value-currency2="' . $row->total_purchase_currency2 . '">' .
                                    \App\Utils\Util::format_currency($row->total_purchase, $business->currency) . " / " .
                                    \App\Utils\Util::format_currency($row->total_purchase_currency2, $business->secondCurrency) . 
                                    '</span>';
                        } else {
                ...
                ->editColumn(
                    'total_purchase_return', function($row) use($business) {
                        if($business->second_currency_id > 0) {
                            return '<span class="total_purchase_return" data-orig-value="'.$row->total_purchase_return .
                                    '" data-orig-value-currency2="' . $row->total_purchase_return_currency2 . '">' .
                                    \App\Utils\Util::format_currency($row->total_purchase_return, $business->currency) . " / " .
                                    \App\Utils\Util::format_currency($row->total_purchase_return_currency2, $business->secondCurrency) .
                                    '</span>';
                        } else {
                            return '<span class="total_purchase_return" data-orig-value="'.$row->total_purchase_return . '">' .
                                    \App\Utils\Util::format_currency($row->total_purchase_return, $business->currency) .
                ...
                ->editColumn(
                    'total_sell_return', function($row) use ($business) {
                        if($business->second_currency_id > 0) {
                            return '<span class="total_sell_return" data-orig-value="' . $row->total_sell_return .
                                    '" data-orig-value-currency2="' . $row->total_sell_return_currency2 . '">' .
                                    \App\Utils\Util::format_currency($row->total_sell_return, $business->currency) . " / " .
                                    \App\Utils\Util::format_currency($row->total_sell_return_currency2, $business->secondCurrency) .
                                    '</span>';
                        } else {
                            return '<span class="total_sell_return" data-orig-value="' . $row->total_sell_return . '">' .
                                    \App\Utils\Util::format_currency($row->total_sell_return, $business->currency) . 
                ...
                ->editColumn(
                    'total_invoice', function($row) use ($business) {
                        if($business->second_currency_id > 0) {
                            return '<span class="total_invoice" data-orig-value="' . $row->total_invoice .
                                    '" data-orig-value-currency2="' . $row->total_invoice_currency2 . '">' .
                                    \App\Utils\Util::format_currency($row->total_invoice, $business->currency) . " / " .
                                    \App\Utils\Util::format_currency($row->total_invoice_currency2, $business->secondCurrency) . 
                                    '</span>';
                        } else {
                            return '<span class="total_invoice" data-orig-value="' . $row->total_invoice . '">' .
                                    \App\Utils\Util::format_currency($row->total_invoice, $business->currency) .
                ...
                ->addColumn('due', function ($row) use($business) {
                    ...
                    if($business->second_currency_id > 0) {
                        $total_ledger_discount_purchase_currency2 = $row->total_ledger_discount_purchase_currency2 ?? 0;
                        $total_ledger_discount_sell_currency2 = $row->total_ledger_discount_sell_currency2 ?? 0;
                        $due_currency2 = ($row->total_invoice_currency2 - $row->invoice_received_currency2 - $total_ledger_discount_sell_currency2)
                                - ($row->total_purchase_currency2 - $row->purchase_paid_currency2 - $total_ledger_discount_purchase_currency2) 
                                - ($row->total_sell_return_currency2 - $row->sell_return_paid_currency2) 
                                + ($row->total_purchase_return_currency2 - $row->purchase_return_received_currency2);

                        if ($row->contact_type == 'supplier') {
                            $due_currency2 -= $row->opening_balance_currency2 - $row->opening_balance_paid_currency2;
                        } else {
                            $due_currency2 += $row->opening_balance_currency2 - $row->opening_balance_paid_currency2;
                        }

                        return '<span class="total_due" data-orig-value="' . $due .
                                '" data-orig-value-currency2="' . $due_currency2 . '">' .
                                \App\Utils\Util::format_currency($due, $business->currency) . " / " .
                                \App\Utils\Util::format_currency($due_currency2, $business->secondCurrency) .
                                '</span>';
                ...
                ->addColumn(
                    'opening_balance_due', function($row) use($business) {
                        $opening_balance_due = $row->opening_balance - $row->opening_balance_paid;
                        $opening_balance_due_currency2 = $row->opening_balance_currency2 - $row->opening_balance_paid_currency2;

                        if($business->second_currency_id > 0) {
                            return '<span class="opening_balance_due" data-orig-value="' . $opening_balance_due .
                                    '" data-orig-value-currency2="' . $opening_balance_due_currency2 . '">' .
                                    \App\Utils\Util::format_currency($opening_balance_due, $business->currency) . " / " .
                                    \App\Utils\Util::format_currency($opening_balance_due_currency2, $business->secondCurrency) .
                                    '</span>';
                        } else {
                            return '<span class="opening_balance_due" data-orig-value="' . $opening_balance_due . '">' .
                                    \App\Utils\Util::format_currency($opening_balance_due, $business->currency) . 
        ...
        ->with(compact('customer_group', 'types', 'business_locations', 'contact_dropdown', 'currencies'));
    }
    ...
    public function getStockReport(Request $request)
    {
        ...
        if ($request->ajax()) {
            $filters = request()->only(['location_id', 'category_id', 'sub_category_id', 'brand_id', 'unit_id', 'tax_id', 'type',
                'only_mfg_products', 'active_state',  'not_for_selling', 'repair_model_id', 'product_id', 'active_state', 'to_date', ]);
            ...
            $datatable = Datatables::of($products)
                ...
                ->editColumn('unit_price', function ($row) use ($allowed_selling_price_group, $business) {
                    $html = '';
                    if (auth()->user()->can('access_default_selling_price')) {
                        $currency = $business->isMainCurrency($row->currency_id) ? $business->currency : $business->secondCurrency;
                        $html .= \App\Utils\Util::format_currency($row->unit_price, $currency);
                    }
                ...
                ->editColumn('stock_price', function ($row) use($business) {
                    if($business->isMainCurrency($row->currency_id)) {
                    ...
                    $html = '<span class="total_stock_price" ' . $data_orig . '="' . $row->stock_price . '">'.
                            \App\Utils\Util::format_currency($row->stock_price, $currency) .
                            '</span>';
                    return $html;
                })
                ->editColumn('stock_value_by_sale_price', function ($row) use($business) {
                    if($business->isMainCurrency($row->currency_id)) {
                        $currency = $business->currency;
                        $data_orig = "data-orig-value";
                    } else {
                    ...
                    return  '<span class="stock_value_by_sale_price" ' . $data_orig . '="' . (float) $stock_price.'" > ' 
                            . \App\Utils\Util::format_currency($stock_price, $currency) 
                            . '</span>';
                })
                ->editColumn('stock_value_by_sale_price_currency2', function ($row) use($business) {
                    if(empty($business->secondCurrency)) {
                        return '';
                    } else {
                        if($business->isMainCurrency($row->currency_id)) {
                            $unit_selling_price = (float) $row->group_price > 0 ? $row->group_price : $row->unit_price;
                            $unit_selling_price *= $business->nation_exchg_rate;
                        } else {
                            $unit_selling_price = (float) $row->group_price > 0 ? $row->group_price : $row->unit_price;
                        }
                        ...
                        return  '<span class="stock_value_by_sale_price_currency2" data-orig-value-currency2="' . (float) $stock_price.'" > ' 
                                . \App\Utils\Util::format_currency($stock_price, $business->secondCurrency) 
                                . '</span>';
                    }
                })
                ->addColumn('potential_profit', function ($row) use($business) {
                    if($business->isMainCurrency($row->currency_id)) {
                        $currency = $business->currency;
                        $data_orig = "data-orig-value";
                    } else {
                    ...
                    return  '<span class="potential_profit" ' . $data_orig . '="' . (float) $potential_profit.'" > '
                            . \App\Utils\Util::format_currency($potential_profit, $currency) 
                            . '</span>';
                })
            ...
            $raw_columns = ['unit_price', 'total_transfered', 'total_sold',
                'total_adjusted', 'stock', 'stock_price', 'stock_value_by_sale_price', 'stock_value_by_sale_price_currency2',
        ...
        return view('report.stock_report')
            ->with(compact('categories', 'brands', 'units', 'business', 'business_locations', 'show_manufacturing_data', 'currencies'));
    }
    ...
    public function getTaxDetails(Request $request)
    {
        ...
        if ($request->ajax()) {
            ...
            foreach ($taxes as $tax) {
                $col = 'tax_'.$tax['id'];
                $raw_cols[] = $col;
                $datatable->addColumn($col, function ($row) use ($tax, $type, $col, $group_taxes, $currencies, $business) {
                    $tax_amount = 0;
                    if ($type == 'sell') {
                    ...
                    if ($tax_amount > 0) {
                        ...
                        return '<span class="'.$col.'" ' . $data_orig . '="'.$tax_amount.'" >'
                                . \App\Utils\Util::format_currency($tax_amount, $row->currency)
                                . '</span>';
                    } else {
                        return '';
                    }
            ...
            $datatable->editColumn(
                    'total_before_tax',
                    function ($row) use($business) {
                        $total_before_tax = $row->total_before_tax;
                        $data_orig = $business->isMainCurrency($row->currency_id) ? "data-orig-value" : "data-orig-value-currency2";
                        return '<span class="total_before_tax" ' . $data_orig . '="' . $total_before_tax.'" >'
                                . \App\Utils\Util::format_currency($total_before_tax, $row->currency)
                                .'</span>';
                    }
                )
                ->editColumn('discount_amount', function ($row) use($currencies) {
                    if ($row->discount_amount !== 0) {
                        if( $row->discount_type == 'percentage') {
                            return $row->discount_amount . '%';
                        } else {
                            return  '<span>' 
                                    . \App\Utils\Util::format_currency($row->discount_amount, $row->currency)
                                    . '</span>';
                        }
                    }
                })
    ...
    public function getTaxReport(Request $request)
    {
        ...
        if ($request->ajax()) {
            ...
            $total_module_output_tax_currency2 = 0;
            foreach ($module_output_taxes as $key => $module_output_tax) {
                $total_module_output_tax += $module_output_tax['total_tax'];
                $total_module_output_tax_currency2 += $module_output_tax['$total_tax_currency2'];
            }

            $total_output_tax = $output_tax_details['total_tax'] + $total_module_output_tax;
            $total_output_tax_currency2 = $output_tax_details['total_tax_currency2'] + $total_module_output_tax_currency2;

            $tax_diff = $total_output_tax - $input_tax_details['total_tax'] - $expense_tax_details['total_tax'];
            $tax_diff_currency2 = $total_output_tax_currency2 - $input_tax_details['total_tax_currency2'] - $expense_tax_details['total_tax_currency2'];

            return [
                'tax_diff' => $tax_diff,
                'tax_diff_currency2' => $tax_diff_currency2,
            ];
        }
        ...
        return view('report.tax_report')
            ->with(compact('business_locations', 'taxes', 'tax_report_tabs', 'contact_dropdown', 'currencies'));
    }
    ...
    public function getExpenseReport(Request $request)
    {
        ...
        return view('report.expense_report')
                    ->with(compact('chart', 'business', 'categories', 'business_locations', 'expenses'));
    }
    ...
    public function getStockAdjustmentReport(Request $request)
    {
            ...
            $stock_adjustment_details = $query->select(
                ...
                DB::raw('SUM( 
                    CASE 
                        WHEN t.currency_id > 0 and t.currency_id != b.currency_id
                        THEN final_total
                    END) as total_amount_currency2'
                ),
                DB::raw('SUM(
                    CASE 
                        WHEN t.currency_id is NULL or t.currency_id = b.currency_id
                        THEN total_amount_recovered
                ...
                DB::raw("SUM(
                    CASE 
                        WHEN t.currency_id is NULL or t.currency_id = b.currency_id
                        THEN IF(adjustment_type = 'normal', final_total, 0)
                    END) as total_normal"
                ),
                DB::raw("SUM(
                    CASE 
                        WHEN t.currency_id > 0 and t.currency_id != b.currency_id
                        THEN IF(adjustment_type = 'normal', final_total, 0)
                    END) as total_normal_currency2"
                ),
                DB::raw("SUM(
                    CASE 
                        WHEN t.currency_id is NULL or t.currency_id = b.currency_id
                        THEN IF(adjustment_type = 'abnormal', final_total, 0)
                ...
                DB::raw("SUM(
                    CASE 
                        WHEN t.currency_id > 0 and t.currency_id != b.currency_id
                        THEN IF(adjustment_type = 'abnormal', final_total, 0)
                    END) as total_abnormal_currency2"
                )
            )->first();
        ...
        return view('report.stock_adjustment_report')
                    ->with(compact('business_locations', 'currencies'));
    }
    ...
    public function getRegisterReport(Request $request)
    {
        ...
        //Return the details in ajax call
        if ($request->ajax()) {
            $registers = CashRegister::leftjoin('cash_register_transactions as ct','ct.cash_register_id','=','cash_registers.id')
        ...
        return view('report.register_report')
                    ->with(compact('users', 'payment_types', 'currencies'));
    }
    ...
    public function getSalesRepresentativeReport(Request $request)
    {
        ...
        return view('report.sales_representative')
                ->with(compact('users', 'business_locations', 'pos_settings', 'currencies'));
    }
    ...
    public function getSalesRepresentativeTotalSell(Request $request)
    {
        ...
        if ($request->ajax()) {
            ...
            $total_sell_return_currency2 = ! empty($sell_return_details['total_sell_return_exc_tax_currency2']) ? $sell_return_details['total_sell_return_exc_tax_currency2'] : 0;

            $total_sell = $sell_details['total_sell_exc_tax'] - $total_sell_return;
            $total_sell_currency2 = $sell_details['total_sell_exc_tax_currency2'] - $total_sell_return_currency2;

            return [
                'total_sell_exc_tax' => $sell_details['total_sell_exc_tax'],
                'total_sell_exc_tax_currency2' => $sell_details['total_sell_exc_tax_currency2'],
                'total_sell_return_exc_tax' => $total_sell_return,
                'total_sell_return_exc_tax_currency2' => $total_sell_return_currency2,
                'total_sell' => $total_sell,
                'total_sell_currency2' => $total_sell_currency2,
            ];
        }
    }
    ...
    public function getCustomerGroup(Request $request)
    {
        ...
        if ($request->ajax()) {
            $query = Transaction::from("transactions as t")
                        ->leftjoin("business as b", "b.id", '=', 't.business_id')
                        ->leftjoin('customer_groups AS CG', 't.customer_group_id', '=', 'CG.id')
                        ->where('t.business_id', $business_id)
                        ->where('t.type', 'sell')
                        ->where('t.status', 'final')
                        ->groupBy('t.customer_group_id')
                        ->select(
                            DB::raw('SUM( 
                                CASE 
                                    WHEN t.currency_id is NULL or t.currency_id = b.currency_id
            ...
            if (! empty($group_id)) {
                $query->where('t.customer_group_id', $group_id);
            }
            ...
            return Datatables::of($query)
                ->editColumn('total_sell', function ($row) use($business) {
                    if($business->second_currency_id > 0) {
                        return  '<span>' 
                                . \App\Utils\Util::format_currency($row->total_sell, $business->currency) . " / " 
                                . \App\Utils\Util::format_currency($row->total_sell_currency2, $business->secondCurrency)
                                . '</span>';
                    } else {
                        return '<span>' . \App\Utils\Util::format_currency($row->total_sell, $business->currency) . '</span>';
                    }
                })
                ->rawColumns(['total_sell', 'total_sell_currency2'])
                ->make(true);
        }
        ...
        return view('report.customer_group')
            ->with(compact('customer_group', 'business_locations', 'currencies'));
    }
    ...
    public function getproductPurchaseReport(Request $request)
    {
        ...
        if ($request->ajax()) {
            ...
            return Datatables::of($query)
                ->editColumn('product_name', function ($row) {
                 ...
                 ->editColumn('unit_purchase_price', function($row) use($business) {
                    if($business->isMainCurrency($row->currency_id)) {
                        $currency = $business->currency;
                    } else {
                        $currency = $business->secondCurrency;
                    ...
                    return  '<span>' . 
                            \App\Utils\Util::format_currency($row->unit_purchase_price, $currency).
                            '</span>';
                 })
                 ...
                 ->editColumn('subtotal', function ($row) use($business) {
                    if($business->isMainCurrency($row->currency_id)) {
                        $currency = $business->currency;
                        $data_orig = "data-orig-value";
                    } else {
                    ...
                    return  '<span class="row_subtotal" ' . $data_orig . '="' . $row->subtotal . '">' .
                            \App\Utils\Util::format_currency($row->subtotal, $currency) .
                            '</span>';
                 })
        ...
        return view('report.product_purchase_report')
            ->with(compact('business_locations', 'suppliers', 'brands', 'currencies'));
    }
    ...
    public function getproductSellReport(Request $request)
    {
        ...
        if ($request->ajax()) {
            ...
            return Datatables::of($query)
                ...
                ->editColumn('transaction_date', '{{@format_datetime($transaction_date)}}')
                ->editColumn('unit_sale_price', function ($row) use ($business) {
                    if($business->isMainCurrency($row->currency_id)) {
                        $currency = $business->currency;
                    } else {
                        $currency = $business->secondCurrency;
                    ...
                    return  '<span class="unit_sale_price" >' .
                            \App\Utils\Util::format_currency($unit_sale_price, $currency) .
                            '</span>';
                })
                ->editColumn('sell_qty', function ($row) {
                 ...
                 ->editColumn('subtotal', function ($row) use ($business) {
                    if($business->isMainCurrency($row->currency_id)) {
                     ...
                     return '<span class="'.$class.'" ' . $data_orig . '="' . $subtotal . '">'.
                            \App\Utils\Util::format_currency($subtotal, $currency). 
                            '</span>';
                 })
                ->editColumn('unit_price', function ($row) use ($business) {
                    if($business->isMainCurrency($row->currency_id)) {
                        $currency = $business->currency;
                    } else {
                        $currency = $business->secondCurrency;
                    ...
                    return  '<span class="unit_price">' .
                            \App\Utils\Util::format_currency($price, $currency) . 
                            '</span>';
                })
                ->editColumn('discount_amount', function($row) use($business) {
                    if($business->isMainCurrency($row->currency_id)) {
                        $currency = $business->currency;
                    } else {
                        $currency = $business->secondCurrency;
                    ...
                    if($row->discount_type == "percentage") {
                        ...
                        return  '<span class="discount-amount" >' .
                                \App\Utils\Util::format_currency($discount_amount, $currency) .
                                '</span>';
                    }
                })
                ->editColumn('tax', function ($row) use($business) {
                    if($business->isMainCurrency($row->currency_id)) {
                        $currency = $business->currency;
                        $data_orig = "data-orig-value";
                    } else {
                    ...
                    $itemTax = $row->item_tax;
                    return  '<span ' . $data_orig . '="' . $row->item_tax . 
                            '" class="tax" data-unit="' . $row->tax . '">' . 
                            \App\Utils\Util::format_currency($itemTax, $currency) .
                            '<small>(' . $row->tax.')</small>' .
                            '</span>';
                })
                ->addColumn('payment_methods', function ($row) use ($payment_types) {
        ...
        return view('report.product_sell_report')
            ->with(compact('business_locations', 'customers', 'categories', 'brands',
                'customer_group', 'product_custom_field1', 'product_custom_field2', 'currencies'));
    }
    ...
    /**
     * Shows purchase invoice payment report
     *
     * @return \Illuminate\Http\Response
     */
    public function purchaseInvoicePaymentReport(Request $request) {
        if (! auth()->user()->can('purchase_n_sell_report.view')) {
            abort(403, 'Unauthorized action.');
        }
        ...
        if ($request->ajax()) {
            ...
            // To get total count of purchases
            $total_count = DB::table(DB::raw("({$query->toSql()}) AS temp_table"))  
                ->mergeBindings($query->getQuery()) // Merge bindings from the subquery  
                ->select(DB::raw('COUNT(*) as total_count'))  
                ->first()->total_count;

            $query->offset($request->get('start', 0));
            $query->limit($request->get('length', 25));
            $purchases = $query->get();
            $recordFiltered = count($purchases);

            // To get purchase_id array
            $purchase_ids = [];
            ...
            // To get payments
            $payments = TransactionPayment::from("transaction_payments as tp")
                ->leftjoin("transactions as t", "t.id", "=", "tp.transaction_id")
                ->whereIn('tp.transaction_id', $purchase_ids)
                ->select(
                    'tp.id',
                    'tp.transaction_id',
                    'tp.method',
            ...
            foreach($purchases as $purchase) {
                $data[] = $purchase;
                $tps = $payments->filter(function($payment) use ($purchase) {
                    return $payment->transaction_id == $purchase->id;
                });
                foreach($tps as $tp) {
                    $data[] = $tp;
                }
            ...
            $output = Datatables::of($data)
                ...
                ->editColumn('amount_paid', function($row) use ($business) {
                    $classname = get_class($row);
                    if($classname == 'App\Transaction') {
                        $paid_total = $row->amount_paid;
                        $data_orig = $business->isMainCurrency($row->currency_id) ? "data-orig-value" : "data-orig-value-currency2";
                        return '<span class="paid_total" ' . $data_orig . '="' . $paid_total . '"> '
                                . \App\Utils\Util::format_currency($paid_total, $row->currency, false)
                                . '</span>';
                    } else {
                        $currency = $business->isMainCurrency($row->currency_id) ? $business->currency : $business->secondCurrency;
                        return \App\Utils\Util::format_currency($row->amount, $currency, false);
                    }
                })
                ...
                ->rawColumns(['final_total', 'amount_paid', 'payment_detail', 'tax_amount', 'exchange_rate', 'action', 'payment_due', 'currency',
                    'payment_status', 'status', 'ref_no', 'name'])
                ->make(true);

            $output_data = $output->getData(true);
            $output->setData([
                'draw' => $output_data['draw'],
        ...
        return view('report.purchase_invoice_payment_report')
            ->with(compact('business_locations', 'suppliers', 'currencies'));        
    }
    ...
    public function purchasePaymentReport(Request $request)
    {
        ...
        if ($request->ajax()) {
            ...
            return Datatables::of($query)
                ...
                ->editColumn('amount', function ($row) use($business) {
                    if($business->isMainCurrency($row->currency_id)) {
                        $currency = $business->currency;
                        $data_orig = "data-orig-value";
                    } else {
                    ...
                    $amount = $row->amount;
                    return  '<span class="paid-amount" ' . $data_orig . '="' . $amount . '" >'.
                            \App\Utils\Util::format_currency($amount, $currency) . 
                            '</span>';
                })
    ...
    /**
     * Shows sell invoice payment report
     *
     * @return \Illuminate\Http\Response
     */
    public function sellInvoicePaymentReport(Request $request) {
        $business_id = $request->session()->get('user.business_id');
        $business = Business::find($business_id);
        $currencies = $this->transactionUtil->purchaseCurrencyDetails($business_id);
        ...
        if ($request->ajax()) {
            ...
            $payments = TransactionPayment::from("transaction_payments as tp")
                ->whereIn('transaction_id', $sell_ids)
                ->leftjoin("transactions as t", "t.id", "=", "tp.transaction_id")
                ->select(
                    'tp.id',
                    'tp.transaction_id',
            ...
            foreach($sells as $sell) {
                $data[] = $sell;
                $tps = $payments->filter(function($payment) use ($sell) {
                    return $payment->transaction_id == $sell->id;
                });
                foreach($tps as $tp) {
                    $data[] = $tp;
                }
            ...
            $output = Datatables::of($data)
                ...
                ->addColumn('total_remaining', function ($row) use($business) {
                    $classname = get_class($row);
                    if($classname == 'App\Transaction') {
                        $data_orig = $business->isMainCurrency($row->currency_id) ? "data-orig-value" : "data-orig-value-currency2";
                ...
                ->rawColumns(['final_total', 'amount_paid', 'tax_amount', 'exchange_rate', 'action', 'payment_due', 'currency',
                    'payment_status', 'status', 'ref_no', 'contact_name', 'total_remaining'])
                ->make(true);
        ...
        return view('report.sell_invoice_payment_report')
            ...
            $query = TransactionSellLine::join('transactions as t','transaction_sell_lines.transaction_id','=','t.id')
                ...
                ->select(
                    ...
                    DB::raw('SUM(
                        CASE
                            WHEN t.currency_id is NULL or t.currency_id = b.currency_id
                            THEN (transaction_sell_lines.quantity - transaction_sell_lines.quantity_returned) * transaction_sell_lines.unit_price_inc_tax
                        END
                    ) as subtotal'),
                    DB::raw('SUM(
                        CASE
                 ...
                 ->editColumn('subtotal', function ($row) use ($business) {
                     ...
                     if($business->second_currency_id > 0) {
                        return  '<span class="'.$class.'" data-orig-value="'.$row->subtotal
                                . '" data-orig-value-currency2="' . $row->subtotal_currency2 . '">'
                                . \App\Utils\Util::format_currency($row->subtotal, $business->currency) . ' / '
                                . \App\Utils\Util::format_currency($row->subtotal_currency2, $business->secondCurrency)
                                .'</span>';
                     } else {
                        return  '<span class="'.$class.'" data-orig-value="'.$row->subtotal.'">'
                                . \App\Utils\Util::format_currency($row->subtotal, $business->currency)
    ...
    public function productSellReportBy(Request $request)
    {
        ...
        if ($request->ajax()) {
            $query = TransactionSellLine::join('transactions as t','transaction_sell_lines.transaction_id','=','t.id')
                ->join('business as bs', 't.business_id', '=', 'bs.id')
                ...
                ->select(
                    ...
                    DB::raw('SUM(
                        CASE 
                            WHEN t.currency_id is NULL or t.currency_id = bs.currency_id
                            THEN (transaction_sell_lines.quantity - transaction_sell_lines.quantity_returned) * transaction_sell_lines.unit_price_inc_tax
                        END) as subtotal'),
                    DB::raw('SUM(
                        CASE 
                            WHEN t.currency_id > 0 and t.currency_id != bs.currency_id
                            THEN (transaction_sell_lines.quantity - transaction_sell_lines.quantity_returned) * transaction_sell_lines.unit_price_inc_tax
                        END) as subtotal_currency2'),
                );
    ...
    public function getProfit($by = null)
    {
        ...
        $query->select(
            DB::raw('SUM(
                CASE 
                    ...
                    THEN IF (TSPL.id IS NULL AND P.type="combo", 
        ...
        if ($by == 'day') {
            ...
            foreach ($results as $result) {
                ...
                $profits_currency2[strtolower($result->day)] = $result->gross_profit_currency2;
            }
            $days = ['monday', 'tuesday', 'wednesday', 'thursday', 'friday', 'saturday', 'sunday'];

            return view('report.partials.profit_by_day')->with(compact('profits','business','profits_currency2', 'days', 'currencies'));
        }
        ...
        $datatable = Datatables::of($query);
        $datatable->editColumn('gross_profit', function ($row) use($business, $by) {
            if(in_array($by, ['invoice'])) {
                ...
                if($business->isMainCurrency($row->currency_id)) {
                    ...
                    $profit = $row->gross_profit_currency2 - $discount;
                    $currency = $business->secondCurrency;
                    $data_orig = "data-orig-value-currency2";
                }

                return  '<span class="gross-profit" ' . $data_orig .'="' . $profit . '" >'
                        . \App\Utils\Util::format_currency($profit, $currency)
                        .'</span>';
            } else {
                if(empty($business->second_currency_id)) {
                    $profit = $row->gross_profit;
                    return  '<span class="gross-profit" data-orig-value="' . $profit . '" >'
                            . \App\Utils\Util::format_currency($profit, $business->currency)
                            .'</span>';
                } else {
                    $profit = $row->gross_profit;
                    $profit_currency2 = $row->gross_profit_currency2;
                    return  '<span class="gross-profit" data-orig-value="' . $profit . '" '
                            . ' data-orig-value-currency2="' . $profit_currency2 . '" >'
                            . \App\Utils\Util::format_currency($profit, $business->currency) . ' / '
                            . \App\Utils\Util::format_currency($profit_currency2, $business->secondCurrency)
                            .'</span>';
                }
            }            
        });
    ...
    public function itemsReport()
    {
        ...
        if (request()->ajax()) {
            $query = TransactionSellLinesPurchaseLines::leftJoin('transaction_sell_lines 
                ...
                ->select(
                    ...
                    'p.type as product_type',
                    'p.name as product_name',
                    'purchase.currency_id as purchase_currency_id',
                    'v.name as variation_name',
                    'pv.name as product_variation',
                    'u.short_name as unit',
                    ...
                    'sale.currency_id as sale_currency_id',
            ...
            return Datatables::of($query)
                ...
                ->editColumn('purchase_price', function ($row) use ($business) {
                    if($row->purchase_currency_id == NULL || $row->purchase_currency_id == $business->currency_id) {
                        $currency = $business->currency;
                    } else {
                        $currency = $business->secondCurrency;
                    }
                    return '<span class="purchase_price">'.
                            \App\Utils\Util::format_currency($row->purchase_price, $currency) .
                            '</span>';
                })
                ...
                ->editColumn('quantity', function ($row) {
                 ...
                 ->editColumn('selling_price', function ($row) use($business) {
                    if($row->sale_currency_id == NULL || $row->sale_currency_id == $business->currency_id) {
                        $currency = $business->currency;
                    } else {
                        $currency = $business->secondCurrency;
                    }
                    $selling_price = ! empty($row->sell_line_id) ? $row->selling_price : $row->stock_adjustment_price;
                    return '<span class="row_selling_price">' . 
                            \App\Utils\Util::format_currency($selling_price, $currency) .
                            '</span>';
                 })

                 ->addColumn('subtotal', function ($row) use ($business){
                    if($row->sale_currency_id == NULL || $row->sale_currency_id == $business->currency_id) {
                        $currency = $business->currency;
                        $data_orig = "data-orig-value";
                    } else {
                     ...
                     return '<span class="row_subtotal" ' . $data_orig . '="' . $subtotal . '">'.
                            \App\Utils\Util::format_currency($subtotal, $currency) .
                            '</span>';
                 })
        ...
        return view('report.items_report')->with(compact('suppliers', 'customers', 'business_locations', 'currencies'));
    }
    ...
    public function getStockValue()
    {
        $business_id = request()->session()->get('user.business_id');
        $location_id = request()->input('location_id');
        $filters = request()->only(['category_id', 'sub_category_id', 'brand_id', 'unit_id', 'to_date']);

        $end_date = \Carbon::now()->format('Y-m-d');
        if(!empty($filters['to_date'])) {
            $end_date = $this->transactionUtil->uf_date($filters['to_date']);
        }
        ...
        $potential_profit = $closing_stock_by_sp['stock'] - $closing_stock_by_pp['stock'];
        $potential_profit_currency2 = $closing_stock_by_sp['stock_currency2'] - $closing_stock_by_pp['stock_currency2'];
        $profit_margin = emptyNumber($closing_stock_by_sp['stock']) ? 0 : ($potential_profit / $closing_stock_by_sp['stock']) * 100;
        $profit_margin_currency2 = emptyNumber($closing_stock_by_sp['stock_currency2']) ? 0 : ($potential_profit_currency2 / $closing_stock_by_sp['stock_currency2']) * 100;

        return [
            'closing_stock_by_pp' => $closing_stock_by_pp['stock'],
            'closing_stock_by_pp_currency2' => $closing_stock_by_pp['stock_currency2'],

            'closing_stock_by_sp' => $closing_stock_by_sp['stock'],
            'closing_stock_by_sp_currency2' => $closing_stock_by_sp['stock_currency2'],

            'potential_profit' => $potential_profit,
            'potential_profit_currency2' => $potential_profit_currency2,

            'profit_margin' => $profit_margin,
            'profit_margin_currency2' => $profit_margin_currency2
        ];
    }
