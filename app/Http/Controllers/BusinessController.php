...
class BusinessController extends Controller
{
    ...
    */
    ...
    public function getNationExchgRateHistory($startDate, $endDate) {
        $nerhChart = new CommonChart;
        $business_id = request()->session()->get('user.business_id');
        $rows = NationExchgRateHistory::where('business_id', $business_id)
        ...
        if (request()->ajax()) {
            ...
            foreach($rows as $r) {
                $dates[] = $r->date;
                $rates[] = intval($r->nation_exchg_rate);
            }
    
            $nerhChart->labels($dates);
            $nerhChart->dataset('Date', 'line', $rates);
            
            return view("business.partials.nerh", compact('nerhChart'));
        }
    }
    ...
    public function postBusinessSettings(Request $request)
    {
        ...
        try {
            ...
            $business_details = $request->only([
                'name', 'start_date', 'currency_id', 'second_currency_id', 'nation_currency_id',
                'nation_exchg_rate', 'tax_label_1', 'tax_number_1', 'tax_label_2', 'tax_number_2', 
                'default_profit_percent', 'default_sales_tax', 'default_sales_discount', 'sell_price_tax', 'sku_prefix', 'time_zone', 'fy_start_month', 'accounting_method', 'transaction_edit_days', 'sales_cmsn_agnt', 'item_addition_method', 'currency_symbol_placement', 'on_product_expiry',
            ...
            $business_details['enable_bcu'] = $request->input("enable_bcu") ?? 0;

            $business_details['second_currency_exchg_rate'] = ($business_details['currency_id'] == $business_details['nation_currency_id']) ? 
                                                    $business_details['nation_exchg_rate'] : (1 / $business_details['nation_exchg_rate']);
            ...
            $checkboxes = ['enable_editing_product_from_purchase',
                ...
                'enable_lot_number', 'enable_racks', 'enable_row', 'enable_position', 'enable_sub_units', 'show_combo_variations'];
            ...
            // insert/update the nation_exchg_rate
            $localtimezone = new DateTimeZone('America/Montevideo');
            $now = new \DateTime('now', $localtimezone);
            $nowStr = $now->format('Y-m-d');

            $nerh = NationExchgRateHistory::where('business_id', $business_id)->where('date', $nowStr)->first();
            if(empty($nerh)) $nerh = new NationExchgRateHistory();

            $nerh->date = $nowStr;
            $nerh->business_id = $business_id;
            $nerh->nation_exchg_rate = $business_details['nation_exchg_rate'];
            $nerh->save();
    ...
    public function postCheckPin(Request $request) {
