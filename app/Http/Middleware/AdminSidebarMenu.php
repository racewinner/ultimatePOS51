                ...
                $menu->dropdown(
                    ...
                    function ($sub) use ($common_settings) {
                        ...
                        $sub->url(
                            ...
                            ['icon' => 'fa fas fa-undo', 'active' => request()->segment(1) == 'payments' && request()->trans_type == 'purchase']
                        );
                        $sub->url(
                            action([\App\Http\Controllers\PymoController::class, 'purchaseCfes']),
                            __('lang_v1.dgi_received_invoices'),
                            ['icon' => 'fa fas fa-file-import', 'active' => request()->segment(1) == 'purchaseCfes']
                        );
                    },
            ...
            if ($is_admin || auth()->user()->hasAnyPermission(['sell.view', 'sell.create', 'direct_sell.access', 'view_own_sell_only', 'view_commission_agent_sell', 'access_shipping', 'access_own_shipping', 'access_commission_agent_shipping', 'access_sell_return', 'direct_sell.view', 'direct_sell.update', 'access_own_sell_return'])) {
                $menu->dropdown(
                    ...
                    function ($sub) use ($enabled_modules, $is_admin, $pos_settings) {
                        ...
                        $sub->url(
                            action([\App\Http\Controllers\TransactionPaymentController::class, 'index']),
                            __('lang_v1.list_payment'),
                            ['icon' => 'fa fas fa-undo', 'active' => request()->segment(1) == 'payments' && request()->trans_type != 'purchase']
                        );
                        
                        $sub->url(
                            action([\App\Http\Controllers\RemitoController::class, 'index']),
                            __('lang_v1.list_remito'),
                            ['icon' => 'fa fas fa-gift', 'active' => request()->segment(1) == 'remitos']
                        );
                        ...
                        $sub->url(
                            action([\App\Http\Controllers\PymoController::class, 'sentCfes']),
                            __('lang_v1.invoices_by_dgi'),
                            ['icon' => 'fa fas fa-file-import', 'active' => request()->segment(1) == 'sentCfes']
                        );
                    },
                ...
                $menu->dropdown(
                    ...
                    function ($sub) use ($enabled_modules, $is_admin) {
                        ...
                        if (auth()->user()->can('purchase_n_sell_report.view')) {
                            ...
                            $sub->url(
                                action([\App\Http\Controllers\ReportController::class, 'purchaseInvoicePaymentReport']),
                                __('lang_v1.purchase_invoice_payment_report'),
                                ['icon' => 'fa fas fa-search-dollar', 'active' => request()->segment(2) == 'purchase-invoice-payment-report']
                            );
                            ...
                            $sub->url(
                                action([\App\Http\Controllers\ReportController::class, 'sellInvoicePaymentReport']),
                                __('lang_v1.sell_invoice_payment_report'),
                                ['icon' => 'fa fas fa-search-dollar', 'active' => request()->segment(2) == 'sell-invoice-payment-report']
                            );
                        }
