...
if (! function_exists('formatDecimalPoint')) {
    function formatDecimalPoint($number, $type = 'currency')
    {
        $precision = 4;
...
if (! function_exists('formatDate')) {
    function formatDate($dateStr, $long=true) {
        $dateTime = new DateTime($dateString);
        
        if($long) $formattedDate = $dateTime->format('Y-m-d H:i:s');
        else $formattedDate = $dateTime->format('Y-m-d');
...
if (! function_exists('emptyNumber')) {
    function emptyNumber($v) {
        return empty($v) || floatval($v) == 0;
    }
}