...
<tr class="product_row" data-row_index="{{$row_count}}" @if(!empty($so_line)) data-so_id="{{$so_line->transaction_id}}" @endif>
	...
	<td>
		...
		@if(!empty($product->remito_line_id))
			<input type="hidden" name="products[{{$row_count}}][remito_line_id]" class="form-control" value="{{$product->remito_line_id}}">
		@endif
		...
		<div class="input-group input-number">
			<span class="input-group-btn"><button type="button" class="btn btn-default  quantity-down"><i class="fa fa-minus text-danger"></i></button></span>
		<input type="text" data-min="1" 
			...
			@endif 
		>
		<span class="input-group-btn"><button type="button" class="btn btn-default  quantity-up"><i class="fa fa-plus text-success"></i></button></span>
		</div>
	...
	<td class="text-center v-center">
		<i class="fa fa-times text-danger remito_remove_row cursor-pointer" aria-hidden="true"></i>
	</td>
</tr>