@forelse ($products as $product)
    ...
    <tr>
        ...
        @if(request()->session()->get('business.enable_lot_number') == 1)
            <td>
                <input type="text" class="form-control" name="products[{{$loop->index + $index}}][lot_number]"
                    value="@if(isset($product->lot_number)){{$product->lot_number}}@endif">
            </td>
        @endif
        @if(request()->session()->get('business.enable_product_expiry') == 1)
            <td>
                <input type="text" class="form-control label-date-picker" name="products[{{$loop->index + $index}}][exp_date]"
