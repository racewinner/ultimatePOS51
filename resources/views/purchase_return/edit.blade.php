...
<input type="hidden" id="p_symbol" value="{{ $purchase_return->currency->symbol }}">
<input type="hidden" id="p_thousand" value="{{ $purchase_return->currency->thousand_separator }}">
<input type="hidden" id="p_decimal" value="{{ $purchase_return->currency->decimal_separator }}">
						...
						<div class="input-group">
							...
							<select name='currency_id' class='form-control select2' disabled>
								@foreach ($currencies as $currency)
									<option value={{ $currency->id }} {{ $currency->id == $purchase_return->currency_id ? "selected" : "" }} data-thousand-separator={{ empty($currency->thousand_separator) ? "NA" : $currency->thousand_separator }} data-decimal-separator={{ empty($currency->decimal_separator) ? "NA" : $currency->decimal_separator}}
				...
				<div class="col-sm-3">
					<div class="form-group">
						...
						<div class="input-group">
							...
							{!! Form::number('nation_exchg_rate', $purchase_return->nation_exchg_rate, ['class' => 'form-control', 'placeholder' => __('lang_v1.currency_exchange_rate'), 'readonly' => true]) !!}
						</div>
					</div>
				</div> <!-- exchange rate -->
			</div>
			...
			<div class="row">
						...
						<select name="tax_id" id="tax_id" class="form-control select2" placeholder="'Please Select'">
							...
							@foreach($taxes as $tax)
								<option value="{{ $tax->id }}" data-tax_amount="{{ $tax->amount }}"
									data-tax_type="{{ $tax->calculation_type }}" @if($purchase_return->tax_id == $tax->id)
									selected @endif>{{ $tax->name }}</option>
							@endforeach
						</select>
				...
				<div class="col-md-8">
					<div class="pull-right">
						...
						<span id="total_return">
							{{ \App\Utils\Util::format_currency($purchase_return->final_total, $purchase_return->currency) }}
						</span>
					</div>
				</div>
			</div>
	...
	<div class="row">
		<div class="col-md-12">
			<button type="button" id="submit_purchase_return_form"
				class="btn btn-primary pull-right ">@lang('messages.update')</button>
		</div>
	</div>
	{!! Form::close() !!}
</section>
