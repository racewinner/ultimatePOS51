...
<section class="content">
				...
				<tbody>
					@foreach($purchase->purchase_lines as $purchase_line)
						...
						<tr>
							...
							<td>
								{{ \App\Utils\Util::format_currency($purchase_line->purchase_price_inc_tax, $purchase->currency) }}
							</td>
							<td><span class="display_currency" data-is_quantity="true"
									data-currency_symbol="false">{{ $purchase_line->quantity }}</span> {{$unit_name}}</td>
	...
	<div class="row">
		<div class="col-sm-4">
			...
			<span id="total_return_tax"></span> @if(!empty($purchase->tax))({{$purchase->tax->name}} -
			{{$purchase->tax->amount}}%)@endif
			@php
				$tax_percent = 0;
