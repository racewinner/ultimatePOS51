        ...
        $(document).ready(function () {
            ...
            purchase_return_table = $('#purchase_return_datatable').DataTable({
                ...
                "fnDrawCallback": function (oSettings) {
                    $('#footer_purchase_return_total').html(
                        __currency_trans_from(sum_table_col($('#purchase_return_datatable'), 'final_total'), true, currencies[0]) +
                        (currencies.length > 1 ? ' / ' + __currency_trans_from(sum_table_col($('#purchase_return_datatable'), 'final_total', 'orig-value-currency2'), true, currencies[1]) : '')
                    );
                    $('#footer_payment_status_count').html(__sum_status_html($('#purchase_return_datatable'), 'payment-status-label'));
                    $('#footer_total_due').html(
                        __currency_trans_from(sum_table_col($('#purchase_return_datatable'), 'payment_due'), true, currencies[0]) +
                        (currencies.length > 1 ? ' / ' + __currency_trans_from(sum_table_col($('#purchase_return_datatable'), 'payment_due', 'orig-value-currency2'), true, currencies[1]) : '')
                    );
                },
