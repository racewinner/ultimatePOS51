						...
						<div class="input-group">
							...
							<select name='currency_id' class='form-control select2' required>
								@foreach ($currencies as $currency)
									<option value={{ $currency->id }} data-thousand-separator={{ empty($currency->thousand_separator) ? "NA" : $currency->thousand_separator }}
										data-decimal-separator={{ empty($currency->decimal_separator) ? "NA" : $currency->decimal_separator}} data-symbol={{ $currency->symbol }}>
										{{ $currency->name }}
									</option>
								@endforeach
							</select>
	...
	<div class="row">
		<div class="col-md-12 text-center">
			<button type="button" id="submit_purchase_return_form"
				class="btn btn-primary btn-big ">@lang('messages.submit')</button>
		</div>
	</div>
	{!! Form::close() !!}
</section>
