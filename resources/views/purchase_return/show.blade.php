    ...
    <div class="modal-body">
      <div class="row">
        ...
        <div class="col-sm-6 col-xs-6">
            ...
            <strong>@lang('messages.date'):</strong> {{@format_date($purchase->transaction_date)}} <br>
            <strong>@lang('lang_v1.currency_exchange_rate'): </strong> {{number_format((float)$purchase->nation_exchg_rate, 2, '.', '')}}
        </div>
        @else
            <div class="col-sm-6 col-xs-6">
                <h4>@lang('lang_v1.purchase_return_details'):</h4>
                ...
                <strong>@lang('purchase.supplier'):</strong> {!! $purchase->contact->contact_address !!} <br>
                <strong>@lang('purchase.business_location'):</strong> {{ $purchase->location->name }} <br>
                @if($purchase->nation_exchg_rate > 0)
                <strong>@lang('lang_v1.currency_exchange_rate'): </strong> {{number_format((float)$purchase->nation_exchg_rate, 2, '.', '')}}
                @endif
            </div>
        @endif
...
<script type="text/javascript">
    ...
    const purchase_currency = @json($purchase->currency);
    __p_currency_symbol = purchase_currency.symbol;
    __p_currency_thousand_separator = purchase_currency.thousand_separator;
    __p_currency_decimal_separator = purchase_currency.decimal_separator;
