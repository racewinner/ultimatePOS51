...
$(document).ready( function(){
    ...
    sell_table = $('#sell_table').DataTable({
        ...
        columns: [
            ...
            { data: 'invoice_no', name: 'pymo_serie', "searchable": true},
        ...
        "footerCallback": function ( row, data, start, end, display ) {
            ...
            var footer_total_remaining_currency2 = 0;
            var footer_total_sell_return_due = 0;
            var footer_total_sell_return_due_currency2 = 0;
            ...
            for (var r in data){
                ...
                footer_total_remaining_currency2 += $(data[r].total_remaining).data('orig-value-currency2') ? parseFloat($(data[r].total_remaining).data('orig-value-currency2')) : 0;
                ...
                footer_total_sell_return_due_currency2 += $(data[r].return_due).find('.sell_return_due').data('orig-value-currency2') ? parseFloat($(data[r].return_due).find('.sell_return_due').data('orig-value-currency2')) : 0;
            ...
            $('.footer_total_sell_return_due').html(
                __currency_trans_from(footer_total_sell_return_due, true, currencies[0]) + 
                (currencies.length > 1 ? ' / ' + __currency_trans_from(footer_total_sell_return_due_currency2, true, currencies[1]) : '')
            );
            $('.footer_total_remaining').html(
                __currency_trans_from(footer_total_remaining, true, currencies[0]) + 
                (currencies.length > 1 ? ' / ' + __currency_trans_from(footer_total_remaining_currency2, true, currencies[1]) : '')
            );
