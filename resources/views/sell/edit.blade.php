...
@section('content')
<input type="hidden" id="p_code" value="{{$transaction->currency->code}}">
<input type="hidden" id="p_symbol" value="{{$transaction->currency->symbol}}">
<input type="hidden" id="p_thousand" value="{{$transaction->currency->thousand_separator}}">
<input type="hidden" id="p_decimal" value="{{$transaction->currency->decimal_separator}}">
<input type="hidden" name="nation_currency_id" value="{{ $business_details->nation_currency_id }}" >
...
{!! Form::open(['url' => action([\App\Http\Controllers\SellPosController::class, 'update'], ['po' => $transaction->id ]), 'method' => 'put', 'id' => 'edit_sell_form', 'files' => true, 'class'=>$transaction->canEdit() ? '':'readonly' ]) !!}
					...
					<div class="form-group">
						...
						<div class="input-group">
							...
							<span class="input-group-btn">
								<button type="button" class="btn btn-default bg-white  add_new_customer" data-name=""><i class="fa fa-plus-circle text-primary fa-lg"></i></button>
							</span>
						</div>
				...
				<div class="col-sm-4">
					<div class="form-group">
						...
						<div class="input-group">
							...
							<select name='currency_id' class='form-control select2' disabled>
				...
				<div class="col-sm-3">
					<div class="form-group">
						...
						<div class="input-group">
							...
							{!! Form::number('nation_exchg_rate', $transaction->nation_exchg_rate, ['class' => 'form-control', 'placeholder' => __('lang_v1.currency_exchange_rate'), 'readonly'=>true]) !!}
						</div>
					</div>
				</div>
				...
				<div class="col-sm-10 col-sm-offset-1">
					<div class="form-group">
						<div class="input-group">
							...
							<span class="input-group-btn">
								<button type="button" class="btn btn-default bg-white  pos_add_quick_product" data-href="{{action([\App\Http\Controllers\ProductController::class, 'quickAdd'])}}" data-container=".quick_add_product_modal"><i class="fa fa-plus-circle text-primary fa-lg"></i></button>
							</span>
						</div>
					</div>
				</div>
	...
	@can('sell.payments')
		...
		@component('components.widget', ['class' => 'box-solid', 'title' => __('lang_v1.electronic_bill')])
			@include('sale_pos.partials.electronic_bill')
		@endcomponent
	@endcan
	@endif
	...
	<div class="row">
		<div class="col-md-12 text-center">
	    	...
	    	<button type="button" class="btn btn-primary btn-big can-edit" id="submit-sell">@lang('messages.update')</button>
	    	<button type="button" id="save-and-print" class="btn btn-success btn-big can-edit">@lang('lang_v1.update_and_print')</button>
	    </div>
	</div>
	@if(in_array('subscription', $enabled_modules))
