                ...
                <ul class="nav nav-tabs">
                    <li class="active">
                        <a href="#input_tax_tab" data-toggle="tab" aria-expanded="true"><i
                                class="fa fas fa-arrow-circle-down" aria-hidden="true"></i>
                            @lang('report.input_tax')</a>
                    </li>
        ...
        $(document).ready(function () {
            ...
            input_tax_table = $('#input_tax_table').DataTable({
                ...
                fnDrawCallback: function (oSettings) {
                    $('#sell_total').text(
                        __currency_trans_from(sum_table_col($('#input_tax_table'), 'total_before_tax'), true, currencies[0]) +
                        (currencies.length > 1 ? ' / ' + __currency_trans_from(sum_table_col($('#input_tax_table'), 'total_before_tax', 'orig-value-currency2'), true, currencies[1]) : '')
                    );
                    @foreach($taxes as $tax)
                        $("#total_input_{{$tax['id']}}").text(
                            __currency_trans_from(sum_table_col($('#input_tax_table'), "tax_{{$tax['id']}}"), true, currencies[0]) +
                            (currencies.length > 1 ? ' / ' + __currency_trans_from(sum_table_col($('#input_tax_table'), "tax_{{$tax['id']}}", "orig-value-currency2"), true, currencies[1]) : '')
                        );
                    @endforeach
                },
            });
                ...
                if ($(e.target).attr('href') == '#output_tax_tab') {
                    if (typeof (output_tax_datatable) == 'undefined') {
                        output_tax_datatable = $('#output_tax_table').DataTable({
                            ...
                            fnDrawCallback: function (oSettings) {
                                debugger;
                                $('#purchase_total').text(
                                    __currency_trans_from(sum_table_col($('#output_tax_table'), 'total_before_tax'), true, currencies[0]) +
                                    (currencies.length > 1 ? ' / ' + __currency_trans_from(sum_table_col($('#output_tax_table'), 'total_before_tax', 'orig-value-currency2'), true, currencies[1]) : '')
                                );
                                @foreach($taxes as $tax)
                                    $("#total_output_{{$tax['id']}}").text(
                                        __currency_trans_from(sum_table_col($('#output_tax_table'), "tax_{{$tax['id']}}"), true, currencies[0]) +
                                        (currencies.length > 1 ? ' / ' + __currency_trans_from(sum_table_col($('#output_tax_table'), "tax_{{$tax['id']}}", "orig-value-currency2"), true, currencies[1]) : '')
                                    );
                                @endforeach
                            },
                        });
                    ...
                    if (typeof (expense_tax_datatable) == 'undefined') {
                        expense_tax_datatable = $('#expense_tax_table').DataTable({
                            ...
                            fnDrawCallback: function (oSettings) {
                                $('#expense_total').text(
                                    __currency_trans_from(sum_table_col($('#expense_tax_table'), 'total_before_tax'), true, currencies[0]) +
                                    (currencies.length > 1 ? ' / ' + __currency_trans_from(sum_table_col($('#expense_tax_table'), 'total_before_tax', 'orig-value-currency2'), true, currencies[1]) : '')
                                );
                                @foreach($taxes as $tax)
                                    $("#total_expense_{{$tax['id']}}").text(
                                        __currency_trans_from(sum_table_col($('#expense_tax_table'), "tax_{{$tax['id']}}"), true, currencies[0]) +
                                        (currencies.length > 1 ? ' / ' + __currency_trans_from(sum_table_col($('#expense_tax_table'), "tax_{{$tax['id']}}", "orig-value-currency2"), true, currencies[1]) : '')
                                    );
                                @endforeach
                            },
                        });
