...
@endif>
	<td class="{{ $product->product_type == 'combo' ? 'has-combo-variations' : '' }} {{ $business_details->show_combo_variations == 1 ? 'show-combo' : '' }}">
		@if(!empty($so_line))
		...
		<div class="d-flex">
			@if(($edit_price || $edit_discount) && empty($is_direct_sell))
				<div title="@lang('lang_v1.pos_edit_product_price_help')" class="flex-fluid">
			...
			@else
				<div class="flex-fluid">{!! $product_name !!}</div>
			@endif

			@if($product->product_type == 'combo')
				<div class="combo-product-tag">
					combo
					<i class="fa {{ $business_details->show_combo_variations == 1 ? 'fa-angle-up' : 'fa-angle-down' }}" aria-hidden="true"></i>
				</div>
			@endif
		</div>
		...
		@if(!empty($is_direct_sell))
			<br>
			<textarea class="form-control can-edit" name="products[{{$row_count}}][sell_line_note]"
				rows="2">{{$sell_line_note}}</textarea>
			<p class="help-block"><small>@lang('lang_v1.sell_line_description_help')</small></p>
		@endif

		@if(!empty($product->comboVariationDetails))
		<section class="combo-variations-section">
			<div class="row header m-0">
				<div class="col-md-8 column">@lang('product.product_name')</div>
				<div class="col-md-4 column">@lang('sale.qty')</div>
			</div>
			@foreach($product->comboVariationDetails as $combo_variation)
			<div class="row body m-0">
				<div class="col-md-8 column">{{ $combo_variation['variation']->product->name }}</div>
				<div class="col-md-4 column">{{ $combo_variation['quantity'] }}</div>
			</div>
			@endforeach
		</section>
		@endif
	...
	<td>
		...
		<div class="input-group input-number">
			<span class="input-group-btn"><button type="button" class="btn btn-default  quantity-down"><i
			...
			<input type="text" data-min="1" class="form-control pos_quantity input_number mousetrap input_quantity"
				...
				@endif>
			<span class="input-group-btn"><button type="button" class="btn btn-default  quantity-up"><i
		...
		<input type="hidden" class="prod-currency-id" value="{{$product->currency_id}}">
		<input type="hidden" class="prod-exchange-rate" value="{{$product->exchange_rate}}">
	...
	<td class="text-center">
		...
		<span class="pos_line_total_text @if(!empty($pos_settings['is_pos_subtotal_editable'])) hide @endif">
			{{ \App\Utils\Util::format_currency($product->quantity_ordered * $unit_price_inc_tax, $product->currency) }}
		</span>
	</td>
