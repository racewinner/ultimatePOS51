<table class="table @if(!empty($for_ledger)) table-slim mb-0 bg-light-gray @else bg-gray @endif" @if(!empty($for_pdf)) style="width: 100%;" @endif>
    ...
    @foreach($sell->sell_lines as $sell_line)
        <tr>
            ...
            @endif
            <td>
                {{ \App\Utils\Util::format_currency($sell_line->unit_price_before_discount, $sell->currency) }}
            </td>
            <td>
                {{ \App\Utils\Util::format_currency($sell_line->get_discount_amount(), $sell->currency) }}
                @if($sell_line->line_discount_type == 'percentage') ({{$sell_line->line_discount_amount}}%) @endif
            </td>
            <td>
                {{ \App\Utils\Util::format_currency($sell_line->item_tax, $sell->currency) }}
            ...
            <td>
                {{ \App\Utils\Util::format_currency($sell_line->quantity * $sell_line->unit_price_inc_tax, $sell->currency) }}
            </td>
        </tr>
        ...
        @foreach($sell_line->modifiers as $modifier)
            <tr>
                ...
                @endif
                <td>
                    {{ \App\Utils\Util::format_currency($modifier->unit_price, $sell->currency) }}
                </td>
                <td>
                    &nbsp;
                </td>
                <td>
                    {{ \App\Utils\Util::format_currency($modifier->item_tax, $sell->currency) }}
                    @if(!empty($taxes[$modifier->tax_id]))
                    ( {{ $taxes[$modifier->tax_id]}} )
                    @endif
                </td>
                <td>
                    {{ \App\Utils\Util::format_currency($modifier->unit_price_inc_tax, $sell->currency) }}
                </td>
                <td>
                    {{ \App\Utils\Util::format_currency($modifier->quantity * $modifier->unit_price_inc_tax, $sell->currency) }}
                </td>
            </tr>
            @endforeach
        @endif
