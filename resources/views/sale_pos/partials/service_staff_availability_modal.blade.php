					...
					<div class="col-md-3 col-xs-6 eq-height-col">
						<div class="small-box @if($is_available && empty($service_staff->paused_at)) bg-green @elseif(!empty($service_staff->paused_at)) bg-gray @else  bg-yellow @endif width-100">
				            ...
				            <div @if(!empty($service_staff->paused_at)) style="position: absolute; bottom: 0;" @endif>
				            	<a class="btn  small-box-footer bg-light-blue mark_as_available width-100" href="{{action([\App\Http\Controllers\SellPosController::class, 'markAsAvailable'], [$service_staff->id])}}">@lang('lang_v1.mark_as_available') <i class="fa fa-arrow-circle-right"></i></a>

				            	@if(empty($service_staff->paused_at))
			            			<button type="button" class="btn  small-box-footer bg-red pause_resume_timer width-100" data-href="{{action([\App\Http\Controllers\SellPosController::class, 'pauseResumeServiceStaffTimer'], [$service_staff->id])}}">@lang('lang_v1.pause_timer') <i class="fa fa-pause-circle"></i></button>
			            		@else
			            			<button type="button" class="btn  small-box-footer bg-green pause_resume_timer width-100" data-href="{{action([\App\Http\Controllers\SellPosController::class, 'pauseResumeServiceStaffTimer'], [$service_staff->id])}}">@lang('lang_v1.resume_timer') <i class="fa fa-redo"></i></button>
			            		@endif
			            	</div>
				            @endif
				         </div>
