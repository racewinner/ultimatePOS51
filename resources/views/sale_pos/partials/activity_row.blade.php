...
@if(!empty($final_total) && $final_total != $old_final_total)
    ...
    <td class="width-50 text-left">
        @if(!empty($old_final_total))
            <span class="label bg-info">{{ \App\Utils\Util::format_currency($old_final_total, $purchase->currency) }}</span> --> 
        @endif
         <span class="label bg-info">{{ \App\Utils\Util::format_currency($final_total, $purchase->currency) }}</span>
     </td>
    </tr>
@endif
