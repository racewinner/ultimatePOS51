...
<div class="col-md-6 col-sm-6 col-xs-6 text-right align-right @if(!empty($for_pdf)) width-50 f-left @endif">
	...
	<tr>
		...
		<td class="align-right">
			{{ \App\Utils\Util::format_currency($contact->balance - $ledger_details['total_reverse_payment'], $business->currency) }}
			@if($business->second_currency_id > 0)
				/ 
			{{ \App\Utils\Util::format_currency(0 - $ledger_details['total_reverse_payment_currency2'], $business->secondCurrency) }} 
			@endif
		</td>
	</tr>
		...
		<tbody>
			@foreach($ledger_details['ledger'] as $data)
				...
				@endif>
					...
					<td class="ws-nowrap align-right">
						{{ \App\Utils\Util::format_currency(abs($data['balance']), $data->currency) }}
						@if($data['balance'] < 0)
							@lang('lang_v1.dr')
						@else
							@lang('lang_v1.cr')
