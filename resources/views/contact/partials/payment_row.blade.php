<tr>
    ...
    @endif
    <td>
        {{ !empty($payment->contact->supplier_business_name) ? $payment->contact->supplier_business_name : $payment->contact->name}}
    </td>
    ...
    <td @if($count_child_payments > 0) class="bg-gray" @endif>
        <span class="paid-amount" data-orig-value=" {{$payment->amount}}">
            {{ \App\Utils\Util::format_currency($payment->amount, $currency) }}
        </span>
    </td>
    <td @if($count_child_payments > 0) class="bg-gray" @endif>
        @php
    ...
    <td>
        @if($payment->parent_pymo_invoice)
            <a href='/uploads/invoices/{{$payment->business_id}}/{{$payment->parent_pymo_invoice}}.pdf' class='ms-2' target='_blank'>
                <div>{{$payment->parent_document_type}}</div>
                <div>( {{$payment->parent_pymo_serie}} )</div>
            </a>
        @elseif($payment->transaction_id)
            <a href='/invoice/transaction/{{$payment->transaction_id}}' class='ms-2' target='_blank'>@lang('invoice.internal')</a>
        @endif
    </td>
    <td>
        @if($payment->pymo_invoice)
            <a href='/uploads/invoices/{{$payment->business_id}}/{{$payment->pymo_invoice}}.pdf' class='ms-2' target='_blank'>
                <div>{{$payment->document_type}}</div>
                <div>( {{$payment->pymo_serie}} )</div>
            </a>
        @endif
    </td>
    <td @if($count_child_payments > 0) class="bg-gray" @endif>
        ...
        @if((in_array($transaction_type, ['purchase', 'purchase_return']) && auth()->user()->can('delete_purchase_payment')) || 
