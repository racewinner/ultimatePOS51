<table class="table table-slim mb-0 bg-light-gray" @if(!empty($for_pdf)) style="width: 100%;" @endif>
  ...
  @foreach($purchase->purchase_lines as $purchase_line)
    <tr>
      ...
      <td >
        {{ \App\Utils\Util::format_currency($purchase_line->pp_without_discount, $purchase->currency) }}
      </td>
      <td >{{@num_format($purchase_line->discount_percent)}} %</td>
      <td >
        {{ \App\Utils\Util::format_currency($purchase_line->item_tax, $purchase->currency) }}
      </td>
      <td >
        {{ \App\Utils\Util::format_currency($purchase_line->purchase_price_inc_tax, $purchase->currency) }}
      </td>
      <td >
        {{ \App\Utils\Util::format_currency($purchase_line->purchase_price_inc_tax * $purchase_line->quantity, $purchase->currency) }}
      </td>
    </tr>
  @endforeach
</table>