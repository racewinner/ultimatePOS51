...
<div class="col-md-12 col-sm-12 @if(!empty($for_pdf)) width-100 @endif">
	@php
		...
		$amount_due_currency2 = 0;
		$current_due_currency2 = 0;
		$due_1_30_days_currency2 = 0;
		$due_30_60_days_currency2 = 0;
		$due_60_90_days_currency2 = 0;
		$due_over_90_days_currency2 = 0;
	@endphp
	@if(!empty($for_pdf))
	<br>
	@endif
		...
		<tbody>
			@foreach($ledger_details['ledger'] as $data)
				@php
					...
					if($data['payment_status'] != 'paid' && !empty($data['due_date'])) {
						...
						if($data['is_maincurrency']) {
							...
							$$amount_due_currency2 += $data['total_due'];

							$days_diff = $data['due_date']->diffInDays();
							if($days_diff == 0){
								$current_due_currency2 += $data['total_due'];
							} elseif ($days_diff > 0 && $days_diff <= 30) {
								$due_1_30_days_currency2 += $data['total_due'];
							} elseif ($days_diff > 30 && $days_diff <= 60) {
								$due_30_60_days_currency2 += $data['total_due'];
							} elseif ($days_diff > 60 && $days_diff <= 90) {
								$due_60_90_days_currency2 += $data['total_due'];
							} elseif ($days_diff > 90) {
								$due_over_90_days_currency2 += $data['total_due'];
							}
						}
					}
				@endphp
				<tr @if(!empty($for_pdf) && $loop->iteration % 2 == 0) class="odd" @endif style="border:hidden;">
					...
					<td>
						{{ \App\Utils\Util::format_currency($data['final_total'], $data['currency']) }}
					</td>
					<td>
						{{ \App\Utils\Util::format_currency($data['total_due'], $data['currency']) }}
					</td>
				</tr>
			@endforeach
	...
	<table class="table" style="margin-top: 0;">
		...
		<tr>
			<td style="text-align: center;">
				{{ \App\Utils\Util::format_currency($current_due, $business->currency) }}
				@if($business->second_currency_id > 0)
				 / 
				 {{ \App\Utils\Util::format_currency($current_due_currency2, $business->secondCurrency) }}
				@endif
			</td>
			<td style="color: #2dce89 !important; text-align: center;">
				{{ \App\Utils\Util::format_currency($due_1_30_days, $business->currency) }}
				@if($business->second_currency_id > 0)
				 / 
				 {{ \App\Utils\Util::format_currency($due_1_30_days_currency2, $business->secondCurrency) }}
				@endif
			</td>
			<td style="color: #ffd026 !important; text-align: center;">
				{{ \App\Utils\Util::format_currency($due_30_60_days, $business->currency) }}
				@if($business->second_currency_id > 0)
				 / 
				 {{ \App\Utils\Util::format_currency($due_30_60_days_currency2, $business->secondCurrency) }}
				@endif
			</td>
			<td style="color: #ffa100 !important;text-align: center;">
				{{ \App\Utils\Util::format_currency($due_60_90_days, $business->currency) }}
				@if($business->second_currency_id > 0)
				 / 
				 {{ \App\Utils\Util::format_currency($due_60_90_days_currency2, $business->secondCurrency) }}
				@endif
			</td>
			<td style="color: #f5365c !important; text-align: center;">
				{{ \App\Utils\Util::format_currency($due_over_90_days, $business->currency) }}
				@if($business->second_currency_id > 0)
				 / 
				 {{ \App\Utils\Util::format_currency($due_over_90_days_currency2, $business->secondCurrency) }}
				@endif
			</td>
			<td style="text-align: center;">
				{{ \App\Utils\Util::format_currency($amount_due, $business->currency) }}
				@if($business->second_currency_id > 0)
				 / 
				 {{ \App\Utils\Util::format_currency($amount_due_currency2, $business->secondCurrency) }}
				@endif
			</td>
		</tr>
	</table>
