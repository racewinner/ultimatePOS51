...
@foreach($pdf_generation_for as $pdf_for)
	...
	<table class="tpdf" style="width:100%;">
		<tr>
			<td class="width-50">
				<strong>@lang('lang_v1.po_no'):</strong> #{{ $purchase->ref_no }} <br>
				<strong>@lang('lang_v1.order_date'):</strong> {{ @format_date($purchase->transaction_date) }} {{ @format_time($purchase->transaction_date) }} <br/>
				<strong>@lang('lang_v1.user'):</strong> {{$causer->username}}
			</td>
			<td class="width-50">
	...
	<table class="table-pdf td-border">
		...
		@foreach($purchase->purchase_lines as $purchase_line)
			<tr @if($loop->iteration % 2 !== 0) class="odd" @endif style="border:hidden;">
				...
				<td>
					{{ \App\Utils\Util::format_currency($purchase_line->purchase_price, $purchase->currency) }}
				</td>
				<td>
					@php 
		            ...
		            @endphp
					{{ \App\Utils\Util::format_currency($purchase_line->quantity * $purchase_line->purchase_price, $purchase->currency) }}
				</td>
			</tr>
		...
		<tr>
			...
			<td colspan="1">
				<strong>
					{{ \App\Utils\Util::format_currency($total, $purchase->currency) }}
				</strong>
			</td>
		</tr>
		<tr>
			...
			<td colspan="3">
				@if(!empty($tax_array))
		        	@foreach($tax_array as $key => $value)
		        		{{$taxes->where('id', $key)->first()->name}} ({{$taxes->where('id', $key)->first()->amount}}%) : {{ \App\Utils\Util::format_currency(array_sum($value), $purchase->currency) }} <br>
		        	@endforeach
		        @endif
				
				{{$invoice_layout->total_label}} : {{ \App\Utils\Util::format_currency($purchase->final_total, $purchase->currency) }}
			</td>
		</tr>
		<tr>
			<td colspan="6">
