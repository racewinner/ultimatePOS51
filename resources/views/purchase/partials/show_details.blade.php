  ...
  <div class="row invoice-info">
    <div class="col-sm-4 invoice-col">
      ...
      <address>
        ...
        @endif
        @if(!empty($purchase->nation_exchg_rate))
        <br>@lang('lang_v1.currency_exchange_rate'): {{number_format((float)$purchase->nation_exchg_rate, 2, '.', '')}}
        @endif
      </address>

      @if($purchase->document_path)
        <a href="{{$purchase->document_path}}" 
        download="{{$purchase->document_name}}" class="btn btn-sm btn-success pull-left no-print me-2">
          <i class="fa fa-download"></i> 
            &nbsp;{{ __('purchase.download_document') }}
        </a>
      @endif

      @if($purchase->document2_path)
        <a href="{{$purchase->document2_path}}" 
        download="{{$purchase->document2_name}}" class="btn btn-sm btn-success pull-left no-print">
          <i class="fa fa-download"></i> 
            &nbsp;{{ __('purchase.download_document') }}2
        </a>
      @endif
    </div>
        ...
        <table class="table bg-gray">
          ...
          @foreach($purchase->purchase_lines as $purchase_line)
            <tr>
              ...
              <td class="text-right"><span>{{ \App\Utils\Util::format_currency($purchase_line->pp_without_discount, $purchase->currency) }}</span></td>
              <td class="text-right"><span class="display_currency">{{ $purchase_line->discount_percent}}</span> %</td>
              <td class="no-print text-right"><span>{{ $purchase_line->purchase_price }}</span></td>
              <td class="no-print text-right"><span>{{ $purchase_line->quantity * $purchase_line->purchase_price }}</span></td>

              <td class="text-right">
                <span>{{ \App\Utils\Util::format_currency($purchase_line->item_tax, $purchase->currency) }} </span> 
                <br/>
                <small>
                  @if(!empty($taxes[$purchase_line->tax_id])) ( {{ $taxes[$purchase_line->tax_id]}} ) 
                  @endif
                </small>
              </td>

              <td class="text-right"><span>{{ \App\Utils\Util::format_currency($purchase_line->purchase_price_inc_tax, $purchase->currency) }}</span></td>
              @if($purchase->type != 'purchase_order')
              @if(session('business.enable_lot_number'))
              ...
              @endif
              @endif
              <td class="text-right"><span>{{ \App\Utils\Util::format_currency($purchase_line->purchase_price_inc_tax * $purchase_line->quantity, $purchase->currency) }}</span></td>
            </tr>
            @php 
        ...
        <table class="table">
          ...
          @forelse($purchase->payment_lines as $payment_line)
            ...
            <tr>
              ...
              <td><span>{{ \App\Utils\Util::format_currency($payment_line->amount, $purchase->currency) }}</span></td>
      ...
      <div class="table-responsive">
        <table class="table">
          ...
          <tr>
            ...
            <td></td>
            <td><span class="pull-right">{{ \App\Utils\Util::format_currency($total_before_tax, $purchase->currency) }}</span></td>
          </tr>
          <tr>
            ...
            <td>
              <span class="pull-right">
                @if($purchase->discount_type == 'percentage')
                  {{ \App\Utils\Util::format_currency($purchase->discount_amount * $total_before_tax / 100, $purchase->currency)}}
                @else
                  {{ \App\Utils\Util::format_currency($purchase->discount_amount, $purchase->currency)}}
                @endif                  
              </span>
            </td>
          </tr>
            ...
            <td class="text-right">
                @if(!empty($purchase_taxes))
                  @foreach($purchase_taxes as $k => $v)
                    ...
                    <span class="pull-right">{{ \App\Utils\Util::format_currency($v, $purchase->currency) }}</span><br>
                  @endforeach
                @else
                0.00
                @endif
          ...
          <tr>
            ...
            <td></td>
            <td><span class="pull-right">{{ \App\Utils\Util::format_currency($purchase->final_total, $purchase->currency) }}</span></td>
          </tr>
        </table>
      </div>
    </div>
