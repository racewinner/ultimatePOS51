<tr class="product_row">
	<td>
		...
		<i class="fa fa-commenting cursor-pointer text-primary add-pos-row-description"
			title="@lang('lang_v1.add_description')" data-toggle="modal"
	...
	<td>
		...
		<div class="input-group input-number">
			...
			<input type="text" class="form-control pos_quantity input_number mousetrap"
				value="{{@num_format($product->quantity_ordered)}}" name="products[{{$row_count}}][quantity]"
				@if($product->unit_allow_decimal == 1) data-decimal=1 @else data-decimal=0 data-rule-abs_digit="true"
	...
	<td>
		<input type="text" name="products[{{$row_count}}][unit_price]"
			...
			value="{{@num_format($product->default_sell_price)}}">
	</td>
	@php
		$hide_tax = 'hide';
	...
	<td>
		<input type="text" readonly name="products[{{$row_count}}][price]" class="form-control pos_line_total"
