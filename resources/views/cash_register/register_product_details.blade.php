...
<div class="row">
  <div class="col-md-12">
    <hr>
    <h3>@lang('lang_v1.product_return_details_register')</h3>
    <table class="table table-condensed">
      <tr>
        <th>#</th>
        <th>@lang('product.sku')</th>
      ...
      @endphp
      @foreach($details['product_details'] as $detail)
        @if(floatval($detail->return_quantity) <= 0)
          @continue
        @else
          @php $cnt++; @endphp
        @endif
        <tr>
          ...
          <td>
            {{@format_quantity($detail->return_quantity)}}
            @php
              $total_quantity += $detail->return_quantity;
            @endphp
          </td>
          <td>
            <span class="display_currency" data-currency_symbol="true">
      ...
      @php
        $total_amount += ($details['transaction_details']->return_total_tax - $details['transaction_details']->return_total_discount);
      @endphp
      ...
      <tr class="success">
        ...
        <th>
          ...
          @if($details['transaction_details']->return_total_tax != 0)
            @lang('sale.order_tax'): (+)
            <span class="display_currency" data-currency_symbol="true">
              {{$details['transaction_details']->return_total_tax}}
            </span>
            <br/>
          @endif

          @if($details['transaction_details']->return_total_discount != 0)
            @lang('sale.discount'): (-)
            <span class="display_currency" data-currency_symbol="true">
              {{$details['transaction_details']->return_total_discount}}
            </span>
            <br/>
          @endif
