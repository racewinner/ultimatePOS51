<div class="modal-dialog" role="document">
  <div class="modal-content">
    ...
    @endif
      ...
      @if($mercadolibre_connected)
      @include('taxonomy.partials.mercadolibre_category')
    @else
      <div class="p-2">
      <a href="{{ $mercadolibre_auth_url }}">Connect to Mercado Libre</a>
      </div>
    @endif
    </div>
...
<script type="text/javascript">
  curr_meli_attributes = {!! json_encode($category->mercadolibre_details->mercadolibre_attributes ?? []) !!};
  curr_meli_item_condition = "{!! $category->mercadolibre_details->mercadolibre_item_condition ?? '' !!}";

  get_ml_category("{{ $category->mercadolibre_category_id ?? '' }}");
</script>