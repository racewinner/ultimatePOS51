$(document).ready(function() {
    ...
    contact_table = $('#contact_table').DataTable({
        ...
        "footerCallback": function ( row, data, start, end, display ) {
            ...
            var total_return_due_currency2 = 0;

            for (var r in data){
                total_due += $(data[r].due).data('orig-value') ? parseFloat($(data[r].due).data('orig-value')) : 0;
                total_due_currency2 += $(data[r].due).data('orig-value-currency2') ? parseFloat($(data[r].due).data('orig-value-currency2')) : 0;

                total_return_due += $(data[r].return_due).data('orig-value') ? parseFloat($(data[r].return_due).data('orig-value')) : 0;
                total_return_due_currency2 += $(data[r].return_due).data('orig-value-currency2') ? parseFloat($(data[r].return_due).data('orig-value-currency2')) : 0;
            }
            $('.footer_contact_due').html(
            ...
            $('.footer_contact_return_due').html(
                __currency_trans_from(total_return_due, true, currencies[0]) + 
                (currencies.length > 1 ? ' / ' + __currency_trans_from(total_return_due, true, currencies[1]) : '')
            );
        }
    });
    ...
    $('.contact_modal').on('shown.bs.modal', function(e) {
        ...
        $("input[name='contact_type_radio']").change(function(e) {
            if(e.currentTarget.value == 'individual') {
                $("#first_name").rules('add', {required: true});
                $("#rut").rules('remove', 'exactlength');
                $('#rut').rules('add', {exactlength: 8});
            } else if(e.currentTarget.value == 'business') {
                $("#first_name").rules('remove');
                $("#rut").rules('remove', 'exactlength');
                $('#rut').rules('add', {exactlength: 12});
            }
        })
        ...
        $('form#contact_add_form, form#contact_edit_form')
            ...
            .validate({
                ...
                errorPlacement: function(error, element) {
                    if(typeof(element) === 'object' && element[0].name === 'contact_type_radio') {
    ...
    expense_table = $('#expense_table').DataTable({
        ...
        fnDrawCallback: function(row, data, start, end, display) {
            ...
            var expense_total_currency2 = sum_table_col($('#expense_table'), 'final-total', 'orig-value-currency2');
            var total_due = sum_table_col($('#expense_table'), 'payment_due');
            var total_due_currency2 = sum_table_col($('#expense_table'), 'payment_due', 'orig-value-currency2');

            $('.footer_expense_total').html(
                __currency_trans_from(expense_total, true, currencies[0]) + 
                (currencies.length > 1 ? ' / ' + __currency_trans_from(expense_total_currency2, true, currencies[1]) : '')
            );
        ...
        $.ajax({
            ...
            success: function(result) {
                $('#closing_stock_by_sp').html(
                    __currency_trans_from(result.closing_stock_by_sp.stock, true, currencies[0]) +
                     (currencies.length > 1 ? " / " + __currency_trans_from(result.closing_stock_by_sp.stock_currency2, true, currencies[1]) : '')
                );
                $('#opening_stock_by_sp').html(
                    __currency_trans_from(result.opening_stock_by_sp.stock, true, currencies[0]) +
                    (currencies.length > 1 ? " / " + __currency_trans_from(result.opening_stock_by_sp.stock_currency2, true, currencies[1]) : '')
                );
            },
        });
    }
...
$(document).on('click', '.combo-product-tag', function(e) {
    e.preventDefault();
    e.stopPropagation();

    const parentEl = $(e.target).closest('.has-combo-variations');
    const iEl = $(e.target).closest('.combo-product-tag').find('i');

    if(parentEl) {
        if($(parentEl).hasClass('show-combo')) {
            $(parentEl).removeClass('show-combo');
            $(iEl).removeClass('fa-angle-up');
            $(iEl).addClass('fa-angle-down');
        } else {
            $(parentEl).addClass('show-combo');
            $(iEl).removeClass('fa-angle-down');
            $(iEl).addClass('fa-angle-up');
        }
    }
