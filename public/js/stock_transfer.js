...
$(document).on('change', 'select[name="currency_id"]', function(e){
    ...
    $("table#stock_adjustment_product_table > tbody tr").toArray().forEach(tr => {
        apply_exchange_rate($(tr), r);
        $(tr).find("input.product_unit_price").trigger("change");
    });
});

function apply_exchange_rate(row_obj, r) {
    let obj = row_obj.find('input.product_unit_price');
    let v = __read_number(obj);
    __write_number(obj, v * r);

    obj = row_obj.find('input.product_line_total');
    v = __read_number(obj);
    __write_number(obj, v * r);
    
    obj = row_obj.find('input.hidden_base_unit_price');
    v = __read_number(obj);
    __write_number(obj, v * r);
}
...
function update_table_total() {
    ...
    $('span#total_adjustment').text(__currency_trans_from_en(table_total, true, true));
    ...
    $('span#final_total_text').text(__currency_trans_from_en(table_total, true, true));
...
function update_table_row(tr) {
    ...
    let unit_price = __read_number(tr.find('input.product_unit_price'));
    var row_total = 0;
