...
function __currency_trans_from_en2(
    input,
    show_symbol = true,
    ...
    if (use_page_currency && p_currency) {
        var s = p_currency.symbol;
        var thousand = p_currency.thousand_separator;
        var decimal = p_currency.decimal_separator;
    } else {
...
function __currency_trans_from(
    input,
    show_symbol = true,
    currency,
    precision = __currency_precision,
    ...
    return __currency_trans_from_en2(input, show_symbol, true, currency, precision, is_quantity);
}
...
function __currency_trans_from_en(
    ...
    if (use_page_currency && __p_currency_symbol) {
        p_currency = {
            symbol: __p_currency_symbol,
            thousand_separator: __p_currency_thousand_separator,
            decimal_separator: __p_currency_decimal_separator
        };
    }
    return __currency_trans_from_en2(input, show_symbol, use_page_currency, p_currency, precision, is_quantity);
}
...
function __select2(selector) {
    for (let i = 0; i < selector.length; i++) {
        ...
        const option = {}
        if ($('html').attr('dir') == 'rtl') option.dir = 'rtl';
        if ($(el).attr('allowClear') !== undefined) option.allowClear = true;
...
function sum_table_col(table, class_name, data_field = null) {
    var sum = 0;
    if (data_field === null) data_field = 'orig-value';
    table
        .find('tbody')
        .find('tr')
        .each(function () {
...
function floatV(v, decimalPos = 2) {
    if (!v) return 0;
    if (typeof (v) === 'string') v = parseFloat(v);
    const fv = parseFloat(v.toFixed(decimalPos));
    return fv;
}
