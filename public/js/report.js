$(document).ready(function() {
    ...
    supplier_report_tbl = $('#supplier_report_tbl').DataTable({
        ...
        fnDrawCallback: function(oSettings) {
            $('#footer_total_purchase').text(
                __currency_trans_from(sum_table_col($('#supplier_report_tbl'), 'total_purchase'), true, currencies[0]) + 
                (currencies.length > 1 ?  ' / ' + __currency_trans_from(sum_table_col($('#supplier_report_tbl'), 'total_purchase', 'orig-value-currency2'), true, currencies[1]) : '')
            );

            $('#footer_total_purchase_return').text(
                __currency_trans_from(sum_table_col($('#supplier_report_tbl'),'total_purchase_return'), true, currencies[0]) + 
                (currencies.length > 1 ?  ' / ' + __currency_trans_from(sum_table_col($('#supplier_report_tbl'),'total_purchase_return', 'orig-value-currency2'), true, currencies[1]) : '')
             );

            $('#footer_total_sell').text(
                __currency_trans_from(sum_table_col($('#supplier_report_tbl'), 'total_invoice'), true, currencies[0]) +
                (currencies.length > 1 ?  ' / ' + __currency_trans_from(sum_table_col($('#supplier_report_tbl'), 'total_invoice', 'orig-value-currency2'), true, currencies[1]) : '')
            );

            $('#footer_total_sell_return').text(
                __currency_trans_from(sum_table_col($('#supplier_report_tbl'), 'total_sell_return'), true, currencies[0]) +
                (currencies.length > 1 ?  ' / ' + __currency_trans_from(sum_table_col($('#supplier_report_tbl'), 'total_sell_return', 'orig-value-currency2'), true, currencies[1]) : '')
            );

            $('#footer_total_opening_bal_due').text(
                __currency_trans_from(sum_table_col($('#supplier_report_tbl'), 'opening_balance_due'), true, currencies[0]) +
                (currencies.length > 1 ?  ' / ' + __currency_trans_from(sum_table_col($('#supplier_report_tbl'), 'opening_balance_due', 'orig-value-currency2'), true, currencies[1]) : '')
            );

            $('#footer_total_due').text(
                __currency_trans_from(sum_table_col($('#supplier_report_tbl'), 'total_due'), true, currencies[0]) +
                (currencies.length > 1 ?  ' / ' + __currency_trans_from(sum_table_col($('#supplier_report_tbl'), 'total_due', 'orig-value-currency2'), true, currencies[1]) : '')
            );
        },
    });
    ...
    var stock_report_cols = [
        ...
        if ($('th.stock_price').length) {
            ...
            stock_report_cols.push({ data: 'stock_value_by_sale_price', searchable: false, orderable: false });
            if ($('th.stock_price_currency2').length) {
                stock_report_cols.push({ data: 'stock_value_by_sale_price_currency2', searchable: false, orderable: false });
            }
    ...
    stock_report_table = $('#stock_report_table').DataTable({
        ...
        "footerCallback": function ( row, data, start, end, display ) {
            ...
            var total_stock_price_currency2 = 0;
            var footer_stock_value_by_sale_price = 0;
            var footer_stock_value_by_sale_price2 = 0;
            var footer_stock_value_by_sale_price_currency2 = 0;
            var total_potential_profit = 0;
            var total_potential_profit_currency2 = 0;
            ...
            for (var r in data){
                ...
                total_stock_price_currency2 += $(data[r].stock_price).data('orig-value-currency2') ? 
                                    parseFloat($(data[r].stock_price).data('orig-value-currency2')) : 0;
                ...
                footer_stock_value_by_sale_price2 += $(data[r].stock_value_by_sale_price).data('orig-value-currency2') ? 
                                    parseFloat($(data[r].stock_value_by_sale_price).data('orig-value-currency2')) : 0;

                footer_stock_value_by_sale_price_currency2 += $(data[r].stock_value_by_sale_price_currency2).data('orig-value-currency2') ? 
                                    parseFloat($(data[r].stock_value_by_sale_price_currency2).data('orig-value-currency2')) : 0;

                total_potential_profit += $(data[r].potential_profit).data('orig-value') ? 
                                    parseFloat($(data[r].potential_profit).data('orig-value')) : 0;

                total_potential_profit_currency2 += $(data[r].potential_profit).data('orig-value-currency2') ? 
                                    parseFloat($(data[r].potential_profit).data('orig-value-currency2')) : 0;
            ...
            $('.footer_total_stock_price').html(
                __currency_trans_from(total_stock_price, true, currencies[0]) +
                (currencies.length > 1 ? ' / ' + __currency_trans_from(total_stock_price_currency2, true, currencies[1]) : '')
            );
            ...
            $('.footer_stock_value_by_sale_price').html(
                __currency_trans_from(footer_stock_value_by_sale_price, true, currencies[0]) + 
                (currencies.length > 1 ? ' / ' + __currency_trans_from(footer_stock_value_by_sale_price2, true, currencies[1]) : '')
            );
            if($('.footer_stock_value_by_sale_price_currency2').length) {
                $('.footer_stock_value_by_sale_price_currency2').html(
                    __currency_trans_from(footer_stock_value_by_sale_price_currency2, true, currencies[1])
                );
            }
            $('.footer_potential_profit').html(
                __currency_trans_from(total_potential_profit, true, currencies[0]) +
                (currencies.length > 1 ? ' / ' + __currency_trans_from(total_potential_profit_currency2, true, currencies[1]) : '')
            );
            if ($('th.current_stock_mfg').length) {
    ...
    $('#stock_report_filter_form #location_id, #stock_report_filter_form #category_id, #stock_report_filter_form #sub_category_id, #stock_report_filter_form #brand, #stock_report_filter_form #unit,#stock_report_filter_form #view_stock_filter, #to_date'
    ).change(function() {
    ...
    if ($('#sr_date_filter').length == 1) {
        ...
        sr_sales_report = $('table#sr_sales_report').DataTable({
            ...
            fnDrawCallback: function(oSettings) {
                $('#sr_footer_sale_total').html(
                    __currency_trans_from(sum_table_col($('#sr_sales_report'), 'final-total'), true, currencies[0]) + 
                    (currencies.length > 1 ? ' / ' + __currency_trans_from(sum_table_col($('#sr_sales_report'), 'final-total', 'orig-value-currency2'), true, currencies[1]) : '')
                );

                $('#sr_footer_total_paid').html(
                    __currency_trans_from(sum_table_col($('#sr_sales_report'), 'total-paid'), true, currencies[0]) +
                    (currencies.length > 1 ? ' / ' + __currency_trans_from(sum_table_col($('#sr_sales_report'), 'total-paid', 'orig-value-currency2'), true, currencies[1]) : '' )
                );

                $('#sr_footer_total_remaining').html(
                    __currency_trans_from(sum_table_col($('#sr_sales_report'), 'payment_due'), true, currencies[0]) + 
                    (currencies.length > 1 ? ' / ' +  __currency_trans_from(sum_table_col($('#sr_sales_report'), 'payment_due', 'orig-value-currency2'), true, currencies[1]) : '')
                );
                $('#sr_footer_total_sell_return_due').html(
                    __currency_trans_from(sum_table_col($('#sr_sales_report'), 'sell_return_due'), true, currencies[0]) + 
                    (currencies.length > 1 ? ' / ' + __currency_trans_from(sum_table_col($('#sr_sales_report'), 'sell_return_due', 'orig-value-currency2'), true, currencies[1]) : '')
                );
    ...
    product_purchase_report = $('table#product_purchase_report_table').DataTable({
        ...
        fnDrawCallback: function(oSettings) {
            $('#footer_subtotal').html(
                __currency_trans_from(sum_table_col($('#product_purchase_report_table'), 'row_subtotal'), true, currencies[0]) + 
                (currencies.length > 1 ? ' / ' + __currency_trans_from(sum_table_col($('#product_purchase_report_table'), 'row_subtotal', 'orig-value-currency2'), true, currencies[1]) : '')
            );
    ...
    if ($('table#product_sell_report_table').length == 1) {
        ...
        product_sell_report = $('table#product_sell_report_table').DataTable({
            ...
            fnDrawCallback: function(oSettings) {
                $('#footer_subtotal').html(
                    __currency_trans_from(sum_table_col($('#product_sell_report_table'), 'row_subtotal'), true, currencies[0]) + 
                    (currencies.length > 1 ? ' / ' + __currency_trans_from(sum_table_col($('#product_sell_report_table'), 'row_subtotal', 'orig-value-currency2'), true, currencies[1]) : '')
                );
                $('#footer_total_sold').html(__sum_stock($('#product_sell_report_table'), 'sell_qty'));
                $('#footer_tax').html(
                    __currency_trans_from(sum_table_col($('#product_sell_report_table'), 'tax'), true, currencies[0]) + 
                    (currencies.length > 1 ? ' / ' + __currency_trans_from(sum_table_col($('#product_sell_report_table'), 'tax', 'orig-value-currency2'), true, currencies[1]) : '')
                );
            },
        });
    }
    ...
    product_sell_grouped_report = $('table#product_sell_grouped_report_table').DataTable({
        ...
        fnDrawCallback: function(oSettings) {
            $('#footer_grouped_subtotal').text(
                __currency_trans_from(sum_table_col($('#product_sell_grouped_report_table'), 'row_subtotal'), true, currencies[0]) + 
                (currencies.length > 1 ? ' / ' + __currency_trans_from(sum_table_col($('#product_sell_grouped_report_table'), 'row_subtotal', 'orig-value-currency2'), true, currencies[0]) : '')
            );
    ...
    purchase_invoice_payment_report = $('table#purchase_invoice_payment_report_table').DataTable({
        processing: true,
        serverSide: true,
        aaSorting: [[2, 'desc']],
        ajax: {
            url: '/reports/purchase-invoice-payment-report',
            data: function(d) {
                d.supplier_id = $('select#supplier_id').val();
                d.location_id = $('select#location_id').val();
                var start = '';
                var end = '';
                if ($('input#pipr_date_filter').val()) {
                    start = $('input#pipr_date_filter')
                        .data('daterangepicker')
                        .startDate.format('YYYY-MM-DD');
                    end = $('input#pipr_date_filter')
                        .data('daterangepicker')
                        .endDate.format('YYYY-MM-DD');
                }
                d.start_date = start;
        ...
        columns: [
            ...
            { data: 'currency', name: 'currency_id', searchable: false, orderable: false},
            { data: 'final_total', name: 'final_total', searchable: false, orderable: false},
            { data: 'amount_paid', name: 'paid_total', searchable: false, orderable: false},
            ...
            { data: 'tax_amount', name: 'tax_amount', searchable: false, orderable: false},
            { data: 'payment_due', name: 'payment_due', searchable: false, orderable: false },
        ],
    });
    $('#purchase_invoice_payment_report_form #location_id, #purchase_invoice_payment_report_form #supplier_id').change(function() {
        purchase_invoice_payment_report.ajax.reload();
    });
    if ($('#pipr_date_filter').length == 1) {
        $('#pipr_date_filter').daterangepicker(dateRangeSettings, function(start, end) {
            $('#pipr_date_filter span').val(
                start.format(moment_date_format) + ' ~ ' + end.format(moment_date_format)
            );
            purchase_invoice_payment_report.ajax.reload();
        });
        $('#pipr_date_filter').on('cancel.daterangepicker', function(ev, picker) {
            $('#pipr_date_filter').val('');
            purchase_invoice_payment_report.ajax.reload();
        });
    }
    ...
    purchase_payment_report = $('table#purchase_payment_report_table').DataTable({
        ...
        fnDrawCallback: function(oSettings) {
            ...
            var total_amount_currency2 = sum_table_col($('#purchase_payment_report_table'), 'paid-amount', 'orig-value-currency2');
            $('#footer_total_amount').html(
                __currency_trans_from(total_amount, true, currencies[0]) +
                (currencies.length > 1 ? ' / ' + __currency_trans_from(total_amount_currency2, true, currencies[1]) : '')
            );
        },
    ...
    // Sell Invoice Payment Report
    sell_invoice_payment_report = $('table#sell_invoice_payment_report_table').DataTable({
        processing: true,
        serverSide: true,
        aaSorting: [[2, 'desc']],
        ajax: {
            url: '/reports/sell-invoice-payment-report',
            data: function(d) {
                d.customer_id = $('select#customer_id').val();
                d.location_id = $('select#location_id').val();
                ...
                var start = '';
                var end = '';
                if ($('input#sipr_date_filter').val()) {
                    start = $('input#sipr_date_filter')
                        .data('daterangepicker')
                        .startDate.format('YYYY-MM-DD');
                    end = $('input#sipr_date_filter')
                        .data('daterangepicker')
                        .endDate.format('YYYY-MM-DD');
                }
                d.start_date = start;
        ...
        columns: [
            { data: 'transaction_date', name: 'transaction_date', searchable: false, orderable: false },
            { data: 'ref_no', name: 'ref_no', orderable: false},
            { data: 'contact_name', orderable: false, searchable: false },
            { data: 'payment_status', orderable: false, searchable: false },
            { data: 'currency', searchable: false, orderable: false},
            { data: 'final_total', searchable: false, orderable: false },
            { data: 'amount_paid', name: 'paid_total', searchable: false, orderable: false},
            { data: 'payment_method', name: 'payment_method', searchable: false, orderable: false},
            { data: 'payment_type', name: 'payment_type', searchable: false, orderable: false},
            { data: 'tax_amount', searchable: false, orderable: false},
            { data: 'total_remaining', searchable: false, orderable: false},
        ],
    });
    if ($('#sipr_date_filter').length == 1) {
        $('#sipr_date_filter').daterangepicker(dateRangeSettings, function(start, end) {
            $('#sipr_date_filter span').val(
                start.format(moment_date_format) + ' ~ ' + end.format(moment_date_format)
            );
            sell_invoice_payment_report.ajax.reload();
        });
        $('#sipr_date_filter').on('cancel.daterangepicker', function(ev, picker) {
            $('#sipr_date_filter').val('');
            sell_invoice_payment_report.ajax.reload();
        });
    }

    $('#sell_invoice_payment_report_form #location_id, #sell_invoice_payment_report_form #customer_id, #sell_invoice_payment_report_form #customer_group_filter').change(
        function() {
            sell_invoice_payment_report.ajax.reload();
        }
    );
    ...
    sell_payment_report = $('table#sell_payment_report_table').DataTable({
        ...
        fnDrawCallback: function(oSettings) {
            ...
            var total_amount_currency2 = sum_table_col($('#sell_payment_report_table'), 'paid-amount', 'orig-value-currency2');
            $('#footer_total_amount').html(
                __currency_trans_from(total_amount, true, currencies[0]) + 
                (currencies.length > 1 ? ' / ' + __currency_trans_from(total_amount_currency2, true, currencies[1]) : '')
            );
    ...
    items_report_table = $('#items_report_table').DataTable({
        ...
        fnDrawCallback: function(oSettings) {
            ...
            //     __currency_trans_from(sum_table_col($('#items_report_table'), 'purchase_price'), true, currencies[0]) + 
            //     (currencies.length > 1 ? ' / ' + __currency_trans_from(sum_table_col($('#items_report_table'), 'purchase_price', 'orig-value-currency2'), true, currencies[1]) : '')
            // );
            // $('#footer_total_sp').html(
            //     __currency_trans_from(sum_table_col($('#items_report_table'), 'row_selling_price'), true, currencies[0]) +
            //     (currencies.length > 1 ? ' / ' + __currency_trans_from(sum_table_col($('#items_report_table'), 'row_selling_price', 'orig-value-currency2'), true, currencies[1]) : '')
            // );
            $('#footer_total_subtotal').html(
                __currency_trans_from(sum_table_col($('#items_report_table'), 'row_subtotal'), true, currencies[0]) + 
                (currencies.length > 1 ? ' / ' + __currency_trans_from(sum_table_col($('#items_report_table'), 'row_subtotal', 'orig-value-currency2'), true, currencies[1]) : '')
            );
            $('#footer_total_qty').html(
                sum_table_col($('#items_report_table'), 'quantity')
            );
        },
    });
...
function updatePurchaseSell() {
    ...
    $.ajax({
        ...
        success: function(data) {
            $('.total_purchase').html(
                __currency_trans_from(data.purchase.total_purchase_exc_tax, true, currencies[0]) +
                (currencies.length > 1 ? ' / ' + __currency_trans_from(data.purchase.total_purchase_exc_tax_currency2, true, currencies[1]): '')
            );
            $('.purchase_inc_tax').html(
                __currency_trans_from(data.purchase.total_purchase_inc_tax, true, currencies[0]) +
                (currencies.length > 1 ? ' / ' + __currency_trans_from(data.purchase.total_purchase_inc_tax_currency2, true, currencies[1]) : '')
            );
            $('.purchase_due').html(
                __currency_trans_from(data.purchase.purchase_due, true, currencies[0]) +
                (currencies.length > 1 ? ' / ' + __currency_trans_from(data.purchase.purchase_due_currency2, true, currencies[1]) : '')
            );
            $('.total_sell').html(
                __currency_trans_from(data.sell.total_sell_exc_tax, true, currencies[0]) +
                (currencies.length > 1 ? ' / ' + __currency_trans_from(data.sell.total_sell_exc_tax_currency2, true, currencies[1]) :'')
            );
            $('.sell_inc_tax').html(
                __currency_trans_from(data.sell.total_sell_inc_tax, true, currencies[0]) +
                (currencies.length > 1 ? ' / ' + __currency_trans_from(data.sell.total_sell_inc_tax_currency2, true, currencies[1]) : '')
            );
            $('.sell_due').html(
                __currency_trans_from(data.sell.invoice_due, true, currencies[0]) + 
                (currencies.length > 1 ? ' / ' + __currency_trans_from(data.sell.invoice_due_currency2, true, currencies[1]) : '')
            );
            ...
            $('.sell_minus_purchase').html(
                __currency_trans_from(data.difference.total, true, currencies[0]) +  
                (currencies.length > 1 ? ' / ' + __currency_trans_from(data.difference.total_currency2, true, currencies[1]) : '')
            );

            __highlight(data.difference.total, $('.sell_minus_purchase'));

            $('.difference_due').html(
                __currency_trans_from(data.difference.due, true, currencies[0]) +  
                (currencies.length > 1 ? ' / ' + __currency_trans_from(data.difference.due_currency2, true, currencies[1]) : '')
            );
...
function updateStockAdjustmentReport() {
    ...
    $.ajax({
        ...
        success: function(data) {
            $('.total_amount').html(
                __currency_trans_from(data.total_amount, true, currencies[0]) + 
                (currencies.length > 1 ? ' / ' + __currency_trans_from(data.total_amount_currency2, true, currencies[1]) : '' )
            );
            $('.total_recovered').html(
                __currency_trans_from(data.total_recovered, true, currencies[0]) +
                (currencies.length > 1 ? ' / ' + __currency_trans_from(data.total_recovered_currency2, true, currencies[1]) : '' )
            );
            $('.total_normal').html(
                __currency_trans_from(data.total_normal, true, currencies[0]) +
                (currencies.length > 1 ? ' / ' + __currency_trans_from(data.total_normal_currency2, true, currencies[1]) : '')
            );
            $('.total_abnormal').html(
                __currency_trans_from_en(data.total_abnormal, true, currencies[0]) +
                (currencies.length > 1 ? ' / ' + __currency_trans_from_en(data.total_abnormal_currency2, true, currencies[1]) : '')
            );
        },
    });
    ...
    $.ajax({
        ...
        success: function(data) {
            $('span#sr_total_sales').html(
                __currency_trans_from(data.total_sell_exc_tax, true, currencies[0]) +
                (currencies.length > 1 ? ' / ' + __currency_trans_from(data.total_sell_exc_tax_currency2, true, currencies[1]) : '')
            );
            $('span#sr_total_sales_return').html(
                __currency_trans_from(data.total_sell_return_exc_tax, true, currencies[0]) +
                (currencies.length > 1 ? ' / ' + __currency_trans_from(data.total_sell_return_exc_tax_currency2, true, currencies[1]) : '')
            );
...
function updateTaxReport() {
    ...
    $.ajax({
        ...
        success: function(data) {
            $('.tax_diff').html(
                __currency_trans_from(data.tax_diff, true, currencies[0]) + 
                (currencies.length > 1 ? ' / ' + __currency_trans_from(data.tax_diff_currency2, true, currencies[1]) : '')
            );
