$(document).ready(function() {
    ...
    $('#customer_id').on('select2:select', function(e) {
        ...
        $('#advance_balance_text').text(__currency_trans_from_en(data.balance), true, true);
        ...
        submitHandler: function(form) {
            ...
            if (cnf) {
                ...
                $.ajax({
                    ...
                    success: function(result) {
                        if (result.success == 1) {
                            const chkInvoice = $("input[name='pymo_sendinvoice']");
                            if(chkInvoice?.length > 0 && chkInvoice[0].checked) {
                                sendInvoiceOnCreate($(form), result);
                            } else {
    ...
    $('button#submit-sell, button#save-and-print').click(function(e) {
        ...
        if (sell_form.valid()) {
            ...
            const data = new FormData(sell_form[0]);
            $.ajax({
                method: 'POST',
                ...
                success: function(result) {
                    if (result.success == 1) {
                        const chkInvoice = $("input[name='pymo_sendinvoice']");
                        if(chkInvoice?.length > 0 && chkInvoice[0].checked) {
                            sendInvoiceOnCreate(sell_form, result);
                        } else {
    ...
    function sendInvoiceOnCreate (sell_form, createResult) {
        let form_data = sell_form.serialize();
        let bill_data = {};
        ...
        bill_data['transaction_id'] = createResult.transaction_id;
        bill_data['invoice_no'] = createResult.invoice_no;
        ...
        $.post('/pymo/sendInvoice', bill_data, function(response) {
            if (response.status && response.status === 'SUCCESS') {
                ...
                let html = "<i class='fa fa-download ms-4'></i>";
                html += `<a class='m-2 ps-4' href='/uploads/invoices/${createResult.business_id}/${response.invoice_id}.pdf' target='_blank'>Download Document</a>`;
                $(".electronic_bill_tab").html(html);

                setTimeout(function() {
                    const aTag = $(".electronic_bill_tab a");
                    aTag[0].click();
    ...
    $('div.view_modal').on('show.bs.modal', function() {
        __currency_convert_recursively($(this), true);
    });
...
function get_recent_transactions(status, element_obj) {
    ...
    $.ajax({
        ...
        success: function(result) {
            ...
            __currency_convert_recursively(element_obj, true);
        },
    });
}
        ...
        $.ajax({
            ...
            success: function(result) {
                if (result.success) {
                    ...
                    this_row.find('span.pos_line_total_text').text(__currency_trans_from_en(line_total, true, true));
                    ...
                    __currency_convert_recursively(this_row, true);
...
$(document).on('change', 'select[name="currency_id"]', function(e){
    ...
    $("table#pos_table > tbody tr").toArray().forEach(tr => {
        apply_exchange_rate($(tr), r);
        $(tr).find("input.pos_unit_price").trigger("change");
    });
});

function apply_exchange_rate(row_obj, r) {
    // To update pos_unit_price
    const input_pos_unit_price = row_obj.find('input.pos_unit_price');
    let v = __read_number(input_pos_unit_price);
    __write_number(input_pos_unit_price, v * r);

    // To update input_row_discount_amount
    const row_discount_type = row_obj.find('select.row_discount_type').val();
    if(row_discount_type === 'fixed') {
        const input_row_discount_amount = row_obj.find('input.row_discount_amount');
        v = __read_number(input_row_discount_amount);
        __write_number(input_row_discount_amount, v * r);
    }

    // To update base_unit_sell_price
    const input_base_unit_sell_price = row_obj.find('input.hidden_base_unit_sell_price');
    v = __read_number(input_base_unit_sell_price);
    __write_number(input_base_unit_sell_price, v * r);
}
...
function enable_sell_form_actions() {
    $('div.pos-processing').hide();
