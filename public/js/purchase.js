$(document).ready(function() {
    ...
    purchase_table = $('#purchase_table').DataTable({
        ...
        "footerCallback": function ( row, data, start, end, display ) {
            ...
            var total_purchase_currency2 = 0;
            var total_due = 0;
            var total_due_currency2 = 0;
            var total_purchase_return_due = 0;
            var total_purchase_return_due_currency2 = 0;
            var total_tax = 0;
            var total_tax_currency2 = 0;
            for (var r in data){
                total_purchase += $(data[r].final_total).data('orig-value') ? 
                                            parseFloat($(data[r].final_total).data('orig-value')) : 0;
                total_purchase_currency2 += $(data[r].final_total).data('orig-value-currency2') ? 
                                            parseFloat($(data[r].final_total).data('orig-value-currency2')) : 0;
                total_tax += $(data[r].tax_amount).data('orig-value') ? 
                                            parseFloat($(data[r].tax_amount).data('orig-value')) : 0;                                            
                total_tax_currency2 += $(data[r].tax_amount).data('orig-value-currency2') ? 
                                            parseFloat($(data[r].tax_amount).data('orig-value-currency2')) : 0;
                ...
                total_due_currency2 += payment_due_obj.find('.payment_due').data('orig-value-currency2') ? 
                                            parseFloat(payment_due_obj.find('.payment_due').data('orig-value-currency2')) : 0;
                ...
                total_purchase_return_due_currency2 += payment_due_obj.find('.purchase_return').data('orig-value-currency2') ? 
                                            parseFloat(payment_due_obj.find('.purchase_return').data('orig-value-currency2')) : 0;
            }

            $('.footer_purchase_total').html(
                __currency_trans_from(total_purchase, true, currencies[0]) + 
                ((currencies.length == 1) ? '' : ' / '+ __currency_trans_from(total_purchase_currency2, true, currencies[1]))
            );
            $('.footer_tax_total').html(
                __currency_trans_from(total_tax, true, currencies[0]) + 
                ((currencies.length == 1) ? '' : ' / '+ __currency_trans_from(total_tax_currency2, true, currencies[1]))
            );            
            $('.footer_total_due').html(
                __currency_trans_from(total_due, true, currencies[0]) + 
                ((currencies.length == 1) ? '' : ' / '+ __currency_trans_from(total_due_currency2, true, currencies[1]))
            );
            $('.footer_total_purchase_return_due').html(__currency_trans_from(total_purchase_return_due, true, currencies[0]));
...
function get_purchase_entry_row(product_id, variation_id) {
    if (product_id) {
        ...
        var purchase_currency_id = $("select[name='currency_id']").val();
        var data = { 
            product_id, 
            row_count, 
            variation_id,
...
function append_purchase_lines(data, row_count, trigger_change = false) {
    $(data)
        ...
        .each(function() {
            ...
            const cur_currency_id = $("select[name='currency_id']").val();
            const prod_currency_id = this_row.find("input.prod-currency-id").val();
            const nation_currency_id = $("input[name='nation_currency_id']").val();
            if( prod_currency_id && prod_currency_id != '0' && cur_currency_id != prod_currency_id) {
                const cur_exchg_rate = $("input[name='nation_exchg_rate']").val();
                const r = prod_currency_id == nation_currency_id ? 1 / cur_exchg_rate : cur_exchg_rate;
                apply_exchange_rate(this_row, r);
            }
...
$(document).on('change', 'select[name="currency_id"]', function(e){
    ...
    const nation_currency_id = $("input[name='nation_currency_id']").val();
    const cur_currency_id = $(e.currentTarget).find(':selected').val();
    const cur_exchg_rate = $("input[name='nation_exchg_rate']").val();
    const r = cur_currency_id == nation_currency_id ? cur_exchg_rate : 1/cur_exchg_rate;

    $("table#purchase_entry_table > tbody tr").toArray().forEach(tr => {
        apply_exchange_rate($(tr), r);
        $(tr).find("input.purchase_quantity").trigger("change");
    });
});

function apply_exchange_rate(row_obj, r) {
    let obj = row_obj.find('.purchase_unit_cost_without_discount');
    let v = __read_number(obj, true);
    __write_number(obj, v * r, true);

    obj = row_obj.find('.purchase_unit_cost');
    v = __read_number(obj, true);
    __write_number(obj, v * r, true);

    obj = row_obj.find('.row_subtotal_before_tax_hidden');
    v = __read_number(obj, true);
    __write_number(obj, v * r, true);
    row_obj.find('.row_subtotal_before_tax').text(
        __currency_trans_from_en(v * r, false, true)
    );
    
    obj = row_obj.find('.purchase_product_unit_tax');
    v = __read_number(obj, true);
    __write_number(obj, v * r, true);
    row_obj.find('.purchase_product_unit_tax_text').text(
        __currency_trans_from_en(v * r, false, true)
    );

    obj = row_obj.find('.purchase_unit_cost_after_tax');
    v = __read_number(obj, true);
    __write_number(obj, v * r, true);

    obj = row_obj.find('.row_subtotal_after_tax_hidden');
    v = __read_number(obj, true);
    __write_number(obj, v * r, true);
    row_obj.find('.row_subtotal_after_tax').text(
        __currency_trans_from_en(v * r, false, true)
    );

    row_obj.find("input.profit_percent").trigger('change');     // for the sake of changing default_sell_price
}
...
function update_inline_profit_percentage(row) {
    ...
    var profit_percent = __get_rate(purchase_after_tax, default_sell_price);
...
function onAddPurchaseFormSubmit() {
    // purchase items
    const addForm = $('form#add_purchase_form');
    ...
    $.ajax({
        ...
        success: function(result) {
            ...
            if(result.success == '1') {
                ...
                if(window.location.href.search("purchase-order") >= 0){
...
// $(document).on('change', 'select[name="currency_id"]', function(e){
//     const oSelect = $(e.currentTarget);
//     const option = oSelect.find(`option[value=${oSelect.val()}]`);
...
$(document).on('click', '#submit_purchase_form', function(e) {
    e.preventDefault();
    ...
    if ($('form#add_purchase_form').valid()) {
        // To check ref_no
        const ref_no = $('#ref_no').val();
        const contact_id = $('#supplier_id').val();
